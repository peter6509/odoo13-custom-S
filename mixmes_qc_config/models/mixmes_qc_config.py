...
class mixmesqcconfig(models.Model):
    ...
    barcode_ids = fields.One2many('barcode.line', 'qc_id', string="BARCODE line")
    pic_ids = fields.One2many('pic.line', 'qc_id', string="PICTURE Line")
...
class mixmesqcconfigbarcodeline(models.Model):
    ...
    def create(self, vals):
        if 'barcode_name' in vals and vals['barcode_name']:
            myres = self.env['barcode.line'].search_count([('barcode_name', '=', vals['barcode_name'])])
            if myres > 1:
...
class mixmesqcconfigpicline(models.Model):
    ...
    def create(self, vals):
        if 'pic_name' in vals and vals['pic_name']:
            myres = self.env['pic.line'].search_count([('pic_name', '=', vals['pic_name'])])
            if myres > 1:
