...
class HelpdeskTicket(models.Model):
    ...
    number = fields.Char(string="單據號碼", default="/", readonly=True)
    name = fields.Char(string="標題", required=True)
    description = fields.Html(required=True, sanitize_style=True,string="說明")
    user_id = fields.Many2one(
        comodel_name="res.users", string="指派人員", tracking=True, index=True
    )
    user_ids = fields.Many2many(
        comodel_name="res.users", related="team_id.user_ids", string="人員"
    )
    ...
    partner_id = fields.Many2one(comodel_name="res.partner", string="聯絡對象")
    partner_name = fields.Char(string="姓名/名稱")
    partner_email = fields.Char(string="Email")

    last_stage_update = fields.Datetime(
        string="最後階段變更時間", default=fields.Datetime.now
    )
    assigned_date = fields.Datetime(string="指派日期")
    closed_date = fields.Datetime(string="結案日期")
    closed = fields.Boolean(related="stage_id.closed")
    unattended = fields.Boolean(related="stage_id.unattended", store=True)
    tag_ids = fields.Many2many(comodel_name="helpdesk.ticket.tag", string="標籤")
    ...
    team_id = fields.Many2one(comodel_name="helpdesk.ticket.team", string="群組",)
    ...
    color = fields.Integer(string="色編")
