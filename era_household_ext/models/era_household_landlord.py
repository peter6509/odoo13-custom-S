...
class ERAHouseholdLandlord(models.Model):
    _name = "era.household_landlord"
    ...
    landlord_ar_line = fields.One2many('era.household_landlord_ar_line','landlord_id',copy=False)
    landlord_arpayment_line = fields.One2many('era.household_landlord_arpayment_line','landlord_id',copy=False)
    landlord_ap_line = fields.One2many('era.household_landlord_ap_line','landlord_id',copy=False)
    landlord_appayment_line = fields.One2many('era.household_landlord_appayment_line','landlord_id',copy=False)
    landlord_line_user = fields.One2many('era.household_landlord_line_user','landlord_id',copy=False)
    landlord_project_ids = fields.Many2many('era.household_house','era_landlord_household_house_rel','landlord_id','house_id',string="所有案場")
    landlord_line_message = fields.One2many('era.household_landlord_line_message','landlord_id',copy=False)
...
class ERAHouseholdLandlordARLine(models.Model):
    _name = "era.household_landlord_ar_line"
    ...
    landlord_id = fields.Many2one('era.household_landlord',ondelete='cascade')
    landlord_ar_ym = fields.Char(string="年月")
    project_no = fields.Many2one('era.household_house',string="租屋案場")
    house_id = fields.Many2one('era.household_house_line',string="房號")
    member_id = fields.Many2one('era.household_member',string="房客")
...
class ERAHouseholdLandlordARPaymentLine(models.Model):
    _name = "era.household_landlord_arpayment_line"
    _description ="房客已繳記錄"
    ...
    member_payment_ids = fields.Many2many('era.member_payment','era_landlord_ar_payment_rel','landlord_payment_line_id','payment_id',string="核銷單號")



class ERAHouseholdLandlordAPLine(models.Model):
    _name = "era.household_landlord_ap_line"
...
class ERAHouseholdLandlordLineUser(models.Model):
    _name = "era.household_landlord_line_user"
...
class ERAHouseholdLandlordLineMessage(models.Model):
    _name = "era.household_landlord_line_message"
    ...
    project_no = fields.Many2one('era.household_house', string="租屋案場")
    house_id = fields.Many2one('era.household_house_line', string="房號")
    member_id = fields.Many2one('era.member_line_user', string="LINE對象")
    line_type = fields.Selection([('1','帳單催繳通知'),('2','公告通知'),('3','修膳訊息')],string="訊息類型")
    send_date = fields.Date(string="發送日期")


class ERAHouseholdLandlordappaymentline(models.Model):
    _name = "era.household_landlord_appayment_line"
    ...
    pay_month = fields.Many2many('era.payment_month','era_month_landlord_appayment_rel','landlord_appayment_id','month_id',string="核銷月份",track_visibility="always")
    ...
    def create(self, vals):
        ...
        res = super(ERAHouseholdLandlordappaymentline, self).create(vals)
        return res
