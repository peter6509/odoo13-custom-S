    ...
    def init(self):
        ...
        self._cr.execute("""create or replace function checksecurity(mytable varchar,recid int,transid int,myuid int) returns Boolean as $BODY$
          ...
          BEGIN
             ...
             SQL = "'select flow_owner,manage_man1,manage_man2,manage_man3,manage_man4 into flowowner,flowman1,flowman2,flowman3,flowman4 from '|| mytable ||' where id = '|| recid ||" ;
             perform SQL ;
             select wkf_id into wkfid from wkf_trans where id = transid ;
             ...
             close t_cur ;
             if nitem = 1 then
               if myuid = flowowner or myuid = flowman1 or myuid = flowman2 or myuid = flowman3 or myuid = flowman4 then
                  myres = True ;
               else
                  myres = False ;
               end if ;  
            elsif nitem = 2 then
               if myuid = flowman1 or myuid = flowman2 or myuid = flowman3 or myuid = flowman4 then
                  myres = True ;
               else
                  myres = False ;
               end if ;  
            elsif nitem = 3 then
               if myuid = flowman2 or myuid = flowman3 or myuid = flowman4 then
                  myres = True ;
               else
                  myres = False ;
               end if ;  
            elsif nitem=4 then
               if myuid = flowman3 or myuid = flowman4 then
                  myres = True ;
               else
                  myres = False ;
               end if ;   
            elsif nitem=5 then
               if myuid = flowman4 then
                  myres = True ; 
               else   
                  myres = False ;
               end if ;  
