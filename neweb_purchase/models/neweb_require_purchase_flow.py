...
class newebrequirepurchaseflow(models.Model):
    ...
    def create(self, vals):
        res = super(newebrequirepurchaseflow, self).create(vals)
        self.env.cr.execute("""select genrfqflowman(%d)""" % res.id)
    ...
    def write(self, vals):
        if 'x_wkf_state' in vals:
           ...
           myflow = False
        res = super(newebrequirepurchaseflow, self).write(vals)
        for rec in self:
            self.env.cr.execute("""select genrfqflowman(%d)""" % rec.id)
            self.env.cr.execute("""commit""")
