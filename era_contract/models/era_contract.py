...
class EraNewContract(models.Model):
    _name = "era.contract"
    _description = "ERA合約管理"
    ...
    project_no = fields.Many2one('era.household_house',string="案名",required=True)
    member_pid = fields.Char(string="身分證號",required=True)
    contract_type = fields.Selection([('1','新約'),('2','續約')],string="合約類型",default='1')
    house_id = fields.Many2one('era.household_house_line', string="入住房間")
    house_id1 = fields.Many2one('era.household_house_line', string="入住房間(續約)")
    ...
    contract_emeter_line = fields.One2many('era.contract_emeter','contract_id',copy=False)
    contract_close_line = fields.One2many('era.contract_close','contract_id',copy=False)
    ...
    def onclientchangepid(self):
        myrec = self.env['era.household_member'].search([('member_pid','=',self.member_pid)])
        if myrec:
    ...
    def run_contract_action(self):     # 合約啟動生效
        ...
        if self.contract_type=='2':     # 續約
            ...
            payrec = self.env['era.member_payment']
            ...
            myrec = self.env['era.household_member']
            myres = myrec.create({'member_no':self.member_no,'member_name':self.member_name,'member_pid':self.member_pid,
                          ...
                          'member_desc':self.member_desc,'member_deposit':self.member_deposit,'active':True,'house_id':self.house_id.id,'contract_id':self.id})
            ...
            self.env.cr.execute("""commit""")
            self.env.cr.execute("""select activememberuser(%d)""" % self.id)
            self.env.cr.execute("""commit""")

        view = self.env.ref('sh_message.sh_message_wizard')
        view_id = view and view.id or False
        context = dict(self._context or {})
        context['message']='租戶合約啟動生效完成'
        return {
            'name':'Success',
            'type':'ir.actions.act_window',
            'view_type':'form',
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('era.contract') or _('New')
        res = super(EraNewContract, self).create(vals)
    ...
    def onchangeproj(self):
        if self.house_id:
            myrec = self.env['era.household_house_line'].search([('house_id', '=', self.project_no.id)])
            ids = []
            for item in myrec:
                ids.append(item.id)
            res = {}
    ...
    def write(self, vals):
        ...
        res = super(EraNewContract, self).write(vals)
        self.env.cr.execute("""select gencontractemeter(%d)""" % self.id)
        self.env.cr.execute("""commit""")
        return res
    ...
    def unlink(self):
        ...
        res = super(EraNewContract, self).unlink()
        return res
...
class ERAContractEmeter(models.Model):
    _name = "era.contract_emeter"
    ...
    def _get_used_scale(self):
        for rec in self:
            self.env.cr.execute("""select max(used_scale) from era_household_used_line where used_emeter_id=%d""" % rec.emeter_id.id)
    ...
    contract_id = fields.Many2one('era.contract',ondelete='cascade')
...
class ERAContractCloseLine(models.Model):
    _name = "era.contract_close"
