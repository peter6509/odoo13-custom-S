...
class ERALandlordContract(models.Model):
    _name = "era.landlord_contract"
    _description = "ERA房東合約管理"

    name = fields.Char(string="合約編號", default='New', copy=False)
    project_no = fields.Many2many('era.household_house', 'era_landlord_contract_house_rel','contract_id','house_id',string="簽約案場", required=True)
    line_user_ids = fields.One2many('era.landlord_line_user','contract_id',string="房東LINE User",copy=False)
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('era.landlord_contract') or _('New')
        res = super(ERALandlordContract, self).create(vals)
        return res
...
class ERALandlordLineUser(models.Model):
    _name = "era.landlord_line_user"
    _description = "房東LINE User"

    contract_id = fields.Many2one('era.landlord_contract',ondelete='cascade')
