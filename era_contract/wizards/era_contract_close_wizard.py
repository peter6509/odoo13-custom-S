...
class ERAContractCloseWizard(models.TransientModel):
    _name = "era.contract_close_wizard"
    _description = "ERA解約返還押金核銷精靈"
    ...
    member_id = fields.Many2one('era.household_member',string="租戶",required=True)
    ...
    def onchangecusname(self):
        myrec = self.env['era.household_member'].search([('id','=',self.member_id.id)])
        myids = []
        for rec in myrec:
            myids.append(rec[0])
    ...
    def run_contract_close(self):
        ...
        if self.member_return_amount > 0 :
            myrec = self.env['era.member_payment']
