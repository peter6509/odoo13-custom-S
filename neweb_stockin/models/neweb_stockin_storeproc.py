...
LANGUAGE plpgsql;""")
        ...
        self._cr.execute("""create or replace function gen_copy_stockout2(projno varchar,stockid int) returns void as $BODY$
            ...
            BEGIN
              ...
              update stock_picking set scheduled_date=current_timestamp,is_proj_main=FALSE where id = stockid ;
              if projid is not null then
                 nitem = 1 ;
                 open pj_cur for select * from neweb_projsaleitem where saleitem_id=projid and (coalesce(prod_num,0) > coalesce(prod_stockoutnum,0))  order by line_item;
                 loop
                   fetch pj_cur into pj_rec ;
                   exit when not found ;
                   ...
                   insert into neweb_stockship_list(stockship_id,stockship_machinetype,stockship_modeltype,stockship_spec,stockship_num,stockship_price,create_date,write_date,stockout_sequence_id,stockship_item,line_item) values
                    (stockid,machinename,coalesce(pj_rec.prod_modeltype,' '),coalesce(pj_rec.prod_desc,' '),unoutnum,coalesce(pj_rec.prod_price,0),mynowday,mynowday,pj_rec.id,nitem,nitem) ;
                   select name into myname from neweb_prodbrand where id=pj_rec.prod_brand;
                   if myname is null then
                       myname = ' ' ; 
                   end if ;
                   insert into neweb_stockout_list(stockout_id,stockout_machinetype,stockout_modeltype,stockout_spec,stockout_num,stockout_price,stockout_sequence_id,create_date,write_date,line_item) values
                    (stockid,myname,coalesce(pj_rec.prod_modeltype,' '),coalesce(pj_rec.prod_desc,' '),unoutnum,coalesce(pj_rec.prod_price,0),pj_rec.id,mynowday,mynowday,coalesce(pj_rec.line_item,0));
                   nitem = nitem + 1 ; 
                 end loop ;
                 close pj_cur ;
        ...
        self._cr.execute("""create or replace function genallstockline() returns void as $BODY$
            DECLARE
              ...
              myid int ;
            BEGIN
              open p_cur for select id,name from neweb_project where main_stockoutno is null ;
              loop
                ...
                if ncount=0 then
                   ...
                   if myid is not null then
                      ...
                      update neweb_project set main_stockoutno = myid where id = p_rec.id ;
                   end if ;    
        ...
        self._cr.execute("""drop function if exists gen_copy_stockin1(purno varchar,stockid int) cascade""")
        self._cr.execute("""create or replace function gen_copy_stockin1(purno varchar,stockid int) returns void as $BODY$
                DECLARE
                  purid int ;
                  innum float ;
                  pur_cur refcursor ;
                  pur_rec record ;
                BEGIN
                  update stock_picking set is_pur_main=FALSE where id=stockid ;
                  ...
                  if purid is not null then
                     ...
                     loop
                        ...
                        exit when not found ;
                        select sum(stockin_num) into innum from neweb_stockin_list where 
                            stockin_id in (select name from neweb_stockinno_line where in_id=purid) 
                            and stockin_sequence_id=pur_rec.id ;
                        update neweb_pitem_list set pitem_stockin_num=innum where id = pur_rec.id ;
                     end loop ;
                     close pur_cur ;
                  end if ;   
                END;$BODY$
        ...
        self._cr.execute("""drop function if exists gen_copy_stockin2(purno varchar,stockid int) cascade""")
        self._cr.execute("""create or replace function gen_copy_stockin2(purno varchar,stockid int) returns void as $BODY$
        DECLARE
          purid int ;
          innum float ;
          pur_cur refcursor ;
        ...
        BEGIN
          select id into purid from purchase_order where name=purno ;
          select current_timestamp into mynowday ;
          update stock_picking set scheduled_date=current_timestamp,is_pur_main=FALSE where id = stockid ;
          if purid is not null then
             nitem = 1 ;
             open pur_cur for select * from neweb_pitem_list where pitem_id=purid and (coalesce(pitem_num,0) > coalesce(pitem_stockin_num,0))  order by pitem_item;
             loop
               fetch pur_cur into pur_rec ;
               exit when not found ;
               uninnum = coalesce(pur_rec.pitem_num::numeric,0) - coalesce(pur_rec.pitem_stockin_num::numeric,0) ;
               if uninnum > 0 then
                  insert into neweb_stockin_list(stockin_id,stockin_machinetype,stockin_modeltype,stockin_prodno,stockin_spec,stockin_num,stockin_num1,stockin_sequence_id,stockin_item) values
                    (stockid,pur_rec.pitem_machine_type,pur_rec.pitem_model_type,pur_rec.pitem_prod_no,pur_rec.pitem_spec,uninnum,uninnum,pur_rec.id,pur_rec.pitem_item) ;
               end if ;
               nitem = nitem + 1 ;
             end loop ;
             close pur_cur ;
             select count(*) into ncount from neweb_stockinno_line where in_id=purid and name=stockid ;
             if ncount = 0 and nitem > 1 then
                insert into neweb_stockinno_line(in_id,name) values (purid,stockid) ; 
             end if ;
          end if ;   
        END;$BODY$
        LANGUAGE plpgsql;""")

        self._cr.execute("""drop function if exists genallstockinline() cascade""")
        self._cr.execute("""create or replace function genallstockinline() returns void as $BODY$
        DECLARE
          p_cur refcursor ;
          p_rec record ;
          ncount int ;
          ...
          myid int ;
        BEGIN
          open p_cur for select id,name from purchase_order where main_stockinno is null ;
          loop
            fetch p_cur into p_rec ;
            exit when not found ;
            select count(*) into ncount from neweb_stockinno_line where in_id=p_rec.id ;
            if ncount=0 then
               select min(id) into myid from stock_picking where 
               picking_type_id in (select id from stock_picking_type where sequence_code='IN') 
                   and origin=p_rec.name ;
               if myid is not null then
                  update stock_picking set is_pur_main=TRUE where id = myid ;
                  update purchase_order set main_stockinno = myid where id = p_rec.id ;
               end if ;    
               insert into neweb_stockinno_line(in_id,name) select p_rec.id,A.id 
                 from stock_picking A where  A.id=myid  ;
            end if;
          end loop ;
          close p_cur ;
        END;$BODY$
        ...
        self._cr.execute("""drop function if exists chk_stockin(stockid int) cascade""")
        self._cr.execute("""create or replace function chk_stockin(stockid int) returns INT as $BODY$
        DECLARE
          purno varchar ;
          purid int ;
          pur_cur refcursor ;
          ...
          myres INT ;
        BEGIN
          select origin into purno from stock_picking where id=stockid ;
          select id into purid from purchase_order where name = purno ;
           if purid is not null then
             nitem = 0 ;
             open pur_cur for select * from neweb_pitem_list where pitem_id=purid ;
             loop
                ...
                select sum(stockin_num) into innum from neweb_stockin_list where 
                    ...
                    and stockin_sequence_id=pur_rec.id ;
                update neweb_pitem_list set pitem_stockin_num=innum where id = pur_rec.id ;
                if coalesce(pur_rec.pitem_num,0) > coalesce(innum,0)  then
                   nitem = nitem + 1 ;
                end if ;
             end loop ;
             close pur_cur ;
