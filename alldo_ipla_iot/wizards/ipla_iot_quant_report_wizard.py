...
class iplaiotquantreportwizard(models.TransientModel):
    _name = "alldo_ipla_iot.quant_report_wizard"
    ...
    def run_quant_report(self):
        self.env.cr.execute("""delete from alldo_ipla_iot_quant_report""")
        self.env.cr.execute("""commit""")
        quant_rec = self.env['alldo_ipla_iot.quant_report']
        if self.report_type=='1':
           ...
           for rec in self.quant_ids:
            ...
            mold_rec = self.env['alldo_ipla_iot.quant_report']
            nitem = 1
            ncount = 1
        ...
        myviewid = self.env.ref('alldo_ipla_iot.alldo_ipla_stock_quant_report_action')
        myrec = self.env['alldo_ipla_iot.quant_report'].search([])
        myid = myrec[0].id
        return {'view_name': 'views_quant_report_action',
                'name': (u'quant report item Data'),
                'views': [[False,'form']],
                'res_model': 'alldo_ipla_iot.quant_report',
