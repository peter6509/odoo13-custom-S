...
class iotinitpicklewizard(models.TransientModel):
    _name = "alldo_ipla_iot.iot_init_pickle_wizard"
    # 目前取消


    iot_server = fields.Many2one('alldo_ipla_iot.server_info',string="IOT伺服器",required=True)
    equipment_ids = fields.Many2many('maintenance.equipment', 'ipla_iot_init_pickle_rel', 'wizard_id',
    ...
    def run_iot_init_pickle(self):
        ...
        myiotserverrec = self.env['alldo_ipla_iot.server_info'].search([('id','=',self.iot_server.id)])
