...
class alldoiplaiotmixsearchreplaceline(models.TransientModel):
    _name = "alldo_ipla_iot.mix_search_replaceline_wizard"
    ...
    def run_mixsearch_replaceline(self):
        ...
        myviewid = self.env.ref('alldo_ipla_iot.ipla_iot_replaceline_list_tree')

        return {
            'view_name': 'ipla_iot_replaceline_list_tree',
            'name': (u'工程師換線複合查詢統計'),
            'type': 'ir.actions.act_window',
            'res_model': 'alldo_ipla_iot.replaceline_list',
            'view_id': myviewid.id,
            'view_type': 'form',
            'view_mode': 'tree',
            'target': 'list'}
