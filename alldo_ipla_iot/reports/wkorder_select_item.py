...
class wkorderselectitem(models.Model):
    _name = "alldo_ipla_iot.wkorder_selectitem"

    report_owner = fields.Many2one('res.users',string="報表產出人")
    report_date = fields.Datetime(string="製作時間")
    report_line = fields.One2many('alldo_ipla_iot.wkorder_selectitem_line','item_id',string="明細")
    ...
    def print_wkorder(self):
        mycount = self.env['alldo_ipla_iot.wkorder_selectitem_line'].search_count([('item_id','=',self.id),('select_yn','=',True)])
        self.env.cr.execute("""select genselectitemreport()""")
        self.env.cr.execute("""commit""")
        if mycount == 0 :
            raise UserError("沒有選擇任何要列印的工單！")
        # self.filtered(lambda s: s.state == 'draft').write({'state': 'sent'})
        myrec = self.env['alldo_ipla_iot.wkorder_printsheet'].search([])
        return self.env.ref('alldo_ipla_iot.action_alldo_ipla_iot_wkorder_report').report_action(myrec)
...
class wkorderselectitemline(models.Model):
    _name = "alldo_ipla_iot.wkorder_selectitem_line"

    item_id = fields.Many2one('alldo_ipla_iot.wkorder_selectitem',ondelete='cascade')
...
class wkorderprintsheet(models.Model):
    _name = "alldo_ipla_iot.wkorder_printsheet"
