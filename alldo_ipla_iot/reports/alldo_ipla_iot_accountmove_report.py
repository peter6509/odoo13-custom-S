...
class alldoaccountmovereport(models.AbstractModel):
    _name ="report.alldo_ipla_iot.alldo_ipla_iot_accountmove_report"

    @api.model
    def _get_report_values(self, docids, data=None):
        # print(docids)
        # self.env.cr.execute("""delete from alldo_ipla_iot_accountmove_selectitem ;""")
        # self.env.cr.execute("""commit""")
        # for rec in docids:
        #     self.env.cr.execute("""select genaccountmovereport(%d,%d)""" % (self.env.uid, rec))
        #     self.env.cr.execute("""commit""")
        docs = self.env['alldo_ipla_iot.accountmove_selectitem'].search([])
        # print(docs)
        res_doc = []
        for line in docs:
            line_ids=[]
        ...
        docargs = {
            'doc_ids': docids,
            'doc_model': 'alldo_ipla_iot_accountmove_selectitem',
            'docs': res_doc,
        }
        return docargs