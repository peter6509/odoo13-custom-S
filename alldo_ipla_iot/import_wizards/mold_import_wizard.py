...
class moldwizard(models.TransientModel):
    _name = "alldo_ipla_iot.mold_wizard"
    ...
    def mold_import(self):
        ...
        myrec = self.env['alldo_ipla_iot.ipla_mold'].search([])
        for row in range(nstartrow -1, nendrow):
            ...
            if mycusno > 0 :
                self.env.cr.execute("""update alldo_ipla_iot_ipla_mold set partner_id=%d where id=%d""" % (mycusno,myid))
                self.env.cr.execute("""commit""")
            if mymoldsupp1 > 0 :
                self.env.cr.execute("""update alldo_ipla_iot_ipla_mold set mold_supplier_id=%d where id=%d""" % (mymoldsupp1, myid))
                self.env.cr.execute("""commit""")
