...
class iplaiotsonoseq(models.Model):
    _name = "alldo_ipla_iot.so_sequence"
    _description = "銷售單序號檔"
...
class iplaiotsonocreate(models.Model):
    _inherit = "sale.order"
    ...
    # def action_confirm(self):
    #     for rec in self:
    #         super(iplaiotsonocreate, rec)._action_confirm()
    #
    ...
    #     res = super(iplaiotsonocreate, self).create(vals)
    #     self.env.cr.execute("""select getsonoseq(%d)""" % res.id)
    #     self.env.cr.execute("""commit""")
    #     return res


class iplaiotsonolinecreate(models.Model):
