...
class alldoiotstoreproc(models.Model):
    ...
    def init(self):
        ...
        self.env.cr.execute("""create or replace function genworkorderqcline(workorderid int,qcdate date,iotnode int,prodnum float,iotowner int) returns void as $BODY$ 
           ...
           BEGIN
            select current_timestamp into mynowdatetime ;
            select product_no into prodid from alldo_ipla_iot_workorder where id = workorderid ;
             select count(*) into ncount from alldo_ipla_iot_workorder_qc where order_id=workorderid and qc_date::DATE=qcdate::DATE and iot_node=iotnode ;
             if ncount > 0 then
                update alldo_ipla_iot_workorder_qc set qc_good_num = coalesce(qc_good_num,0) + prodnum,iot_owner1=iotowner,write_date=mynowdatetime,
                product_no=prodid  where order_id=workorderid and qc_date::DATE=qcdate::DATE and iot_node=iotnode ;
             else  
                insert into alldo_ipla_iot_workorder_qc(order_id,qc_date,iot_node,qc_good_num,iot_owner1,create_date,product_no) values
        ...
        self.env.cr.execute("""create or replace function geniotdata(nodename varchar,workorderno varchar,empno varchar,prodnum float,iotserial varchar) returns void 
           ...
           BEGIN
              ...
              if ncount = 0 then
                  ...
                  select work_datetime,id into mylastworktime,mylastid from alldo_ipla_iot_workorder_lastwork where work_datetime::DATE = mynowdatetime::DATE
                    and equipment_id=mynodeid and workorder_id=myworkorderid ;
                  if mymoldwkman > 1 then
                      select id into myworkorderid from alldo_ipla_iot_workorder where name = workorderno ;
                      select max(id) into mymaxid from alldo_ipla_iot_workorder_lastwork where 
                        equipment_id=mynodeid and workorder_id=myworkorderid ;
                      select iot_owner,iot_owner1 into myempid1,myempid2 from alldo_ipla_iot_workorder_lastwork where 
                        id = mymaxid ;
                  end if ;      
                  ...
                  if mylastworktime is not null then  
                      ...
                      iotdurmin1 = 0 ;
                      if mymoldwkman > 1 then
                         insert into alldo_ipla_iot_workorder_lastwork (work_datetime,equipment_id,workorder_id,create_date,iot_owner,iot_owner1) values (mynowdatetime,mynodeid,myworkorderid,mynowdatetime,myempid1,myempid2) ;
                      else
                  ...
                  if myworkorderid is not null and mynodeid is not null and myempid is not null then  
                     if mymoldwkman > 1 then
                        insert into alldo_ipla_iot_workorder_iot_data(order_id,iot_date,iot_node,iot_owner,iot_owner1,iot_num,iot_duration,iot_serial,create_date) values
                           (myworkorderid,mynowdatetime,mynodeid,myempid1,myempid2,prodnum,iotdurmin1,iotserial,mynowdatetime) ;
                        insert into alldo_ipla_iot_equipment_iot_data(iot_id,iot_datetime,iot_owner,iot_owner1,iot_workorder,iot_num,iot_serial,create_date) values
                            (mynodeid,mynowdatetime,myempid1,myempid2,myworkorderid,prodnum,iotserial,mynowdatetime) ;
                     else
                        insert into alldo_ipla_iot_workorder_iot_data(order_id,iot_date,iot_node,iot_owner,iot_num,iot_duration,iot_serial,create_date) values
                     ...
                     update alldo_ipla_iot_workorder set prod_date=mynowdatetime,state='3' where id = myworkorderid ;
        ...
        self.env.cr.execute("""create or replace function genpowkorderngnum(wkorderid int) returns void as $BODY$
           ...
           BEGIN
             ...
             select sum(coalesce(qc_good_num,0) - coalesce(material_ng_num,0) - coalesce(processing_ng_num,0)) into myprodinnum from 
                 alldo_ipla_iot_workorder_qc where order_id=wkorderid ;
             update alldo_ipla_iot_workorder set prodin_num=myprodinnum where id = wkorderid ;
           END;$BODY$
           LANGUAGE plpgsql""")
        ...
        self.env.cr.execute("""create or replace function genselectitemreport() returns void as $BODY$
           ...
           BEGIN
             ...
             loop
               ...
               exit when not found ;
               select workorder_memo into workordermemo from alldo_ipla_iot_workorder where name=sel_rec.name ;
               select product_tmpl_id into myprodtmplid from product_product where id = sel_rec.product_no ;
               select default_code,name into myprodcode,myproductname from product_template where id = myprodtmplid ;
               myprodcode = concat('[',myprodcode,']',myproductname) ;
               insert into alldo_ipla_iot_wkorder_printsheet (name,product_no,cus_name,order_num,eng_type,shipping_date,
               workorder_memo) values (sel_rec.name,myprodcode,sel_rec.cus_name,sel_rec.order_num::NUMERIC,
               sel_rec.eng_type,sel_rec.shipping_date,workordermemo) ;
        ...
        self.env.cr.execute("""create or replace function genwkorderreport(myuid int,mywkid int) returns void as $BODY$
           ...
           BEGIN
             ...
             loop
               ...
               exit when not found ;
               select product_tmpl_id into myprodtmplid from product_product where id = wk_rec.product_no ;
               insert into alldo_ipla_iot_wkorder_selectitem_line(item_id,name,cus_name,product_no,eng_type,order_num,shipping_date,select_yn) values
        ...
        self.env.cr.execute("""create or replace function genpooutsuborderngnum(outsuborderid int) returns void as $BODY$
           ...
           BEGIN
             ...
             update alldo_ipla_iot_po_suborder_line set ng_num=myngnum,write_date=mynowdatetime where outsourcing_id=outsuborderid ;  
           END;$BODY$
           LANGUAGE plpgsql""")

        self.env.cr.execute(
            """drop function if exists genoutsourcingout(outid int,outownerid int,outnum int) cascade""")
        self.env.cr.execute("""drop function if exists genoutsourcingout(outid1 int ,outid int,outownerid int,outnum int) cascade""")
        self.env.cr.execute("""create or replace function genoutsourcingout(outid1 int,outid int,outownerid int,outnum int) returns void as $BODY$
           DECLARE
              mynowdatetime timestamp ;
              myprodid int ;
              cusid int ;
           ...
           BEGIN
              if outid1 = 0 then
                 cusname = 'ipla供料' ;
                  ...
                  select cus_name into cusid from  alldo_ipla_iot_outsuborder where id=outid1 ;
              ...
              select current_timestamp into mynowdatetime ;
              insert into alldo_ipla_iot_outsuborder_prodout(order_id,prodout_datetime,product_no,out_good_num,out_owner,create_date,out_loc)
                 values (outid,mynowdatetime,myprodid,outnum,outownerid,mynowdatetime,cusname) ;
              update alldo_ipla_iot_outsuborder set blank_num=coalesce(blank_num,0) + outnum,state='2',write_date=mynowdatetime where id = outid ;    
           END;$BODY$
           LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function genoutsourcingin(suborderid int,suborderid1 int,inownerid int,ingoodnum int,inngnum int) returns void as $BODY$
           DECLARE
             myprodid int ;
             mynowdatetime timestamp ;
             myengorder char ;
           ...
           BEGIN
             select current_timestamp into mynowdatetime ;
             select product_no,so_no,eng_order,eng_seq,name,mrp_prod_id into myprodid,sono,myengorder,myengseq,myname,mrpprodid from alldo_ipla_iot_outsuborder where id = suborderid ;
             if suborderid1=0 then
                cusname = '移轉回ipla入庫' ;
             else
                select cus_name into cusid from alldo_ipla_iot_outsuborder where id = suborderid1;
                select name into cusname from res_partner where id = cusid ;
             end if ;   
             select product_tmpl_id into myprodtmplid from product_product where id = myprodid ;
             insert into alldo_ipla_iot_outsuborder_prodin(order_id,prodin_datetime,product_no,in_good_num,in_ng_num,in_owner,create_date,in_loc) values 
                 (suborderid,mynowdatetime,myprodid,ingoodnum,inngnum,inownerid,mynowdatetime,cusname) ;
       ...
       """)
        ...
        self.env.cr.execute("""create or replace function getmomoldcavity(mono varchar) returns Integer as $BODY$
           ...
           BEGIN
             ...
             select product_tmpl_id into myprodtmplid from product_product where id = myprodid ;
             select mold_id into moldid from alldo_ipla_iot_product_mold where product_id = myprodtmplid and active = True ;
             if moldid is not null then
                select coalesce(mold_cavity,1) into myres from alldo_ipla_iot_ipla_mold where id = moldid ;
             else
                myres = 1 ;
             end if ;
             return myres ;
        ...
        self.env.cr.execute("""create or replace function getmomoldwkman(mono varchar) returns Integer as $BODY$
           ...
           BEGIN
             ...
             select mold_id into moldid from alldo_ipla_iot_product_mold where product_id = myprodtmplid and active = True ;
             if moldid is not null then
                select coalesce(work_man,1) into myres from alldo_ipla_iot_ipla_mold where id = moldid ;
             else
                myres = 1 ;
             end if ;
             return myres ;
        ...
        self.env.cr.execute("""create or replace function getnodelastmono(nodename varchar) returns varchar as $BODY$
          ...
          BEGIN
            ...
            select name,product_no into myres,prodid from alldo_ipla_iot_workorder where id = mylastmoid ;
            if myres is null then
                select max(id) into mymaxid from alldo_ipla_iot_workorder_lastwork where equipment_id = equipid and workorder_id is not null ;
            ...
            select coalesce(work_man,1) into workman from alldo_ipla_iot_ipla_mold where id = moldid ;
        ...
        self.env.cr.execute("""create or replace function runcncreplaceline(nodename varchar,empno varchar,wkorder varchar,reptype varchar) returns void as $BODY$
           ...
           BEGIN
             ...
             if typecode='P' then
                select count(*) into prep_count from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='P' and order_id=mywkorderid ;
                if prep_count > 0 then
                   select max(id) into myrepid from alldo_ipla_iot_wkorder_replaceline  where replace_end_datetime is null and replace_type='P' and order_id=mywkorderid ;
                   update alldo_ipla_iot_wkorder_replaceline set replace_end_datetime=mynowdatetime where id = myrepid  ;
                   /* execute recalreplinetime(myrepid) ; */
                end if ;
                insert into alldo_ipla_iot_wkorder_replaceline(order_id,replace_owner,replace_start_datetime,create_date,replace_type)
                     values (mywkorderid,myempid,mynowdatetime,mynowdatetime,typecode) ;
             elsif typecode='L' then
                 select count(*) into load_count from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='L' and equipment_id=mynodeid and order_id=mywkorderid;
                if load_count > 0 then
                   select max(id) into myrepid from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='L' and equipment_id=mynodeid and order_id=mywkorderid;
                   update alldo_ipla_iot_wkorder_replaceline set replace_end_datetime=mynowdatetime where id = myrepid ;
                   /* execute recalreplinetime(myrepid) ; */
                end if ;
                insert into alldo_ipla_iot_wkorder_replaceline(order_id,replace_owner,equipment_id,replace_start_datetime,create_date,replace_type)
                  values (mywkorderid,myempid,mynodeid,mynowdatetime,mynowdatetime,typecode) ;
             elsif typecode='B' then
                select count(*) into bake_count from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='B' and order_id=mywkorderid and order_id=mywkorderid ;
                if bake_count > 0 then
                   select max(id) into myrepid from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='B' and equipment_id=mynodeid and order_id=mywkorderid;
                   update alldo_ipla_iot_wkorder_replaceline set replace_end_datetime=mynowdatetime where id = myrepid ;
                   /* execute recalreplinetime(myrepid) ; */
                end if ;
                insert into alldo_ipla_iot_wkorder_replaceline(order_id,replace_owner,equipment_id,replace_start_datetime,create_date,replace_type)
                ...
                if mywkorderid is not null then
                   update alldo_ipla_iot_wkorder_replaceline set replace_end_datetime=mynowdatetime where replace_end_datetime is null and order_id=mywkorderid ;
                end if ;   
             end if ;
           ...
           BEGIN
             ...
             if nodename is not null or nodename != '' then
                select id,mo_no into myequipid,mywkorderid from maintenance_equipment where equipment_no=nodename ;
                 select id,replace_start_datetime into myrepid,myrepstartdatetime from alldo_ipla_iot_wkorder_replaceline where order_id=mywkorderid 
             ...
             if myrepid is not null then
                  select order_id into mywkorderid1 from alldo_ipla_iot_wkorder_replaceline where id = myrepid ; 
                  ...
                  if mywkorderid1 is null then 
                     ...
                     insert into alldo_ipla_iot_workorder_lastwork(work_datetime,equipment_id,workorder_id) values (mynowdatetime,myequipid,mywkorderid1) ;
                  end if ;   
             end if ;    
        ...
        self.env.cr.execute("""create or replace function genallreplinedur() returns void as $BODY$
          DECLARE
            ...
            rep_rec record ;
          BEGIN
            open rep_cur for select id from alldo_ipla_iot_wkorder_replaceline where (complete_tag is null or complete_tag = False)
               and replace_end_datetime is not null ;
            loop
               fetch rep_cur into rep_rec ;
               exit when not found ;
               execute recalreplinetime(rep_rec.id) ; 
               update alldo_ipla_iot_wkorder_replaceline set complete_tag=True where id = rep_rec.id  ;
            end loop ;
            close rep_cur ;
          END;$BODY$
          LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function recalreplinetime(repid int) returns void as $BODY$
          ...
          BEGIN
            select replace_start_datetime,replace_end_datetime into myrepstartdatetime,myrependdatetime from alldo_ipla_iot_wkorder_replaceline
                  where id = repid ;
            ...
            repdurmin1 = repdurmin + (repdurhour * 60) + (repdurday * 24 * 60) ;
            update alldo_ipla_iot_wkorder_replaceline set replace_duration=repdurmin1 where id = repid ;
          END;$BODY$
          LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function genoutpartner(partnerid1 int,partnerid int,prodid int,ownerid int,prodnum int,outdesc varchar,outmemo varchar,outretdate date) returns void as $BODY$
           ...
           BEGIN
             ...
             insert into alldo_ipla_iot_partner_prodout(partner_id,prodout_datetime,product_no,out_good_num,out_owner,out_loc,out_memo,out_return_date) values 
        ...
        self.env.cr.execute("""create or replace function geninpartner(partnerid int,partnerid1 int,prodid int,ownerid int,prodnum int,ngnum int,indesc varchar) returns void as $BODY$
           ...
           BEGIN
             ...
             insert into alldo_ipla_iot_partner_prodin(partner_id,prodin_datetime,product_no,in_good_num,in_ng_num,in_owner,in_loc) values 
        ...
        self.env.cr.execute("""create or replace function getcusmo(cusid int) returns setof INT as $BODY$
           DECLARE
             ...
             mo_rec record ;
           BEGIN
             open mo_cur for select id,cus_name,state,eng_order from alldo_ipla_iot_workorder where cus_name=cusid and 
        ...
        self.env.cr.execute("""create or replace function genblankstockin1(moid int,blanknum float,inowner int) returns void as $BODY$
           DECLARE
             ...
             mycusid int ;
           BEGIN
             select product_no,cus_name into prodid,mycusid from alldo_ipla_iot_workorder where id = moid ;
             ...
             update alldo_ipla_iot_workorder set blank_num = coalesce(blank_num,0) + blanknum,write_date=mynowdatetime where id = moid ;
             select blank_num into mynowblanknum from alldo_ipla_iot_workorder where id = moid ;
           END;$BODY$
           LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function setallmoclose(moid int) returns void as $BODY$
           ...
           BEGIN
             ...
             update alldo_ipla_iot_workorder set state='4' where mo_group_id=mogroupid ;
        ...
        self.env.cr.execute("""create or replace function runmoadd(moid int,prodnum numeric) returns void as $BODY$
          ...
          BEGIN
            ...
            update alldo_ipla_iot_workorder set order_num = coalesce(order_num,0)+prodnum where mrp_prod_id = moid ;
            update alldo_ipla_iot_outsuborder set order_num = coalesce(order_num,0) + prodnum where mrp_prod_id = moid ;
            ...
            select furnace_prod_id into mixprodid from alldo_ipla_iot_company_stockloc ;
            if bomid is not null then
               ...
               open pack_cur for select * from alldo_ipla_iot_materialline where mrp_prod_id = moid ;
               loop
                 ...
                 if pack_rec.product_no = blankprodid then
                    ...
                    update alldo_ipla_iot_materialline set product_qty=blankprodqty,onhand_qty=onhandqty,need_qty=needqty where id = pack_rec.id ;
                 else
                    msetqty = 0 ;
                    csetqty = 0 ;
                    select m_set_qty,c_set_qty into msetqty,csetqty from alldo_ipla_iot_packaging_line where bom_id=bomid and 
                    ...
                    if msetqty > 0 and csetqty > 0 and moqty > 0 then
                       ...
                       update alldo_ipla_iot_materialline set product_qty=blankprodqty,onhand_qty=onhandqty,need_qty=needqty
        ...
        self.env.cr.execute("""create or replace function candelwkorder(wkorderid int) returns Boolean as $BODY$
          ...
          BEGIN
            ...
            select count(*) into ncount from alldo_ipla_iot_workorder where mo_group_id= mygroupid and state in ('2','3','4') ;
            if ncount = 0 and mygroupid is not null then
               delete from alldo_ipla_iot_workorder where mo_group_id = mygroupid and state in ('1','5') ;
               myres = True ;
            else
               myres = False ;
            end if ;
        ...
        self.env.cr.execute("""create or replace function gennewshipping(partnerid int) returns void as $BODY$
          ...
          BEGIN
            ...
            select id,number_next into myid,numbernext from ir_sequence where code='alldo_ipla_iot.shipping' ;
            numbernext1 = numbernext::TEXT ;
            iplareportno = concat('S',myyear,mymonth,myday,lpad(numbernext1,3,'0')) ;
            update ir_sequence set number_next=number_next+1 where id = myid ;
            /*     */
            ...
            update alldo_ipla_iot_stockpicking_report set name=iplareportno where id = myreportid ;
               ...
               loop
                  ...
                  if ml_rec.qty_done > 0 then
                      insert into alldo_ipla_iot_stockpicking_report_line(rep_id,item,prod_no,prod_num,prod_uom,prod_price,line_memo) values 
                         (myreportid,nitem,ml_rec.product_id,ml_rec.qty_done,'PCS',0,' ') ;
                      select max(id) into myreplineid from alldo_ipla_iot_stockpicking_report_line where rep_id=myreportid ; 
                      /* update stock_picking set date_done=reportdate,report_no=iplareportno where id = pick_rec.id ; */
                      nitem = nitem + 1 ;
                  end if ;    
               end loop ;
               close ml_cur ;
               update stock_picking set report_no=iplareportno where  id = pick_rec.id ;
            end loop ;
            close pick_cur ;
        ...
        self.env.cr.execute("""create or replace function gennewngreturn(partnerid int) returns void as $BODY$
          ...
          BEGIN
            ...
            nitem = 1 ; 
            open ng_cur for select * from alldo_ipla_iot_workorder_qc where (material_ng_num > 0 or processing_ng_num > 0) 
                and (report_no is null or report_no='') and cus_name=partnerid ;
            loop
               fetch ng_cur into ng_rec ;
               exit when not found ;
               if nitem = 1 then
                  insert into alldo_ipla_iot_ngreturn_report_line(rep_id,item,prod_no,m_ng_num,p_ng_num,prod_uom,line_memo) values 
                 ...
                 select max(id) into myreplineid from alldo_ipla_iot_ngreturn_report_line where rep_id=myreportid ; 
                  ...
                  update alldo_ipla_iot_ngreturn_report_line set item1=nitem,prod_no1=ng_rec.product_no,m_ng_num1=ng_rec.material_ng_num,
                    p_ng_num1=ng_rec.processing_ng_num,prod_uom1='PCS',line_memo1=ng_rec.line_memo where id = myreplineid ;
               elsif nitem = 3 then
                  update alldo_ipla_iot_ngreturn_report_line set item2=nitem,prod_no2=ng_rec.product_no,m_ng_num2=ng_rec.material_ng_num,
                    p_ng_num2=ng_rec.processing_ng_num,prod_uom2='PCS',line_memo2=ng_rec.line_memo where id = myreplineid ;
               elsif nitem= 4 then
                  update alldo_ipla_iot_ngreturn_report_line set item3=nitem,prod_no3=ng_rec.product_no,m_ng_num3=ng_rec.material_ng_num,
                    p_ng_num3=ng_rec.processing_ng_num,prod_uom3='PCS',line_memo3=ng_rec.line_memo where id = myreplineid ;
               elsif nitem = 5 then
                  update alldo_ipla_iot_ngreturn_report_line set item4=nitem,prod_no4=ng_rec.product_no,m_ng_num4=ng_rec.material_ng_num,
               ...
               loop
                  ...
                  if ncount = 0 then
                     if att_rec.attendance_type='1' then
                        insert into alldo_ipla_iot_emp_attendance_list(emp_no,attend_date,attend_date1,attendance_start) values 
                        (att_rec.attendance_id,mydate,mydate1,mytime) ;
                     else
                       insert into alldo_ipla_iot_emp_attendance_list(emp_no,attend_date,attend_date1,attendance_end) values 
                        (att_rec.attendance_id,mydate,mydate1,mytime) ;
                     end if ;
                  else 
                     if att_rec.attendance_type='1' then
                        update alldo_ipla_iot_emp_attendance_list set attendance_start=mytime where emp_no=emp_rec.id and
                               attend_date1 = mydate1 and  attendance_start is null ;
                     else
                        update alldo_ipla_iot_emp_attendance_list set attendance_end=mytime where emp_no=emp_rec.id and
        ...
        self.env.cr.execute("""create or replace function genattendance2(empno int,attstart DATE,attend DATE) returns void as $BODY$
          ...
          BEGIN
            ...
            open att_cur for select * from alldo_ipla_iot_emp_attendance where attendance_id=empno and 
                 (attendance_date + interval '8 hours')::DATE >= attstart::DATE and 
                 (attendance_date + interval '8 hours')::DATE <= attend::DATE order by attendance_date ;
            loop
              fetch att_cur into att_rec ;
              ...
              select (att_rec.attendance_date + interval '8 hours')::DATE into mydate ;
              mydate1 = mydate::TEXT ;
              select count(*) into ncount from alldo_ipla_iot_emp_attendance_list where emp_no=empno and
                   attend_date1 = mydate1 ;
              select lpad(date_part('hour',att_rec.attendance_date + interval '8 hours')::TEXT,2,'0') into chour ;
              select lpad(date_part('minute',att_rec.attendance_date)::TEXT,2,'0') into cmin ;
              select lpad(date_part('second',att_rec.attendance_date)::TEXT,2,'0') into csecond ;
              ...
              if ncount = 0 then
                 ...
                 if att_rec.attendance_type='1' then
                    update alldo_ipla_iot_emp_attendance_list set attendance_start=mytime,att_start_date=att_rec.attendance_date where emp_no=empno and
                           attend_date1=mydate1 and  attendance_start is null ;
                 else
                    update alldo_ipla_iot_emp_attendance_list set attendance_end=mytime,att_end_date=att_rec.attendance_date where emp_no=empno and
                           attend_date1=mydate1 and  attendance_end is null;
                 end if ;
              end if ;     
           end loop ;
          ...
          BEGIN
            ...
            loop
                ...
                if out_rec.out_owner is null then
                   insert into alldo_ipla_iot_inout_prod_list(inout_date1,product_id,out_num) values
                      (mydate,out_rec.product_no,out_rec.out_good_num) ;
                else
                   insert into alldo_ipla_iot_inout_prod_list(inout_date1,product_id,out_num,out_owner) values
            ...
            loop
                ...
                if in_rec.in_owner is null then
                   insert into alldo_ipla_iot_inout_prod_list(inout_date1,product_id,in_good_num,in_ng_num) values
                       (mydate,in_rec.product_no,in_rec.in_good_num,in_rec.in_ng_num) ;
                else
                    insert into alldo_ipla_iot_inout_prod_list(inout_date1,product_id,in_good_num,in_ng_num,in_owner) values
        ...
        self.env.cr.execute("""create or replace view alldo_replaceline_view AS select A.order_id,A.replace_owner,A.equipment_id,A.replace_type,
           A.replace_start_datetime,A.replace_end_datetime,A.replace_duration,B.product_no,B.eng_type from alldo_ipla_iot_wkorder_replaceline A left join
           alldo_ipla_iot_workorder B on A.order_id = B.id ;""")
        ...
        self.env.cr.execute("""create or replace function genreplineperformance(startdate date,enddate date,prodid int,empid int,equipid int,reptype char) returns void as $BODY$
              ...
              BEGIN
                ...
                loop
                  fetch rep_cur into rep_rec ;
                  exit when not found ;
                    insert into alldo_ipla_iot_replaceline_list(equipment_id,order_id,product_no,eng_type,replace_start_datetime,replace_end_datetime,replace_duration,replace_owner,replace_type) values
        ...
        self.env.cr.execute("""create or replace function getmogpseq() returns INT as $BODY$
          ...
          BEGIN
            ...
            if ncount=0 then
               myres = 1 ;
               insert into alldo_ipla_iot_mo_group_seq(seqnum) values (2) ;
            else
               select coalesce(seqnum,1) into myres from alldo_ipla_iot_mo_group_seq  ;
               update alldo_ipla_iot_mo_group_seq set seqnum = coalesce(seqnum,1) + 1 ;
            end if ;
            return myres ;
          END;$BODY$
          LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function genfurnaceloc(locid int) returns void as $BODY$
          DECLARE
            ...
            myid int ;
          BEGIN
            select count(*) into ncount from  alldo_ipla_iot_casting_furnace ;
            if ncount = 0 then
               if locid > 0 then
                  insert into alldo_ipla_iot_casting_furnace(furnace_loc) values (locid) ;
               end if ;   
            else
               if locid > 0 then
                  select min(id) into myid from alldo_ipla_iot_casting_furnace ;
                  update alldo_ipla_iot_casting_furnace set furnace_loc=locid ;
                  delete from alldo_ipla_iot_casting_furnace where id != myid ;
               end if ;
            end if ;
          END;$BODY$
          LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function getfurnaceqty(bomid int) returns void as $BODY$
          ...
          BEGIN
            moldcavity = 0 ;
            select min(furnace_prod_id) into furnaceprodid from alldo_ipla_iot_company_stockloc ;
            select product_tmpl_id into prodtmplid from mrp_bom where id = bomid ;
            select min(mold_id) into moldid from alldo_ipla_iot_product_mold where product_id=prodtmplid and active=True ;
            select coalesce(mold_cavity,1) into moldcavity from alldo_ipla_iot_ipla_mold where id = moldid ;
        ...
        self.env.cr.execute("""create or replace function chfurnaceloc(mpid int) returns void as $BODY$
          DECLARE
            ...
            mympname varchar ;
          BEGIN
            select min(furnace_loc),min(furnace_prod_id) into furnacelocid,furnaceprodid from alldo_ipla_iot_company_stockloc ;
        ...
        self.env.cr.execute("""create or replace function setprodmold(prodtmplid int,moldid int) returns void as $BODY$
          ...
          BEGIN
            myengtype='1' ; 
            select count(*) into ncount from alldo_ipla_iot_eng_order where prod_id=prodtmplid  ;
            if ncount = 0 then
               insert into alldo_ipla_iot_eng_order(prod_id,eng_type,eng_desc) values (prodtmplid,myengtype,'鑄造') ;
            else
               select min(id) into myminid from alldo_ipla_iot_eng_order where prod_id=prodtmplid ; 
               select eng_type into myengtype from  alldo_ipla_iot_eng_order where id = myminid ; 
            end if ;
            select count(*) into ncount1 from alldo_ipla_iot_product_mold where product_id=prodtmplid  ;
            if ncount1 = 0 then
               insert into alldo_ipla_iot_product_mold(product_id,eng_type,mold_id,active) values (prodtmplid,myengtype,moldid,True) ;
            else
               select min(id) into myminid from alldo_ipla_iot_product_mold where product_id=prodtmplid ; 
               update alldo_ipla_iot_product_mold set mold_id=moldid where id = myminid ;
            end if ;
          END;$BODY$
          LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function mrpproductionupdate(wono varchar) returns void as $BODY$
          ...
          BEGIN
            ...
            update alldo_ipla_iot_workorder set so_pi=sopi where mrp_prod_id = moid ;
            update alldo_ipla_iot_outsuborder set so_pi=sopi where mrp_prod_id = moid ;
            select material_loc,prod_loc,blank_loc,mo_loc,furnace_loc,furnace_prod_id,uncomplete_loc into 
              materiallocid,prodlocid,blanklocid,molocid,furnacelocid,furnaceprodid,uncompletelocid from alldo_ipla_iot_company_stockloc ;
        ...
        self.env.cr.execute("""create or replace function autogencastingeng(prodtmplid int) returns void as $BODY$
          DECLARE
            ...
            moldid int ;
          BEGIN
            select count(*) into ncount from alldo_ipla_iot_eng_order where prod_id=prodtmplid and eng_type='1' ;
            if ncount = 0 then
               insert into alldo_ipla_iot_eng_order(prod_id,eng_type,eng_order,eng_desc) values (prodtmplid,'1',1,'鑄件工程') ;
            end if ;
            select count(*) into ncount1 from alldo_ipla_iot_product_mold where product_id=prodtmplid and eng_type='1' ;
            select coalesce(mold_id,0) into moldid from product_template where id = prodtmplid ;
            if ncount1 = 0 then
               if moldid > 0 then
                  insert into alldo_ipla_iot_product_mold(product_id,eng_type,mold_id,active) values (prodtmplid,'1',moldid,True) ;
               else
                  insert into alldo_ipla_iot_product_mold(product_id,eng_type,active) values (prodtmplid,'1',True) ;
               end if ;
            else
               if moldid > 0 then
                  update alldo_ipla_iot_product_mold set mold_id=moldid where product_id=prodtmplid and eng_type='1' ;
               end if ;   
            end if ;
          END;$BODY$
          LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function setmrpprodloc(prodid int,prodqty float) returns void as $BODY$
          DECLARE
            ...
            uncompletelocid int ;
          BEGIN
          select uncomplete_loc into uncompletelocid from alldo_ipla_iot_company_stockloc ;
        ...
        self.env.cr.execute("""create or replace function getmcategprod() returns setof INT as $BODY$
          DECLARE
            ...
            prod_rec record ;
          BEGIN
            select m_categ into mymcategid from alldo_ipla_iot_company_stockloc ;
        ...
        self.env.cr.execute("""create or replace function getmaterialonhand(prodid int) returns float as $BODY$
          DECLARE
             ...
             materialloc int ;
          BEGIN
             select min(material_loc) into materialloc from alldo_ipla_iot_company_stockloc ;
        ...
        self.env.cr.execute("""create or replace function gen_mrp_materialline(moid int) returns void as $BODY$
          DECLARE
            ...
            packagingqty int ;
          BEGIN
            delete from alldo_ipla_iot_materialline where mrp_prod_id = moid ;
            select product_id,product_qty into prodid,productqty from mrp_production where id = moid ;
            select product_tmpl_id into prodtmplid from product_product where id = prodid ;
            select product_blank1 into blanktmplid from product_template where id = prodtmplid ;
            select id into blankprodid from product_product where product_tmpl_id=blanktmplid ;
            select furnace_prod_id,material_loc into mixprodid,materialloc from alldo_ipla_iot_company_stockloc ;
            ...
            loop
               ...
               if m_rec.product_id = mixprodid then
                  ...
                  insert into alldo_ipla_iot_materialline(mrp_prod_id,product_no,product_qty,prod_uom_id,onhand_qty,need_qty) values
            ...
            open bom_cur for select * from alldo_ipla_iot_packaging_line where bom_id=bomid ;
            loop
               fetch bom_cur into bom_rec ;
               exit when not found ;
        ...
        self.env.cr.execute("""create or replace function genmoldtimes(workorderid int) returns void as $BODY$
          ...
          BEGIN
            select product_no into prodid from alldo_ipla_iot_workorder where id = workorderid ;
            select product_tmpl_id into prodtmplid from product_product where id = prodid ;
            select mold_id into moldid from alldo_ipla_iot_product_mold where product_id=prodtmplid ;
            update alldo_ipla_iot_ipla_mold set current_times=coalesce(current_times,0) + 1 where id = moldid ;
          END;$BODY$
          LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function mo_produce_in(productno varchar,lotno varchar,equipno varchar,scaleweight float,scaleowner varchar) returns Boolean as $BODY$
          ...
          BEGIN
            ...
            if lotno ='' or lotno = ' ' then
               ncount1 = 0 ;
                ...
                if ncount1 > 0 then
                   ...
                   select min(material_loc) into materiallocid from alldo_ipla_iot_company_stockloc ;
            ...
            if ncount > 0 and ncount1 > 0 and ncount2 > 0 and ncount3 > 0 then
               insert into alldo_ipla_iot_electronic_scale(product_no,need_lotno,lot_no,scale_weight,equipment_no,uom_id,scale_owner,scale_type,is_posting,scale_datetime) values
               ...
               if ncount4 > 0 then  
                   insert into alldo_ipla_iot_electronic_scale(product_no,need_lotno,scale_weight,equipment_no,uom_id,scale_owner,scale_type,is_posting,scale_datetime) values
        ...
        self.env.cr.execute("""create or replace function mo_stock_in(productno varchar,scaleweight float,scaleowner varchar) returns Boolean as $BODY$
         ...
         BEGIN
            ...
            if ncount > 0 and ncount1 > 0 then
               insert into alldo_ipla_iot_electronic_scale(product_no,scale_weight,uom_id,scale_owner,scale_type,is_posting,scale_datetime) values
                 (prodid,scaleweight,uomid,userid,'2','1',mynowdatetime) ;
               myres = True ;  
            elsif ncount > 0 and ncount1 = 0 then
               insert into alldo_ipla_iot_electronic_scale(product_no,scale_weight,uom_id,scale_type,is_posting,scale_datetime) values
        ...
        self.env.cr.execute("""create or replace function updatemoldline(nitem int,moldcode varchar) returns void as $BODY$
           DECLARE
             ...
             ncount int ;
           BEGIN
             select max(id) into myid from alldo_ipla_iot_mold_report ;
             if nitem=2 then
                update alldo_ipla_iot_mold_report_line set mold_code2=moldcode where mold_id=myid ;
             elsif nitem=3 then
                update alldo_ipla_iot_mold_report_line set mold_code3=moldcode where mold_id=myid ;
             elsif nitem=4 then
                update alldo_ipla_iot_mold_report_line set mold_code4=moldcode where mold_id=myid ;
             elsif nitem=5 then
                update alldo_ipla_iot_mold_report_line set mold_code5=moldcode where mold_id=myid ;
             elsif nitem=6 then
                update alldo_ipla_iot_mold_report_line set mold_code6=moldcode where mold_id=myid ;
             elsif nitem=7 then
                update alldo_ipla_iot_mold_report_line set mold_code7=moldcode where mold_id=myid ;
             elsif nitem=8 then
                update alldo_ipla_iot_mold_report_line set mold_code8=moldcode where mold_id=myid ;
             elsif nitem=9 then
                update alldo_ipla_iot_mold_report_line set mold_code9=moldcode where mold_id=myid ;
             elsif nitem=10 then
                update alldo_ipla_iot_mold_report_line set mold_code10=moldcode where mold_id=myid ;
             elsif nitem=11 then
                update alldo_ipla_iot_mold_report_line set mold_code11=moldcode where mold_id=myid ;
             elsif nitem=12 then
                update alldo_ipla_iot_mold_report_line set mold_code12=moldcode where mold_id=myid ;
             elsif nitem=13 then
                update alldo_ipla_iot_mold_report_line set mold_code13=moldcode where mold_id=myid ;
             elsif nitem=14 then
                update alldo_ipla_iot_mold_report_line set mold_code14=moldcode where mold_id=myid ;
             elsif nitem=15 then
                update alldo_ipla_iot_mold_report_line set mold_code15=moldcode where mold_id=myid ;
             elsif nitem=16 then
                update alldo_ipla_iot_mold_report_line set mold_code16=moldcode where mold_id=myid ;
             elsif nitem=17 then
                update alldo_ipla_iot_mold_report_line set mold_code17=moldcode where mold_id=myid ;
             elsif nitem=18 then
                update alldo_ipla_iot_mold_report_line set mold_code18=moldcode where mold_id=myid ;
             elsif nitem=19 then
                update alldo_ipla_iot_mold_report_line set mold_code19=moldcode where mold_id=myid ;
             elsif nitem=20 then
                update alldo_ipla_iot_mold_report_line set mold_code20=moldcode where mold_id=myid ;
             end if ;
           END;$BODY$
           LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function addmoldline(moldcode varchar) returns void as $BODY$
           ...
           BEGIN
             select max(id) into myid from alldo_ipla_iot_mold_report ;
             insert into alldo_ipla_iot_mold_report_line(mold_id,mold_code1) values (myid,moldcode) ;
           END;$BODY$
           LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function updatequantline(nitem int,lotcode varchar) returns void as $BODY$
           DECLARE
             ...
             ncount int ;
           BEGIN
             select max(id) into myid from alldo_ipla_iot_quant_report ;
             if nitem=2 then
                update alldo_ipla_iot_quant_report_line set lot_code2=lotcode where quant_id=myid ;
             elsif nitem=3 then
                update alldo_ipla_iot_quant_report_line set lot_code3=lotcode where quant_id=myid ;
             elsif nitem=4 then
                update alldo_ipla_iot_quant_report_line set lot_code4=lotcode where quant_id=myid ;
             elsif nitem=5 then
                update alldo_ipla_iot_quant_report_line set lot_code5=lotcode where quant_id=myid ;
             elsif nitem=6 then
                update alldo_ipla_iot_quant_report_line set lot_code6=lotcode where quant_id=myid ;
             elsif nitem=7 then
                update alldo_ipla_iot_quant_report_line set lot_code7=lotcode where quant_id=myid ;
             elsif nitem=8 then
                update alldo_ipla_iot_quant_report_line set lot_code8=lotcode where quant_id=myid ;
             elsif nitem=9 then
                update alldo_ipla_iot_quant_report_line set lot_code9=lotcode where quant_id=myid ;
             elsif nitem=10 then
                update alldo_ipla_iot_quant_report_line set lot_code10=lotcode where quant_id=myid ;
             elsif nitem=11 then
                update alldo_ipla_iot_quant_report_line set lot_code11=lotcode where quant_id=myid ;
             elsif nitem=12 then
                update alldo_ipla_iot_quant_report_line set lot_code12=lotcode where quant_id=myid ;
             elsif nitem=13 then
                update alldo_ipla_iot_quant_report_line set lot_code13=lotcode where quant_id=myid ;
             elsif nitem=14 then
                update alldo_ipla_iot_quant_report_line set lot_code14=lotcode where quant_id=myid ;
             elsif nitem=15 then
                update alldo_ipla_iot_quant_report_line set lot_code15=lotcode where quant_id=myid ;
             elsif nitem=16 then
                update alldo_ipla_iot_quant_report_line set lot_code16=lotcode where quant_id=myid ;
             elsif nitem=17 then
                update alldo_ipla_iot_quant_report_line set lot_code17=lotcode where quant_id=myid ;
             elsif nitem=18 then
                update alldo_ipla_iot_quant_report_line set lot_code18=lotcode where quant_id=myid ;
             elsif nitem=19 then
                update alldo_ipla_iot_quant_report_line set lot_code19=lotcode where quant_id=myid ;
             elsif nitem=20 then
                update alldo_ipla_iot_quant_report_line set lot_code20=lotcode where quant_id=myid ;
             end if ;
           END;$BODY$
           LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function addquantline(lotcode varchar) returns void as $BODY$
           ...
           BEGIN
             select max(id) into myid from alldo_ipla_iot_quant_report ;
             insert into alldo_ipla_iot_quant_report_line(quant_id,lot_code1) values (myid,lotcode) ;
           END;$BODY$
           LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function genpackagingbom(mypackagingno varchar,myprodno varchar,prodset int,childset int) returns void as $BODY$
          ...
          BEGIN
            ...
            if ncount > 0 and ncount1 > 0 then
               select product_tmpl_id into prodtmplid from product_product where id = prodid ;
               select id into bomid from mrp_bom where product_tmpl_id = prodtmplid ;
               select count(*) into ncount2 from alldo_ipla_iot_packaging_line where bom_id=bomid and 
                   product_id = packagingid ;
               if ncount2 = 0 then
                  insert into alldo_ipla_iot_packaging_line(bom_id,product_id,product_uom_id,m_set_qty,c_set_qty) 
                      values (bomid,packagingid,1,prodset,childset) ;
               else
                  update alldo_ipla_iot_packaging_line set m_set_qty=prodset,c_set_qty=childset where bom_id=bomid and 
                     product_id = packagingid ;
               end if ;    
            end if ;   
          END;$BODY$
        ...
        self.env.cr.execute("""create or replace function getsupplierprod(partnerid int) returns setof INT as $BODY$
          DECLARE
            ...
            out_rec record ;
          BEGIN
            open out_cur for select distinct product_no from alldo_ipla_iot_partner_prodout where partner_id=partnerid ;
            loop
              fetch out_cur into out_rec ;
              exit when not found ;
        ...
        self.env.cr.execute("""CREATE OR REPLACE VIEW alldo_ipla_iot_ngreturn_view AS
            ...
            from alldo_ipla_iot_outsuborder_prodin QC left join alldo_ipla_iot_outsuborder SUB
        ...
        self.env.cr.execute("""create or replace function genngreturnexcel(partnerid int,prodid int,startdate date,enddate date) returns void as $BODY$
         ...
         BEGIN
           ...
           if prodid = 0 then
              open ng_cur for select * from alldo_ipla_iot_ngreturn_view where prodin_datetime::DATE >= startdate and prodin_datetime::DATE <= enddate and cus_name=partnerid   ;
           else
             open ng_cur for select * from alldo_ipla_iot_ngreturn_view where prodin_datetime::DATE >= startdate and prodin_datetime::DATE <= enddate and cus_name=partnerid and  
              (product_no=prodid) ;
           end if ;
           loop
             fetch ng_cur into ng_rec ;
             ...
             mytype = '2' ;   /* 托工NG */
             insert into alldo_ipla_iot_ngreturn_list(name,partner_id,ngreturn_date,ngreturn_date1,product_id,processing_ng_num,ngreturn_type,return_good_num) values
        ...
        self.env.cr.execute("""create or replace function gen_accountmove(partnerid int,startdate date,enddate date) returns void as $BODY$
         ...
         BEGIN
           ...
           /*  delete from alldo_ipla_iot_accountmove_selectitem_line ; */
           delete from alldo_ipla_iot_accountmove_selectitem ;
           sedate = concat(startdate::TEXT,' ~ ',enddate::TEXT) ;
           insert into alldo_ipla_iot_accountmove_selectitem(report_date,partner_id,report_date1,startenddate) values (mynowdate,partnerid,mynowdate1,sedate) ;
           select max(id) into myreportid from alldo_ipla_iot_accountmove_selectitem ;
           ...
           loop
             ...
             loop
                ...
                insert into alldo_ipla_iot_accountmove_selectitem_line(move_id,account_date,ref,sales_no,prod_no,prod_desc,uom_id,prod_num,prod_price,tax_type,amount_untax_num,amount_tax_num,account_move_line_id) values
        ...
        self.env.cr.execute("""create or replace function gen_accountmove1(partnerid int,reportno varchar) returns void as $BODY$
         ...
         BEGIN
           ...
           select mynowdate::TEXT into mynowdate1 ;
           delete from alldo_ipla_iot_accountmove_selectitem ;
           insert into alldo_ipla_iot_accountmove_selectitem(report_date,partner_id,report_date1) values (mynowdate,partnerid,mynowdate1) ;
           select max(id) into myreportid from alldo_ipla_iot_accountmove_selectitem ;
           untaxamounttot = 0 ;
           taxtot = 0 ;
           taxamounttot = 0 ;
           ...
           loop
             ...
             loop
                ...
                insert into alldo_ipla_iot_accountmove_selectitem_line(move_id,account_date,ref,sales_no,prod_no,prod_desc,uom_id,prod_num,prod_price,tax_type,amount_untax_num,amount_tax_num,account_move_line_id,selectyn) values
           ...
           update alldo_ipla_iot_accountmove_selectitem set amount_untax_total=untaxamounttot,amount_tax_total=taxamounttot,
        ...
        self.env.cr.execute("""create or replace function getaccountseq(mytype varchar) returns varchar as $BODY$
         ...
         BEGIN
           ...
           select lpad(date_part('month',mynowdate)::TEXT,2,'0') into mymonth ;
           select lpad(date_part('day',mynowdate)::TEXT,2,'0') into myday ;
           select count(*) into ncount from alldo_ipla_iot_account_seq where seq_type=mytype and seq_date=mynowdate ;
           if ncount = 0 then
              insert into alldo_ipla_iot_account_seq(seq_type,seq_date,seq_num) values (mytype,mynowdate,2) ;
              myres = concat(mytype,myyear,mymonth,myday,'01') ;
           else
              select id,seq_num into myid,mynum from alldo_ipla_iot_account_seq where seq_type=mytype and seq_date=mynowdate ;
              cnum = lpad(mynum::TEXT,2,'0') ;
              myres = concat(mytype,myyear,mymonth,myday,cnum) ;
              update alldo_ipla_iot_account_seq set seq_num=seq_num + 1 where id = myid ;
           end if ;
           return myres ;
         END;$BODY$
         LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function genaccountarno(reportno varchar) returns void as $BODY$
         DECLARE
           ...
           accmoveid int ;
         BEGIN
           /* select max(name) into myarno from alldo_ipla_iot_accountmove_selectitem ; */
           open ar_cur for select * from alldo_ipla_iot_accountmove_selectitem_line where selectyn=True ;
           loop
        ...
        self.env.cr.execute("""create or replace function updatetaiwanreceipt(itemid int) returns void as $BODY$
         DECLARE
           ...
           moveid int ;
         BEGIN
           select taiwan_receipt into taiwanreceipt from alldo_ipla_iot_accountmove_selectitem where id = itemid ;
           open sel_cur for select * from alldo_ipla_iot_accountmove_selectitem_line where move_id=itemid and selectyn = True ;
           loop
             fetch sel_cur into sel_rec ;
             exit when not found ;
        ...
        self.env.cr.execute("""create or replace function getmaterialno() returns setof INT as $BODY$
         DECLARE
           ...
           prodid int ;
         BEGIN
           select max(m_categ),max(r_categ) into mcateg,rcateg from alldo_ipla_iot_company_stockloc ;
        ...
        self.env.cr.execute("""create or replace function genscalemovemixsearch(startdate date,enddate date,prodid int,scaletype char) returns void as $BODY$
         DECLARE
           ...
           ncount int ;
         BEGIN
            delete from alldo_ipla_iot_mixsearch_scalemovelist ;
           if prodid = 0 and scaletype='0' then
              open sca_cur for select * from alldo_ipla_iot_electronic_scale where scale_datetime::DATE between startdate::DATE and enddate::DATE and is_posting='2';
           elsif prodid = 0 and scaletype='1' then
              open sca_cur for select * from alldo_ipla_iot_electronic_scale where scale_datetime::DATE between startdate::DATE and enddate::DATE 
              ...
              open sca_cur for select * from alldo_ipla_iot_electronic_scale where scale_datetime::DATE between startdate::DATE and enddate::DATE and product_no=prodid 
                   and is_posting='2' ;
           elsif prodid > 0 and scaletype='1' then
              open sca_cur for select * from alldo_ipla_iot_electronic_scale where scale_datetime::DATE between startdate::DATE and enddate::DATE and product_no=prodid 
                   and is_posting='2' and scale_type='1' ;
           ...
           loop
             ...
             exit when not found ;
             insert into alldo_ipla_iot_mixsearch_scalemovelist(scale_type,product_no,need_lotno,lot_no,scale_weight,equipment_no,uom_id,scale_owner,picking_no,scale_datetime) values
        ...
        self.env.cr.execute("""create or replace function resetproductionnum(moid int) returns void as $BODY$
         DECLARE
           ...
           furnaceprodid int ;
         BEGIN
           select max(furnace_prod_id) into furnaceprodid from alldo_ipla_iot_company_stockloc ;
           ...
           loop
             ...
             update alldo_ipla_iot_materialline set product_qty=rawmaterialnum where mrp_prod_id=moid and product_no=boml_rec.product_id  ;
           end loop ;
           close boml_cur ;
         END;$BODY$
         LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function updatetodaynum() returns void as $BODY$
         ...
         BEGIN
           ...
           loop
              fetch equ_cur into equ_rec ;
              exit when not found ;
              select sum(coalesce(iot_num,0))::INT into todaynum from alldo_ipla_iot_equipment_iot_data where iot_id = 
        ...
        self.env.cr.execute("""create or replace function getmopreprodnum(moid int) returns INT as $BODY$
         DECLARE
           ...
           myres int ;
         BEGIN
           update alldo_ipla_iot_workorder_iot_data set is_select=False where is_select=True and order_id = moid ;
           select count(*) into ncount from alldo_ipla_iot_workorder_iot_data where (is_posting is null or is_posting=False) 
              and order_id = moid ;
           if ncount > 0 then
              update alldo_ipla_iot_workorder_iot_data set is_select=True where (is_posting is null or is_posting=False) and order_id = moid ;
              select sum(coalesce(iot_num,0)) into myres from alldo_ipla_iot_workorder_iot_data where (is_posting is null or is_posting=False) and 
        ...
        self.env.cr.execute("""create or replace function completepreprodnum(moid int) returns void as $BODY$
         DECLARE
           ncount int ;
         BEGIN
           select count(*) into ncount from alldo_ipla_iot_workorder_iot_data where is_select=True and order_id=moid ;
           if ncount > 0 then
              update alldo_ipla_iot_workorder_iot_data set is_posting=True where order_id=moid and is_select=True ;
           end if ;
         END;$BODY$
         LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function genwkorderqcrecord(moid int,ngnum float) returns void as $BODY$
          ...
          BEGIN
             ...
             select max(id) into myqclineid from alldo_ipla_iot_workorder_qc where order_id=moid and qc_date::DATE = qcdate::DATE ;
             if myqclineid is null then
                select max(id) into myqclineid from alldo_ipla_iot_workorder_qc where order_id=moid ;
             end if ;
             select coalesce(processing_ng_num,0),coalesce(qc_good_num,0) into qcngnum,qcgoodnum from alldo_ipla_iot_workorder_qc where id = myqclineid ;
             mynowngnum = qcngnum + ngnum ;
             update alldo_ipla_iot_workorder_qc set processing_ng_num=mynowngnum where id = myqclineid ;
          END;$BODY$
          LANGUAGE plpgsql ;""")
        ...
        self.env.cr.execute("""create or replace function updatesuborderframe(subid int,outframe1 int,outframe2 int,outpallet int) returns void as $BODY$
          DECLARE
            ncount int ;
          BEGIN
            update alldo_ipla_iot_outsuborder set out_plastic_frame1 = coalesce(out_plastic_frame1,0) + coalesce(outframe1,0),out_plastic_frame2 = coalesce(out_plastic_frame2,0) + coalesce(outframe2,0),
        ...
        self.env.cr.execute("""create or replace function updatesuborderframe1(subid int,inframe1 int,inframe2 int,inpallet int) returns void as $BODY$
          DECLARE
            ncount int ;
          BEGIN
            update alldo_ipla_iot_outsuborder set in_plastic_frame1 = coalesce(in_plastic_frame1,0) + coalesce(inframe1,0),in_plastic_frame2 = coalesce(in_plastic_frame2,0) + coalesce(inframe2,0),
        ...
        self.env.cr.execute("""create or replace function updatesuborderframe2(subid1 int,subid2 int,tframe1 int,tframe2 int,tpallet int) returns void as $BODY$
          DECLARE
            ncount int ;
          BEGIN
            update alldo_ipla_iot_outsuborder set in_plastic_frame1 = coalesce(in_plastic_frame1,0) + coalesce(tframe1,0),in_plastic_frame2 = coalesce(in_plastic_frame2,0) + coalesce(tframe2,0),
                 in_pallet = coalesce(in_pallet,0) + coalesce(tpallet,0) where id = subid1 ;
            update alldo_ipla_iot_outsuborder set out_plastic_frame1 = coalesce(out_plastic_frame1,0) + coalesce(tframe1,0),out_plastic_frame2 = coalesce(out_plastic_frame2,0) + coalesce(tframe2,0),
        ...
        self.env.cr.execute("""create or replace function genqcngnum(ngtype char,noid int,ngnum float) returns void as $BODY$
         DECLARE
           ncount int ;
           currentdatetime timestamp ;
         BEGIN
           select current_timestamp into currentdatetime ;
           if ngtype='2' then
              select count(*) into ncount from alldo_ipla_iot_outsuborder where id = noid ;
              if ncount > 0 then
                 insert into alldo_ipla_iot_outsuborder_prodin(order_id,prodin_datetime,in_good_num,in_ng_num,in_loc) values
                   (noid,currentdatetime::DATE,0,ngnum,'QC NG 追加') ;
              end if ;
           else
              select count(*) into ncount from alldo_ipla_iot_workorder where id = noid ;
              if ncount > 0 then
                 insert into alldo_ipla_iot_workorder_qc(order_id,qc_date,qc_good_num,processing_ng_num,line_memo) values
                   (noid,currentdatetime::DATE,0,ngnum,'QC NG 追加') ;
              end if ;
           end if ;
         END;$BODY$
        ...
        self.env.cr.execute("""create or replace function updatemowkman(nodename varchar,wkorder varchar,empno1 varchar,empno2 varchar) returns void as $BODY$
         ...
         BEGIN
           ...
           select id into wkorderid from alldo_ipla_iot_workorder where name=wkorder ;
           select count(*) into ncount from alldo_ipla_iot_workorder_lastwork where equipment_id=nodeid and workorder_id=wkorderid ;
           select id into empid1 from hr_employee where emp_code=empno1 ;
           select id into empid2 from hr_employee where emp_code=empno2 ;
           if ncount > 0 then
              update alldo_ipla_iot_workorder_lastwork set work_datetime=wkdatetime,iot_owner=empid1,iot_owner1=empid2 where equipment_id=nodeid and workorder_id=wkorderid ;
           else
              insert into alldo_ipla_workorder_lastwork(work_datetime,equipment_id,workorder_id,iot_owner,iot_owner1) values
        ...
        self.env.cr.execute("""create or replace function getsuborderprod(prodid int,engtype varchar) returns varchar as $BODY$
         ...
         BEGIN
           ...
           select eng_desc into myengdesc from alldo_ipla_iot_eng_order where prod_id = myprodtmplid and eng_type=engtype ;
        ...
        self.env.cr.execute("""create or replace function genlastoutsuborder(subid int) returns void as $BODY$
         DECLARE
           ...
           lastnum int ;
         BEGIN
           update alldo_ipla_iot_outsuborder_prodout set last_record = False where order_id = subid ;
           select max(prodout_datetime) into outdatetime from alldo_ipla_iot_outsuborder_prodout where order_id = subid ;
           lastdate = outdatetime::DATE ;
           update alldo_ipla_iot_outsuborder_prodout set last_record = True where order_id = subid and prodout_datetime::DATE = lastdate::DATE ;
           select sum(coalesce(out_good_num,0))::INT into lastnum from alldo_ipla_iot_outsuborder_prodout where last_record = True ;
           update alldo_ipla_iot_outsuborder set last_blank_num=lastnum where id = subid ;
         END;$BODY$
         LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function iotresettime() returns void as $BODY$
          ...
          BEGIN
            ...
            select current_timestamp::DATE into mynowdate ;
            select restart_time,restart_freq,next_run_restart into mycdate1,restartfreq,mynextrestarttime from alldo_ipla_iot_restartiot_setting;
            ...
            update alldo_ipla_iot_restartiot_setting set next_run_restart = mynextrestarttime  ;
          END;$BODY$
          LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function geniotrestart() returns varchar as $BODY$
          ...
          BEGIN
            ...
            select count(*) into ncount from alldo_ipla_iot_restartiot_setting where next_run_restart::timestamp < mycurrenttime1::timestamp ;
            if ncount > 0 then
              select next_run_restart,restart_freq,restart_time into mynextrestarttime,myrestartfreq,mycdate1 from alldo_ipla_iot_restartiot_setting ;
             /* if myrestartfreq is not null then
                 select (mynowdate::DATE + myrestartfreq) into mynextdate ;
              else
                 mynextdate = mynowdate + interval '1 days' ;
              end if ;   
              mycdate = concat(mynextdate::TEXT,' ',mycdate1) ;
        ...
        self.env.cr.execute("""create or replace function gencastprepstart(mono varchar,empno varchar) returns varchar as $BODY$
          DECLARE
            ...
            mydate varchar ;
             myres varchar ;
          BEGIN
             select id into mywkid from alldo_ipla_iot_workorder where name=mono ;
             select id into myempid from hr_employee where emp_code=empno ;
             select (now()::timestamp)::DATE  into mynowdate ;
             select current_timestamp into mynowdatetime ;
             select current_timestamp + interval '8 hours' into mynowdatetime1 ;
             ...
             select count(*) into ncount from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='P' and order_id=mywkid ;
             if ncount > 0 then
                select max(id) into myrepid from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='P' and order_id=mywkid ;
                update alldo_ipla_iot_wkorder_replaceline set replace_end_datetime=mynowdatetime where id = myrepid ;
                 execute recalreplinetime(myrepid) ;
             end if ;
             insert into alldo_ipla_iot_wkorder_replaceline(order_id,replace_owner,replace_start_datetime,create_date,replace_type)
        ...
        self.env.cr.execute("""create or replace function gencastbakestart(nodeno varchar,empno varchar) returns varchar as $BODY$
          ...
          BEGIN
             ...
             select substring((current_time + interval '8 hours')::TEXT,1,8) into mytime ;
             myworktime = concat(mydate,' ',mytime) ;
             select count(*) into ncount from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='B' and order_id=mywkid ;
             if ncount > 0 then
                update alldo_ipla_iot_wkorder_replaceline set replace_end_datetime=mynowdatetime where replace_end_datetime is null and replace_type='B' and order_id=mywkid  ;
             end if ;
             insert into alldo_ipla_iot_wkorder_replaceline(order_id,replace_owner,replace_start_datetime,create_date,replace_type)
        ...
        self.env.cr.execute("""create or replace function gencastbakeend(nodeno varchar,empno varchar) returns varchar as $BODY$
          ...
          BEGIN
             ...
             select count(*) into ncount from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='B' and order_id=mywkid ;
              if ncount > 0 then
                select max(id) into myrepid from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='B' and order_id=mywkid ;
                update alldo_ipla_iot_wkorder_replaceline set replace_end_datetime=mynowdatetime where id = myrepid ;
                myres = myworktime ;
                execute recalreplinetime(myrepid) ; 
        ...
        self.env.cr.execute("""create or replace function gencastloadstart(nodeno varchar,mono varchar,empno varchar) returns varchar as $BODY$
          ...
          BEGIN
             ...
             select substring((current_time + interval '8 hours')::TEXT,1,8) into mytime ;
             myworktime = concat(mydate,' ',mytime) ;
             select count(*) into ncount from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='L' and order_id=mywkid ;
             if ncount > 0 then
                select max(id) into myrepid from alldo_ipla_iot_wkorder_replaceline where replace_end_datetime is null and replace_type='L' and order_id=mywkid ;
                 execute recalreplinetime(myrepid) ; 
                update alldo_ipla_iot_wkorder_replaceline set replace_end_datetime=mynowdatetime where id = myrepid ;
             end if ;
             insert into alldo_ipla_iot_wkorder_replaceline(order_id,replace_owner,equipment_id,replace_start_datetime,create_date,replace_type)
        ...
        self.env.cr.execute("""create or replace function postockinqcline(moid int,prodnum float,ngnum float) returns void as $BODY$
          DECLARE
            ...
            ncount int ;
          BEGIN
            select count(*) into ncount from alldo_ipla_iot_workorder_qc where order_id = moid ;
            if ncount > 0 then
               select max(id) into mymaxid from alldo_ipla_iot_workorder_qc where order_id = moid ;
               update alldo_ipla_iot_workorder_qc set qc_good_num=prodnum,processing_ng_num=ngnum where id =mymaxid ;
            end if ;
          END;$BODY$
          LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function genprodrealnum() returns void as $BODY$
         DECLARE
           ...
           mymoldid int ;
         BEGIN
           delete from alldo_ipla_iot_production_real ;
           select current_date::DATE into currentdate1 ;
           select (current_date - interval '3 month')::DATE into currentdate2 ;
           open wk_cur for select * from alldo_ipla_iot_workorder_iot_data where iot_date::DATE between currentdate2::DATE and currentdate1::DATE ;
           loop
             fetch wk_cur into wk_rec ;
             exit when not found ;
             select product_no,eng_type into prodid,engtype from alldo_ipla_iot_workorder where id = wk_rec.order_id ;
             select product_tmpl_id into prodtmplid from product_product where id = prodid ;
             select mold_id into mymoldid from product_template where id = prodtmplid ;
             select mold_cavity into moldcavity from alldo_ipla_iot_ipla_mold where id = mymoldid ;
             select id into myid from alldo_ipla_iot_production_real where prod_id=prodtmplid and eng_type='1' ;
             if myid is not null then
                update alldo_ipla_iot_production_real set prod_num = coalesce(prod_num,0) + coalesce(wk_rec.iot_num,0),
                   prod_time = coalesce(prod_time,0) + coalesce(wk_rec.iot_duration,0) where id = myid ;
             else
                insert into alldo_ipla_iot_production_real(prod_id,eng_type,prod_num,prod_time) values (prodtmplid,'1',coalesce(wk_rec.iot_num,0),coalesce(wk_rec.iot_duration,0)) ;
             end if ;
            
           end loop ;
           close wk_cur ;    
           open real_cur for select * from alldo_ipla_iot_production_real ;
           loop
             ...
             select id,coalesce(mold_cavity,1) into myid,moldcavity from alldo_ipla_iot_eng_order where prod_id = real_rec.prod_id and eng_type = real_rec.eng_type ;
             /* realprod = round((realprod::numeric/moldcavity::numeric),0) ;*/
             update alldo_ipla_iot_eng_order set prod_real_num = realprod where id = myid ;
           end loop ;
           close real_cur ;
         END;$BODY$
         LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function genwkequipment(startdate date,enddate date) returns void as $BODY$
         ...
         BEGIN
           ...
           loop
             ...
             select sum(coalesce(replace_duration,0)) into iotdurmin from alldo_ipla_iot_wkorder_replaceline where equipment_id=equ_rec.id and 
                replace_start_datetime::DATE between startdate::DATE and enddate::DATE ;   
             iotdurh1 = round((iotdurmin::numeric/60),2) ;    
             iotdurh = round((iotdur::numeric/3600),2) + iotdurh1 ;
        ...
        DECLARE
          ...
          mywkid int ;
        BEGIN
          delete from alldo_ipla_iot_prod_goodng_list ; 
          if prodid = 0 then
             if nodeid = 0 then
                open prod_cur for select distinct product_no from alldo_ipla_iot_workorder_qc where qc_date::DATE between
                ...
                loop
                   ...
                   select sum(coalesce(total_amount_num,0)),sum(coalesce(qc_good_num,0)),sum(coalesce(material_ng_num,0)+coalesce(processing_ng_num,0)) into goodnum,totamount,ngnum 
                       from alldo_ipla_iot_workorder_qc where product_no=prod_rec.product_no and qc_date::DATE between startdate::DATE and enddate::DATE; 
                   ...
                   insert into alldo_ipla_iot_prod_goodng_list(product_id,good_ratio,ng_ratio) values (prod_rec.product_no,goodratio,ngratio) ;
                end loop ;
                close prod_cur ;    
             else
                open prod_cur for select distinct product_no from alldo_ipla_iot_workorder_qc where qc_date::DATE between
                ...
                loop
                   ...
                   select sum(coalesce(total_amount_num,0)),sum(coalesce(qc_good_num,0)),sum(coalesce(material_ng_num,0)+coalesce(processing_ng_num,0)) into totamount,goodnum,ngnum 
                       from alldo_ipla_iot_workorder_qc where product_no=prod_rec.product_no and iot_node=nodeid and qc_date::DATE between
                       startdate::DATE and enddate::DATE; 
                   goodnum = totamount - ngnum ;    
                   goodratio = round((goodnum::numeric / totamount::numeric) * 100 ,2) ;
                   ngratio =  round((ngnum::numeric / totamount::numeric) * 100 ,2) ; 
              ...
              if nodeid = 0 then
                 select sum(coalesce(total_amount_num,0)),sum(coalesce(qc_good_num,0)),sum(coalesce(material_ng_num,0)+coalesce(processing_ng_num,0)) into totamount,goodnum,ngnum 
                    from alldo_ipla_iot_workorder_qc where product_no=prodid and qc_date::DATE between startdate::DATE and enddate::DATE; 
              else
                 select sum(coalesce(total_amount_num,0)),sum(coalesce(qc_good_num,0)),sum(coalesce(material_ng_num,0)+coalesce(processing_ng_num,0)) into totamount,goodnum,ngnum 
                    from alldo_ipla_iot_workorder_qc where product_no=prodid and iot_node=nodeid and qc_date::DATE between startdate::DATE and enddate::DATE;      
              end if ;
              goodnum = totamount - ngnum ;
              goodratio = round((goodnum::numeric / totamount::numeric) * 100 ,2) ;
              ngratio =  round((ngnum::numeric / totamount::numeric) * 100 ,2) ;
              insert into alldo_ipla_iot_prod_goodng_list(product_id,good_ratio,ng_ratio) values (prodid,goodratio,ngratio) ; 
          end if ;
          delete from alldo_ipla_iot_prod_goodng_list where product_id is null ;
        END;$BODY$
        LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create or replace function genqcprod() returns void as $BODY$
         DECLARE
           ...
           wk_rec record ;
         BEGIN
           open wk_cur for select id,product_no from alldo_ipla_iot_workorder ;
           loop
             fetch wk_cur into wk_rec ;
             exit when not found ;
             update alldo_ipla_iot_workorder_qc set product_no=wk_rec.product_no where order_id=wk_rec.id ;
           end loop ;
           close wk_cur ;
           update alldo_ipla_iot_workorder_qc set iot_owner1=iot_owner ;
         END;$BODY$
         LANGUAGE plpgsql;""")
        ...
        self.env.cr.execute("""create index qcdate_index on alldo_ipla_iot_workorder_qc(qc_date)""")
        self.env.cr.execute("""drop index if exists iotowner1_index cascade""")
        self.env.cr.execute("""create index iotowner1_index on alldo_ipla_iot_workorder_qc(iot_owner1)""")
        self.env.cr.execute("""drop index if exists iotdata1_index cascade""")
        self.env.cr.execute("""create index iotdata1_index on alldo_ipla_iot_workorder_iot_data(iot_owner)""")
        self.env.cr.execute("""drop index if exists iotdata2_index cascade""")
        self.env.cr.execute("""create index iotdata2_index on alldo_ipla_iot_workorder_iot_data(iot_date)""")
