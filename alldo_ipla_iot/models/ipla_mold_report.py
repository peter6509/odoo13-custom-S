...
class iplamoldreport(models.Model):
    _name = "alldo_ipla_iot.mold_report"
    _description = "模具條碼列印主檔"

    report_owner = fields.Many2one('res.users',string="製表人",default=lambda self:self.env.uid)
    mold_line = fields.One2many('alldo_ipla_iot.mold_report_line','mold_id',string="條碼列印檔",copy=False)


class iplamoldreportline(models.Model):
    _name = "alldo_ipla_iot.mold_report_line"
    _description = "模具條碼列印檔"

    mold_id = fields.Many2one('alldo_ipla_iot.mold_report',ondelete='cascade')
