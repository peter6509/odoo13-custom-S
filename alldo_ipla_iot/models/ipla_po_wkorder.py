...
class alldoiplapowkorder(models.Model):
    _name = "alldo_ipla_iot.po_wkorder"
    ...
    po_line = fields.One2many('alldo_ipla_iot.po_wkorder_line','po_id',copy=False)
    sub_line = fields.One2many('alldo_ipla_iot.po_suborder_line','po_id',copy=False)
...
class alldoiplapowkorderline(models.Model):
    _name = "alldo_ipla_iot.po_wkorder_line"
    _description = "訂單-工單明細記錄"

    po_id = fields.Many2one('alldo_ipla_iot.po_wkorder',ondelete='cascade')
    wkorder_id = fields.Many2one('alldo_ipla_iot.workorder',string="工單")
...
class alldoiplaposuborderline(models.Model):
    _name = "alldo_ipla_iot.po_suborder_line"
    _description = "訂單-委外加工明細記錄"

    po_id = fields.Many2one('alldo_ipla_iot.po_wkorder',ondelete='cascade')
    outsourcing_id = fields.Many2one('alldo_ipla_iot.outsuborder',string="委外加工單")
    eng_type = fields.Char(string="工程別")
    eng_seq = fields.Integer(string="工序seq")
    eng_order = fields.Selection([('1','首工序'),('2','中工序'),('3','完工序')],string="工序位置")
    complete_num = fields.Float(digits=(10,2),string="完成數量")
