...
class newebpartnerexportwizard(models.TransientModel):
    _name = "neweb.partner_export_wizard"
    _description = "業務匯出客戶資料"

    @api.depends()
    def _get_empid(self):
        myempid = self.env.user.employee_ids[0].id
        self.emp_id = myempid
        return myempid

    emp_id = fields.Many2one('hr.employee',string="業務員",default=_get_empid)
    all_select = fields.Boolean(string="全選",default=False)
    export_type = fields.Selection([('1','客戶'),('2','供應商'),('3','兩者皆要')],string="匯出類別",default='1')
    partner_ids = fields.Many2many('res.partner','partner_export_wizard_rel','wizard_id','partner_id',string="客戶")


    # @api.onchange('create_uid')
    # def onchangeclient(self):
    #     self.env.cr.execute("""select getuseremp(%d)""" % self.env.uid)
    #     self.emp_id = self.env.cr.fetchone()[0]


    @api.onchange('emp_id')
    def onchangeckient1(self):
        if not self.emp_id:
            myempid = 0
        else:
            myempid = self.emp_id.id
        self.env.cr.execute("""select getsalepartner(%d)""" % myempid)
        myrec = self.env.cr.fetchall()
        partner_ids=[]
        for rec in myrec:
            partner_ids.append(rec[0])
        res = {}
        res['domain'] = {'partner_ids': [('id', 'in', partner_ids)]}
        return res

    def run_partner_export(self):

        if self.all_select==True:
            if not self.emp_id:
                myempid = 0
            else:
                myempid = self.emp_id.id
            ...
            self.env.cr.execute("""select genallpartnerlist(%d,'%s')""" % (myempid,self.export_type))
        else:
            self.env.cr.execute("""select genpartnerlist(%d)""" % self.id)
        self.env.cr.execute("""commit""")
        mypartnerrec = self.env['neweb.partner_list'].search([])
        output = io.BytesIO()

        myxlsfilename = "PARTNER_%s_%s.xlsx" % (self.env.user.name, datetime.now().strftime("%Y%m%d"))
        mysubject = 'PARTNER_%s_%s.xlsx' % (self.env.user.name, datetime.now().strftime("%Y%m%d"))
        ...
        titles1 = ['業務代表','客戶名稱', '統一編號', '電話', '地址', '聯絡人', '人員別', '職稱', '聯絡人電話', '聯絡人手機', '聯絡人EMAIL',
                   '生日', '備註']
        title_width = [60, 45, 20, 25, 60, 20, 30, 20, 25, 25, 35, 15, 60]
