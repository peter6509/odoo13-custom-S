...
class newebprojextstoreproc(models.Model):
    ...
    def init(self):
        ...
        self._cr.execute("""create or replace function a4q3check_cusproject(projid int) returns Boolean as $BODY$
             declare 
               ncount int;
               myresult Boolean;
             begin 
        ...
        self._cr.execute("""create or replace function gencostdeptdata(projid int) returns void as $BODY$
           ...
           BEGIN
             ...
             loop
               ...
               if pl_rec.cost_type=3 then   /*  建置 */
               ...
               elsif pl_rec.cost_type=4 or pl_rec.cost_type=9 or pl_rec.cost_type=5 or pl_rec.cost_type=12 then  /* 維護舊約,維護新約,維運人力,保固維護 */
                  if pl_rec.cost_dept in (2,3) then  /* R6 */
                  ...
                  elsif pl_rec.cost_dept in (10,11,12) then   /* SUPPORT & 駐點成本 */ 
                     ...
                     elsif pl_rec.cost_dept=10 or pl_rec.cost_dept=12 then /* SUPPORT OUTSOURCING and 駐點成本 COST */   
             ...
             if m_r6outcost+m_ntoutcost+m_nwoutcost+m_supoutcost > 0 then
                 ...
                 if m_supoutcost > 0 then
                    select count(*) into ncount from neweb_maincost_line where project_id=projid and name='1'  ;
                     if ncount = 0 then
                        insert into neweb_maincost_line (project_id,name) values (projid,'1') ;
                        update neweb_maincost_line set support_revenue=m_supoutcost,support_percent=m_supoutper where project_id=projid and name='1' ;  
                     end if ;  
                    
                 end if ;     
                 select sum(support_revenue) into m_supoutcost from neweb_maincost_line where name in ('2','3','4') and project_id=projid ;  
                 if m_supoutcost > 0 then
                    select count(*) into ncount from neweb_maincost_line where project_id=projid and name='1'  ;
                     if ncount = 0 then
                        insert into neweb_maincost_line (project_id,name) values (projid,'1') ;            
        ...
        self._cr.execute("""create or replace function gencostdeptper(projid int) returns void as $BODY$
           ...
           BEGIN
              ...
              loop
               ...
               exit when not found ;
               if (coalesce(prev_rec.r6_revenue,0) + coalesce(prev_rec.nt_revenue,0) + coalesce(prev_rec.networking_revenue,0) + coalesce(prev_rec.pm_revenue,0)) > 0 then
        ...
        self._cr.execute("""create or replace function getanalysistotrev(projid int) returns Float as $BODY$
         ...
         BEGIN
             ...
             select sum(coalesce(analysis_revenue,0)) into mysetupcost from neweb_projanalysis where analysis_id = projid 
                and analysis_costtype = 3 ;
             select sum(coalesce(analysis_revenue,0)) into mymaincost from neweb_projanalysis where analysis_id = projid 
                and analysis_costtype in (9,4,12,5) ;  
        ...
        DECLARE
          myres float ;
        BEGIN
            select sum(analysis_revenue) into myres from neweb_projanalysis where analysis_id = projid and analysis_costtype in (4,9) ;
            return myres ;   
        END;$BODY$
        LANGUAGE plpgsql;""")
        ...
        BEGIN
            myres = 0 ;
            select sum(coalesce(analysis_cost,0)) into myres from neweb_projanalysis where analysis_id = projid 
               and analysis_costtype in (4,5,9,12) ;
            return coalesce(myres,0) ;   
        END;$BODY$
        LANGUAGE plpgsql;""")
         ...
         BEGIN
           ...
           loop 
             ...
             select sum(colesce(prod_num,0) * coalesce(prod_revenue,0)) into subtotrev from neweb_projsaleitem
                     where saleitem_id=main_rec.project_id and cost_type in (4,9) ;
             if subtotrev > 0 then
