...
class newebtravelflow(models.Model):
    ...
    def create(self, vals):
        res = super(newebtravelflow, self).create(vals)
        self.env.cr.execute("""select gentravelflowman(%d)""" % res.id)
    ...
    def write(self, vals):
        if 'x_wkf_state' in vals:
           ...
           myflow = False
        res = super(newebtravelflow, self).write(vals)
        for rec in self:
            self.env.cr.execute("""select gentravelflowman(%d)""" % rec.id)
            self.env.cr.execute("""commit""")
    ...
    def travel_start(self):
        self.env.cr.execute("""update neweb_sale_analysis_travel_report set write_lock=True where id=%d""" % self.id)
        self.env.cr.execute("""commit""")

    def travel_email_1_line(self):
        self.env.cr.execute("""update neweb_sale_analysis_travel_report set write_lock=True where id=%d""" % self.id)
        self.env.cr.execute("""commit""")
        ...
        for rec in self:
            ...
            if rec.flow_man1.work_email and rec.flow_man1.work_email.strip():
                ...
                template = self.env.ref('neweb_sale_analysis.travel_flow_message')
                template.with_context(local_context).send_mail(self.id, force_send=True)

    def travel_email_2_line(self):
        self.env.cr.execute("""update neweb_sale_analysis_travel_report set write_lock=True where id=%d""" % self.id)
        self.env.cr.execute("""commit""")
        base_url = request.env['ir.config_parameter'].sudo().get_param('web.bf.url')
        base_url += '/web#id=%d&view_type=form&model=%s' % (self.id, self._name)
    ...
    def travel_email_1_reject(self):
        self.env.cr.execute("""update neweb_sale_analysis_travel_report set write_lock=False where id=%d""" % self.id)
        self.env.cr.execute("""commit""")
        base_url = request.env['ir.config_parameter'].sudo().get_param('web.bf.url')
        base_url += '/web#id=%d&view_type=form&model=%s' % (self.id, self._name)
        for rec in self:
            ...
            if rec.flow_owner.work_email and rec.flow_owner.work_email.strip():
                ...
                template = self.env.ref('neweb_sale_analysis.travel_flow_message_reject')
                template.with_context(local_context).send_mail(self.id, force_send=True)

    def travel_email_2_reject(self):
        self.env.cr.execute("""update neweb_sale_analysis_travel_report set write_lock=False where id=%d""" % self.id)
        self.env.cr.execute("""commit""")
        base_url = request.env['ir.config_parameter'].sudo().get_param('web.bf.url')
        base_url += '/web#id=%d&view_type=form&model=%s' % (self.id, self._name)
    ...
    def travel_email_3_reject(self):
        self.env.cr.execute("""update neweb_sale_analysis_travel_report set write_lock=False where id=%d""" % self.id)
        self.env.cr.execute("""commit""")
        base_url = request.env['ir.config_parameter'].sudo().get_param('web.bf.url')
        base_url += '/web#id=%d&view_type=form&model=%s' % (self.id, self._name)
    ...
    def travel_email_4_reject(self):
        self.env.cr.execute("""update neweb_sale_analysis_travel_report set write_lock=False where id=%d""" % self.id)
        self.env.cr.execute("""commit""")
        base_url = request.env['ir.config_parameter'].sudo().get_param('web.bf.url')
        base_url += '/web#id=%d&view_type=form&model=%s' % (self.id, self._name)
    ...
    def travel_email_complete(self):
        base_url = request.env['ir.config_parameter'].sudo().get_param('web.bf.url')
        base_url += '/web#id=%d&view_type=form&model=%s' % (self.id, self._name)
        for rec in self:
            local_context = self.env.context.copy()
            if rec.flow_owner.work_email and rec.flow_owner.work_email.strip():
                ...
                template = self.env.ref('neweb_sale_analysis.travel_flow_message_complete')
                template.with_context(local_context).send_mail(self.id, force_send=True)
