...
class newebchiinvoicingunexportmainproj(models.Model):
    _name = "neweb_chi_invoicing.un_export_main_proj"
    _description = "生成未匯出(維護)專案數據暫存主檔"
    ...
    export_line = fields.One2many('neweb_chi_invoicing.un_export_main_proj_line','export_id', string="專案清單")
    ...
    def selectitemall(self):
        myrec = self.env['neweb_chi_invoicing.un_export_main_proj_line'].search([])
        for rec in myrec:
    ...
    def run_project_export(self,projid):
        ...
        wb1.set_properties({
            ...
            'manager': 'NEWEB INFO',
            'company': '藍新資訊股份有限公司',
            'category': '進銷存專案數據(維護案)',
            'keywords': '進銷存專案數據(維護案)',
            'created': datetime.now(),
            'comments': 'Created By Odoo'})

        ws1 = wb1.add_worksheet("進銷存成本分析(維護案)主檔")
        ...
        for line in myprojrec:
            ...
            mycontractrec = self.env['neweb_contract.contract'].search([('project_no','=',line.name)])
            if mycontractrec:
                myprojno = mycontractrec.contract_no[:-6]
                mycusname = mycontractrec.customer_name.name[0:20]
            else:
                myprojno = line.name
                mycusname = line.cus_project[0:20]
            myprojname = mycusname
    ...
    def run_package_project_export(self):
        ...
        ws5 = wb5.add_worksheet("進銷存(維護案)成本分析主檔(匯整)")
    ...
    def run_product_export(self,projid):
        ...
        ws2 = wb2.add_worksheet("進銷存(維護案)成本分析產品料號檔")
    ...
    def run_package_product_export(self):
        ...
        ws6= wb6.add_worksheet("進銷存(維護案)成本分析產品料號檔(匯整)")
    ...
    def run_purchase_export(self, purid):
        ...
        if mypurrec:
            ...
            ws3 = wb3.add_worksheet("進銷存(維護案)成本分析進項憑證檔")
    ...
    def run_sale_export(self, projid):
        ...
        if mysalerec:
            ...
            ws4 = wb4.add_worksheet("進銷存(維護案)成本分析銷項憑證檔")
    ...
    def run_package_sale_export(self):
        ...
        if mysalerec:
            ...
            ws8 = wb8.add_worksheet("進銷存(維護案)成本分析銷項憑證檔(匯整)")
    ...
    def selectbtn(self):                 # 點選匯入
        myrec = self.env['neweb_chi_invoicing.un_export_main_proj_line'].search([('selectyn','=',True)])
...
class newebchiinvoicingunexportmainprojline(models.Model):
    _name = "neweb_chi_invoicing.un_export_main_proj_line"
    _description = "生成未匯出(維護)數據暫存明細檔"


    export_id = fields.Many2one('neweb_chi_invoicing.un_export_main_proj',ondelete="cascade")
...
class newebchiinvoicingunexportmainprojlog(models.Model):
    _name = "neweb_chi_invoicing.export_main_proj_log"
    _description = "生成匯出(維護案)數據記錄"
