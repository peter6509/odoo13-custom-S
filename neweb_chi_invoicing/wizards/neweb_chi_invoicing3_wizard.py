...
class newebchiinvoicing3wizard(models.Model):
    _name = "neweb_chi_invoicing.chiinvoicing3_wizard"
    _description = "進貨憑證整批匯出精靈"


    export_user = fields.Many2one('res.users', string="匯出人員", default=lambda self: self.env.uid)


    def run_invoicing3_export(self):
        # self.env.cr.execute("""delete from neweb_chi_invoicing_package_purchase""")
        # self.env.cr.execute("""commit""")
        self.env.cr.execute("""select gen_unexport_purinv(%d)""" % (self.export_user.id))
