...
class partnerwizard(models.TransientModel):
    ...
    partner_type = fields.Selection([('1','客戶'),('2','供應商'),('3','兩者皆是')],string="合作伙伴類別",default='1')
    ...
    def respartner_import(self):
        ...
        if self.start_row < 2 or self.end_row < 2:
            raise UserError("數值錯誤,ROW數值不能小於2")
        ...
        for row in range(nstartrow -1, nendrow):
            ...
            if cell.ctype in (XL_CELL_TEXT, XL_CELL_NUMBER):
                myvat = '' + str(cell.value).decode('utf-8')              # vat
                if len(myvat) > 8:
                    myvat=myvat[:-2]
            ...
            myname = ''
            if cell.ctype in (XL_CELL_TEXT,XL_CELL_NUMBER):
               myname = '' + str(cell.value).decode('utf-8')             # name

            cell = sheet.cell(row, 2)
            mycompsname = ''
            if cell.ctype in (XL_CELL_TEXT, XL_CELL_NUMBER):
                mycompsname = '' + str(cell.value).decode('utf-8')       # comp_sname


            cell = sheet.cell(row, 3)
            myzop = ''
            if cell.ctype in (XL_CELL_TEXT,XL_CELL_NUMBER):
               myzip = '' + str(cell.value).decode('utf-8')               # zopcode
               if not myzip:
                   myzip = ''
               else:
                   myzip = myzip[:-2]
            ...
            mystreet = ''
            if cell.ctype in (XL_CELL_TEXT,XL_CELL_NUMBER):
               mystreet = ''+ str(cell.value).decode('utf-8')             # street
            ...
            mycontactname = ''
            if cell.ctype in (XL_CELL_TEXT,XL_CELL_NUMBER):
               mycontactname = ''+str(cell.value).decode('utf-8')         # contactname


            cell = sheet.cell(row, 6)
            myphone = ''
            if cell.ctype in (XL_CELL_TEXT,XL_CELL_NUMBER):
               myphone = ''+str(cell.value).decode('utf-8')               # phone


            cell = sheet.cell(row, 7)
            mymobile = ''
            if cell.ctype in (XL_CELL_TEXT,XL_CELL_NUMBER):
                mymobile = ''+str(cell.value).decode('utf-8')             # mobile

            cell = sheet.cell(row, 8)
            myfax = ''
            if cell.ctype in (XL_CELL_TEXT, XL_CELL_NUMBER):
                myfax = '' + str(cell.value).decode('utf-8')              # fax


            cell = sheet.cell(row, 9)
            myemail = ''
            if cell.ctype in (XL_CELL_TEXT,XL_CELL_NUMBER):
               myemail = ''+str(cell.value).decode('utf-8')               # email
