...
class newebprojwizard(models.TransientModel):
    ...
    comp_name1 = fields.Many2one('res.partner', string='專案客戶',
                                 domain=[('is_company', '=', 'True'), ('customer', '=', 'True'),
                                         ('parent_id', '=', False)], required=True)
    comp_name2 = fields.Many2one('res.partner', string='維護客戶',
                                 domain=[('is_company', '=', 'True'), ('customer', '=', 'True'),
                                         ('parent_id', '=', False)])
    proj_branch = fields.Many2one('neweb.projbranch', string="專案成本分區", required=True)
    proj_sale = fields.Many2one('hr.employee', string='專案業務', required=True,default=lambda self:self.get_projsale)
