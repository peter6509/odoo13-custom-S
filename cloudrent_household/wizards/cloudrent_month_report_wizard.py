...
class cloudrentReportWizard(models.TransientModel):
    _name = "cloudrent_household.month_report_wizard"
    ...
    house_id = fields.Many2one('cloudrent.household_house',string="案場名稱",required=True)
    report_type = fields.Selection([('1','對帳報表PDF'),('2','對帳報表EXCEL')],string="輸出型態",default='1')


    def cloudrent_month_print(self):
        ...
        if self.report_type=='1':  # PDF對帳單
            ...
            return self.env.ref('cloudrent_household.action_cloudrent_mmonth_report').report_action(self, data=data)
        else:                  # EXCEL 對帳單
            # mybillm = self.bill_ym[5:7]
            # mybilly = self.bill_ym[0:4]
            # if mybillm == '01':
            ...
            month_rec = self.env['cloudrent.household_house_line'].search([])
            output = io.BytesIO()
            myxlsfilename = "cloudrent租戶月費用對帳報表_%s.xlsx" % datetime.now().strftime("%Y%m%d")
            mysubject = 'cloudrent租戶月費用對帳報表_%s.xlsx' % datetime.now().strftime("%Y%m%d")
            wb = xlsxwriter.Workbook(output, {'in_memory': True})

            wb.set_properties({
                'title': 'cloudrent租戶月費用對帳報表',
                'subject': mysubject,
            ...
            nnum = 1
            ws = {}
            ws = wb.add_worksheet("cloudrent(%s)-%s對帳單" % (month_rec[0].house_id.project_no,mybillym))
            ########################################
            row = 0
            col = 0
            for title in titles:
