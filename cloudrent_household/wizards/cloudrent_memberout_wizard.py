...
class cloudrentMemberOutWizard(models.TransientModel):
    _name = "cloudrent_household.member_out_wizard"
    ...
    house_id = fields.Many2one('cloudrent.household_house_line', string="退租房間", required=True)
    member_id = fields.Many2one('cloudrent.household_member', string="原租戶")
    ...
    def run_member_out(self):
        if self.old_archive:
            self.env.cr.execute("""select cloudrent_chkout_archive(%d,'%s')""" % (self.member_id.id,True))
            self.env.cr.execute("""commit""")
        else:
            self.env.cr.execute("""select cloudrent_chkout_archive(%d,'%s')""" % (self.member_id.id,False))
            self.env.cr.execute("""commit""")

        self.env.cr.execute("""update cloudrent_household_house_line set member_id=null,user_id=null,in_used=False where id=%d""" % self.house_id.id)
        self.env.cr.execute("""commit""")
