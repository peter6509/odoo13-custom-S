...
class IOT_UTIL:
    ...
    def wip_reboot(node_ip):
        ...
        ssh.get_transport().open_session().exec_command("kill -9 $(ps -ef|grep mes_client.py|grep -v grep|awk '{print $2}')")
    ...
    def iot_push_file(node_ip, localpath, remotepath):  ## 傳檔到終端點
        ...
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        # ssh.connect(node_ip, 22, 'root', '!99999ibm')
        # ssh.get_transport().open_session().exec_command("kill -9 $(ps -ef|grep cnc_client.py|grep -v grep|awk '{print $2}')")
