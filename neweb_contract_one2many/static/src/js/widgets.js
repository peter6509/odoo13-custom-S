odoo.define('web_one2many_selectable_10.form_widgets', function (require) {
	...
	ListRenderer.include({
		...
		_updateSelection: function () {
	        ...
	        if(this.selection.length > 0){
	        	$('.button_delete_contract_lines').show()
	        }else{
	        	$('.button_delete_contract_lines').hide()
	        }
	...
	var One2ManySelectable = FieldOne2Many.extend({
		...
		events: {
			"click .button_delete_contract_lines": "action_selected_lines",
		},
		start: function()
	    {
		...
		{		
			...
			rpc.query({
                'model': 'neweb_contract.contract',
                'method': 'delete_contract_lines',
                'args': [selected_ids],
