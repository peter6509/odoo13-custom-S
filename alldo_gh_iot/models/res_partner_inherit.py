...
class alldorespartnerinherit(models.Model):
    ...
    outsourcing_out_line = fields.One2many('alldo_gh_iot.partner_prodout','partner_id',string="委外供應商加工給料記錄")
    outsourcing_in_line = fields.One2many('alldo_gh_iot.partner_prodin','partner_id',string="委外供應商加工回廠記錄")

    @api.model
    def create(self, vals):
        res = super(alldorespartnerinherit, self).create(vals)
        self.env.cr.execute("""select gensupplierloc(%d)""" % res.id)
        self.env.cr.execute("""commit""")
        return res


class partneroutprodin(models.Model):
    _name = "alldo_gh_iot.partner_prodin"
...
class partneroutprodout(models.Model):
    _name = "alldo_gh_iot.partner_prodout"
