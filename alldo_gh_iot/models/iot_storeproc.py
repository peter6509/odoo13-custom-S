...
class alldoiotstoreproc(models.Model):
    ...
    def init(self):
       ...
       """)
        ...
        self.env.cr.execute("""create or replace function setallmoclose(moid int) returns void as $BODY$
           ...
           BEGIN
             ...
             update alldo_gh_iot_schedule_line set state='4',active=false where mo_group_id=mogroupid ;
        ...
        self._cr.execute("""drop function if exists getprodactive(prodid int) cascade""")
        self._cr.execute("""create or replace function getprodactive(prodid int) returns boolean as $BODY$
          DECLARE
             myres boolean ;
             ncount int ;
             prodtmplid int ;
          BEGIN
             select active into myres from product_product where id = prodid ;
             if myres is null then
                 myres = False ;
             end if ;
             return myres ;
        ...
        self.env.cr.execute("""CREATE OR REPLACE VIEW ghiot_prodstock_view AS
           ...
           (select getprodcus(A.product_id)) as cus_no,(select getprodactive(A.product_id)) as prodactive from stock_quant A left join stock_location B
        ...
        self.env.cr.execute("""create or replace function genprodstock(prodid int) returns void as $BODY$
          ...
          BEGIN
            ...
            if prodid = 0 then
               open st_cur for select * from ghiot_prodstock_view where location_id in (8,19) order by product_id  ;
            else
               open st_cur for select * from ghiot_prodstock_view where product_id=prodid and location_id in (8,19)  ;
            end if ;
            loop
       ...
       """)
        ...
        self.env.cr.execute("""drop function if exists genstockmovemixsearch1(partnerid int,prodid int) cascade""")
        self.env.cr.execute("""create or replace function genstockmovemixsearch1(partnerid int,prodid int) returns void as $BODY$
          DECLARE
            mv_cur refcursor ;
            mv_rec record ;
            mypartnerid int ;
          ...
          BEGIN
            ...
            nitem = 0 ;
            if partnerid = 0 then
                open mv_cur for select * from ghiot_stockmovelist where  product_id=prodid  order by partner_id,product_id ;  
            elsif prodid = 0 then
                open mv_cur for select * from ghiot_stockmovelist where partner_id=partnerid order by product_id ;
            else
                open mv_cur for select * from ghiot_stockmovelist where partner_id=partnerid and product_id=prodid  order by product_id;     
            end if ;
            loop
              fetch mv_cur into mv_rec ;
              exit when not found ;
