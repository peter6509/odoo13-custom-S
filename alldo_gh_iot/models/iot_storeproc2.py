...
class iotstoreproc2(models.Model):
    ...
    def init(self):
        ...
        BEGIN  
            ...
            select product_no,id into prodid,poid from alldo_gh_iot_po_wkorder where po_no=pono ;
            if prodid is not null then
               ...
               open wk_cur for select * from alldo_gh_iot_po_wkorder_line where po_id=poid order by eng_seq ;
               loop
                 fetch wk_cur into wk_rec ;
                 exit when not found ;
                 open iot1_cur for select distinct iot_owner from alldo_gh_iot_workorder_iot_data where order_id=wk_rec.wkorder_id ;
                    loop
                        fetch iot1_cur into iot1_rec ;
                        exit when not found ;
                        select sum(iot_num),sum(iot_duration),sum(std_duration) into iottotnum,iottotdur,stdtotdur from alldo_gh_iot_workorder_iot_data where order_id=wk_rec.wkorder_id and iot_owner=iot1_rec.iot_owner ; 
                        wktottime = round((iottotdur::numeric/3600::numeric),2) ; /* 工時(H) */
                        select std_duration,iot_node into stddur,iotnode from alldo_gh_iot_workorder_iot_data where order_id=wk_rec.wkorder_id and iot_owner=iot1_rec.iot_owner limit 1 ;
                        if stddur > 0 then
                           stdtotdur = round((iottotdur::numeric/stddur::numeric),0) ; /* 標準生產總量 */
                        else
                            stdtotdur = 0 ;
                        end if ;   
                        insert into alldo_gh_iot_piprod_analysis_line(piprod_id,eng_type,iot_node,iot_owner,prod_num,std_num,wk_time) values 
                          (analysisid,wk_rec.eng_type,iotnode,iot1_rec.iot_owner,iottotnum,stdtotdur,wktottime) ;
                    end loop;
                    close iot1_cur ;        
               end loop ;
               close wk_cur ;
