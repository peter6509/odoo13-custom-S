...
class alldoghiotempinerit(models.Model):
    _inherit = "hr.employee"

    emp_code = fields.Char(string="員工編號")
    attendance_line = fields.One2many('alldo_gh_iot.emp_attendance','attendance_id',string="出勤明細")
    ...
    def create(self, vals):
        ...
        res = super(alldoghiotempinerit, self).create(vals)
        return res

    def _write(self, vals):
        res = super(alldoghiotempinerit, self)._write(vals)
        for rec in self:
...
class alldoghiotempattendance(models.Model):
    _name = "alldo_gh_iot.emp_attendance"
    ...
    is_check = fields.Boolean(string="CHECK OK",default=False)
    is_active = fields.Boolean(string="ACTIVE OK",default=False)
    ...
    def check_attendance(self):  # 每日執行
        self.env.cr.execute("""select check_attendance()""")  # 檢查刷卡異常資料
