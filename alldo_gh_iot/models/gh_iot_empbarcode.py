...
class ghiotempinfo(models.Model):
   _name = "alldo_gh_iot.empinfo"
   _rec_name= "empinfo_date"

   empinfo_date = fields.Date(string="製作日期")
   barcode_line = fields.One2many('alldo_gh_iot.empbarcode','barcode_id',copy=False)


class ghiotempbarcode(models.Model):
   _name = "alldo_gh_iot.empbarcode"
   _description = "人員卡片產生檔"


   barcode_id = fields.Many2one('alldo_gh_iot.empinfo',ondelete='cascade')
