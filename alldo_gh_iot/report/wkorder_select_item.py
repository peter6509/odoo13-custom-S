...
class wkorderselectitem(models.Model):
    _name = "alldo_gh_iot.wkorder_selectitem"

    report_owner = fields.Many2one('res.users',string="報表產出人")
    report_date = fields.Datetime(string="製作時間")
    report_line = fields.One2many('alldo_gh_iot.wkorder_selectitem_line','item_id',string="明細")

    def run_all_select(self):
        myrec = self.env['alldo_gh_iot.wkorder_selectitem'].search([])
    ...
    def print_wkorder(self):
        mycount = self.env['alldo_gh_iot.wkorder_selectitem_line'].search_count([('item_id','=',self.id),('select_yn','=',True)])
        self.env.cr.execute("""select genselectitemreport()""")
        self.env.cr.execute("""commit""")
        if mycount == 0 :
            raise UserError("沒有選擇任何要列印的工單！")
        # self.filtered(lambda s: s.state == 'draft').write({'state': 'sent'})
        myrec = self.env['alldo_gh_iot.wkorder_printsheet'].search([])
        return self.env.ref('alldo_gh_iot.action_alldo_gh_iot_wkorder_report').report_action(myrec)
...
class wkorderselectitemline(models.Model):
    _name = "alldo_gh_iot.wkorder_selectitem_line"

    item_id = fields.Many2one('alldo_gh_iot.wkorder_selectitem',ondelete='cascade')
...
class wkorderprintsheet(models.Model):
    _name = "alldo_gh_iot.wkorder_printsheet"
    ...
    correct_no1 = fields.Char(string="補正號碼1")
    inspect1_point = fields.Char(string="頻率1")
    inspect_tool1 = fields.Many2one('alldo_gh_iot.measure_tool', string="量測工具1")
    ...
    correct_no2 = fields.Char(string="補正號碼2")
    inspect2_point = fields.Char(string="頻率2")
    inspect_tool2 = fields.Many2one('alldo_gh_iot.measure_tool', string="量測工具2")
    ...
    correct_no3 = fields.Char(string="補正號碼3")
    inspect3_point = fields.Char(string="頻率3")
    inspect_tool3 = fields.Many2one('alldo_gh_iot.measure_tool', string="量測工具3")
    ...
    correct_no4 = fields.Char(string="補正號碼4")
    inspect4_point = fields.Char(string="頻率4")
    inspect_tool4 = fields.Many2one('alldo_gh_iot.measure_tool', string="量測工具4")
    ...
    correct_no5 = fields.Char(string="補正號碼5")
    inspect5_point = fields.Char(string="頻率5")
    inspect_tool5 = fields.Many2one('alldo_gh_iot.measure_tool', string="量測工具5")
