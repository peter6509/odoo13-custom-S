...
class ghiotreplacelinewizard(models.TransientModel):
    _name = "alldo_gh_iot.replaceline_wizard"
    ...
    def run_replaceline(self):
        ...
        mywkrec = self.env['alldo_gh_iot.replaceline_list'].search([])
        mycount = self.env['alldo_gh_iot.replaceline_list'].search_count([])
        output = io.BytesIO()
        if self.rep_owner.id == False :
