...
class outsourcinginoutwizard(models.TransientModel):
    _name = "alldo_gh_iot.outsourcing_inout_wizard"
    ...
    def run_outsourcing_inout(self):
        ...
        myinoutrec = self.env['alldo_gh_iot.inout_prod_list'].search([])
        mycount = self.env['alldo_gh_iot.inout_prod_list'].search_count([])
        output = io.BytesIO()
        ...
        ws = wb.add_worksheet("精宏機械委外料件進出記錄")
        ########################################
