...
class ghiotblankstockin(models.TransientModel):
    _name = "alldo_gh_iot.blank_stockin_wizard"
    _description = "毛胚追加精靈"


    po_id = fields.Many2one('alldo_gh_iot.po_wkorder',string="客戶單號")
    ...
    mo_no = fields.Many2one('alldo_gh_iot.workorder',string=u"工單號",required=True)
    ...
    is_out_blank = fields.Boolean(string="是否直接委外供料？", default=False)
    ...
    def _get_complete_stockin(self):
        myrec = self.env['alldo_gh_iot.workorder'].search([('id','=',self.mo_no.id)])
    ...
    def run_blank_stockin(self):
        ...
        if self.blank_num > 0:
            ...
            myres = myrec.create({'picking_type_id': 1, 'location_id': mycuslocid, 'location_dest_id': myblanklocid, 'move_type': 'direct',
                                  'user_id': self.stockin_owner.id, 'origin': self.po_desc,'partner_id':self.cus_name.id,'mo_group_id':mymogpid,'origin': '毛胚額外追加',
                                  'move_line_ids': [
        ...
        if self.is_out_blank==True and self.supplier_num > 0:
            ...
            myres = myrec.create(
                {'picking_type_id': 5, 'location_id': myblanklocid, 'location_dest_id': mysupplocid,
                 'move_type': 'direct',
                 'user_id': self.stockin_owner.id, 'origin': '毛胚追加直接委外供料', 'partner_id': self.supplier_no.id,
                 'move_line_ids': [
        ...
        self.env.cr.execute("""commit""")
        self.env.cr.execute("""select runmoadd(%d,%d)""" % (self.mo_no.id,self.blank_num))
        self.env.cr.execute("""commit""")
