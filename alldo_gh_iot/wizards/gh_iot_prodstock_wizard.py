...
class ghiotprodstockwizard(models.TransientModel):
    _name ="alldo_gh_iot.prodstock_wizard"
    ...
    def run_prod_stock(self):
        ...
        myviewid = self.env.ref('alldo_gh_iot.gh_iot_prodstock_tree')
        return {
            'view_name': 'prod_stock_list',
            'name': (u'產品庫存查詢'),
            'type': 'ir.actions.act_window',
            'res_model': 'alldo_gh_iot.prod_stock_list',
            'view_id': myviewid.id,
            'view_type': 'form',
            'view_mode': 'tree',
            'target': 'list'}