...
class ghoutsourcinginwizard(models.TransientModel):
    _name = "alldo_gh_iot.outsourcing_in"
    ...
    in_loc = fields.Selection([('1','WH/總庫(成品倉)'),('2','WH/毛胚倉'),('3','WH/半成品倉')],string="入庫倉別",default='2')
    ...
    def onclientchangeprodno(self):
        ...
        if not myrec:
            self.product_id1 = self.product_id.id
            return {'domain': {'product_id1': [(1, '=', 1)]}}
        else:
            ids = []
            for rec in myrec:
                ids.append(rec[0])
            return {'domain': {'product_id1': [('id', 'in', ids)]}}
                ...
                if self.product_id != self.product_id1:
                    ...
                    if self.in_good_num > 0:
                        ...
                        if self.in_loc == '1': # 成品倉
                            ...
                            myres1 = myrec.create(
                                {'picking_type_id': 5, 'location_id': mytranslocid, 'location_dest_id': mysemiprodlocid,
                                 'move_type': 'direct',
                                 'user_id': self.in_owner.id, 'origin': self.in_desc, 'partner_id': self.partner_id.id,
                                 'move_line_ids': [
                                     (0, 0,
                    ...
                    if self.in_good_num > 0 :
                        ...
                        if self.in_loc=='1': # 成品倉
                            ...
                            myres1 = myrec.create(
                                {'picking_type_id': 5, 'location_id': mysupplocid, 'location_dest_id': mysemiprodlocid,
                                 'move_type': 'direct','user_id': self.in_owner.id, 'origin': self.in_desc, 'partner_id': self.partner_id.id,
                                 'move_line_ids': [
                                     (0, 0,
                                      {'product_id': self.product_id1.id, 'company_id': 1, 'location_id': mysupplocid,
                                       'location_dest_id': mysemiprodlocid, 'product_uom_id': 1,
