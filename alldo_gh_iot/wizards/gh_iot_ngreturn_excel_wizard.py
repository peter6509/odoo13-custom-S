...
class ghiotngreturnexcelwizard(models.TransientModel):
    _name = "alldo_gh_iot.ngreturn_excel_wizard"
    ...
    def run_ngreturn_excel(self):
        ...
        myngreturnrec = self.env['alldo_gh_iot.ngreturn_list'].search([])
        mycount = self.env['alldo_gh_iot.ngreturn_list'].search_count([])
        output = io.BytesIO()

        myxlsfilename = "客戶NG退回記錄_%s_%s_%s.xlsx" % (self.partner_id.name,self.product_no.name, datetime.now().strftime("%Y%m%d"))
        mysubject = '客戶NG退回記錄_%s_%s_%s.xlsx' % (self.partner_id.name,self.product_no.name, datetime.now().strftime("%Y%m%d"))
        ...
        ws = wb.add_worksheet("精宏機械客戶NG退回記錄")
        ########################################
        ...
        titles = ['工單號碼', 'NG單號', '客戶', 'NG日期', '產品料號', '材料不良數', '加工不良', '來料短少' , '類型']
        title_width = [20, 20, 60, 20, 50, 20, 20, 20,20]
        ...
        for line in myngreturnrec:
            ...
            ws.write(row, 2, line.partner_id.name if line.partner_id else ' ', okl_content_format)
            ws.write(row, 3, line.ngreturn_date1 if line.ngreturn_date1 else ' ', okl_content_format)
            ws.write(row, 4, line.product_id.name if line.product_id else ' ', okl_content_format)
            ws.write(row, 5, line.material_ng_num if line.material_ng_num > 0 else ' ', okl_content_format)
            ws.write(row, 6, line.processing_ng_num if line.processing_ng_num > 0 else ' ', okl_content_format)
            ws.write(row, 7, line.loss_num if line.loss_num > 0 else ' ', okl_content_format)
            ws.write(row, 8, mytype, okc_content_format)
