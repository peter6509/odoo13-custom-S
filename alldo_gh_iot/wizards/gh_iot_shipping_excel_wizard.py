...
class ghiotshippingexcelwizard(models.TransientModel):
    _name = "alldo_gh_iot.shipping_excel_wizard"
    ...
    def run_shipping_excel(self):
        ...
        self.env.cr.execute("""select genshippingexcel(%d,%d,'%s','%s')""" % (
        self.partner_id.id, myprodid, self.start_date, self.end_date))
        self.env.cr.execute("""commit""")
        myshippingrec = self.env['alldo_gh_iot.shipping_list'].search([])
        mycount = self.env['alldo_gh_iot.shipping_list'].search_count([])
        output = io.BytesIO()

        myxlsfilename = "客戶出貨記錄_%s_%s_%s.xlsx" % (self.partner_id.name,self.product_no.name, datetime.now().strftime("%Y%m%d"))
        mysubject = '客戶出貨記錄_%s_%s_%s.xlsx' % (self.partner_id.name,self.product_no.name, datetime.now().strftime("%Y%m%d"))
        ...
        ws = wb.add_worksheet("精宏機械客戶出貨記錄")
        ########################################
        ...
        title_width = [20, 20, 20, 60, 20, 60, 20, 20]
        ...
        for line in myshippingrec:
            ...
            ws.write(row, 5, line.product_id.name if line.product_id else ' ', okl_content_format)
            ...
            # ws.write(row, 8, nsum ,okc_content_format)
            row += 1
            nitem += 1
