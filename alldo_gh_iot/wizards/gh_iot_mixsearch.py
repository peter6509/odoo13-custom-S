...
class alldoghiotmixsearch(models.TransientModel):
    _name = "alldo_gh_iot.mix_search_wizard"
    ...
    search_type = fields.Selection([('1','人員/機台/料號 生產效率明細表'),('2','機台稼動(時數)統計表'),('3','人員達成率統計表'),('4','不良率統計表'),('5','生產稼動率(依班別)統計表')],string="查詢類別",default='1')
    ...
    def run_mixsearch(self):
        if self.search_type=='1':
            ...
            myviewid = self.env.ref('alldo_gh_iot.ghiot_workorder_performance_tree')

            return {
                'view_name': 'workorder_performance_list',
                'name': (u'生產數據複合查詢統計'),
                'type': 'ir.actions.act_window',
                'res_model': 'alldo_gh_iot.workorder_performance_list1',
                'view_id': myviewid.id,
                'view_type': 'form',
                'view_mode': 'tree',
                'target': 'list'}
            ...
            self.env.cr.execute("""commit""")
            myviewid = self.env.ref('alldo_gh_iot.gh_iot_equipperformance_tree')

            return {
                'view_name': 'equipment_performance_list',
                'name': (u'生產數據機台稼動時間記錄'),
                'type': 'ir.actions.act_window',
                'res_model': 'alldo_gh_iot.equipment_performance_list',
                'view_id': myviewid.id,
                'view_type': 'form',
                'view_mode': 'tree',
                'target': 'list'}
            ...
            self.env.cr.execute("""commit""")
            myviewid = self.env.ref('alldo_gh_iot.gh_iot_manperformance_tree')
            return {
                'view_name': 'man_performance_list',
                'name': (u'人員達成率生產數據'),
                'type': 'ir.actions.act_window',
                'res_model': 'alldo_gh_iot.man_performance_list',
                'view_id': myviewid.id,
                'view_type': 'form',
                'view_mode': 'tree',
                'target': 'list'}
            ...
            self.env.cr.execute("""commit""")
            myviewid = self.env.ref('alldo_gh_iot.ghiot_goodng_list_tree')
            return {
                'view_name': 'man_performance_list',
                'name': (u'生產良率/不良率數據統計'),
                'type': 'ir.actions.act_window',
                'res_model': 'alldo_gh_iot.prod_goodng_list',
                'view_id': myviewid.id,
                'view_type': 'form',
                'view_mode': 'tree',
                'target': 'list'}
            ...
            self.env.cr.execute("""select genwkequipment2('%s','%s')""" % (self.wk_start_date, self.wk_end_date))
            self.env.cr.execute("""commit""")
            myviewid = self.env.ref('alldo_gh_iot.gh_iot_equipperformance_tree2')

            return {
                'view_name': 'equipment_performance_list',
                'name': (u'生產稼動率統計表'),
                'type': 'ir.actions.act_window',
                'res_model': 'alldo_gh_iot.workorder_performance_list2',
                'view_id': myviewid.id,
                'view_type': 'form',
                'view_mode': 'tree',
                'target': 'list'}
