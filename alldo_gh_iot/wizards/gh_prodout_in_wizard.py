...
class ghprodoutsourcinginwizard(models.TransientModel):
    _name = "alldo_gh_iot.prodoutsourcing_in"
    _description = "成品委外加工完工回廠"
    ...
    partner_prod_onhand1 = fields.Float(string="委外廠商數量", default=0.00)
    wh_prod_onhand1 = fields.Float(string="公司成品倉數量")
    ...
    in_loc = fields.Selection([('1','WH/總庫(成品倉)'),('2','WH/毛胚倉'),('3','WH/半成品倉')],string="入庫倉別",default='1')
    ...
    def onchangeclient1(self):
        self.env.cr.execute("""select getpartnerprodonhand(%d,%d)""" % (self.partner_id.prod_loc.id, self.product_id.id))
        self.partner_prod_onhand1 = self.env.cr.fetchone()[0]


    @api.onchange('product_id1')
    def onchangeclient2(self):
        self.env.cr.execute("""select getwhprodonhand(%d)""" % self.product_id1.id)
        self.wh_prod_onhand1 = self.env.cr.fetchone()[0]
    ...
    def run_prodout_in(self):
        ...
        if not self.partner_id:
            ...
            if self.in_good_num > 0 or self.in_ng_num > 0:
                ...
                mysupplocid = self.partner_id.prod_loc.id  # 委外商成品倉庫
