...
class ghiotngreturnreportwizard(models.TransientModel):
    _name = "alldo_gh_iot.ngreturn_report_wizard"
    ...
    def run_ngreturn_report(self):
        ...
        myrec = self.env['alldo_gh_iot.ngreturn_select'].search([])
        for rec in myrec:
            myid = rec.id

        myviewid = self.env.ref('alldo_gh_iot.ghiot_ngreturn_select_form')

        return {'view_name': 'ngreturn_report_action',
                'name': (u'NG return Data'),
                'views': [[False, 'form']],
                'res_model': 'alldo_gh_iot.ngreturn_select',
