...
class iotmostopwizard(models.TransientModel):
    _name = "alldo_gh_iot.iot_mo_stop_wizard"

    equipment_ids = fields.Many2many('maintenance.equipment', 'gh_iot_mo_stop_rel', 'wizard_id','equipment_id', string="機台",required=True)
    action_type = fields.Selection([('1','暫停後重啟'),('2','暫停'),('3','停機')],string="執行類別",required=True)

    def run_action_iot(self):
        myiotserverrec = self.env['alldo_gh_iot.server_info'].search([])
