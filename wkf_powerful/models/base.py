...
def wkf_trans_condition_expr_eval(self, lines):
    ...
    for line in lines.split('\n'):
        ...
        if line == 'True':
            ...
            self.env.cr.execute("""select count(*) from %s where %s and id=%d""" % (self._table,line,self.id))
...
def wkf_action(self, message=''):
    ...
    myids = []
    myres = False
    for g in trans.group_ids:
        data = data_obj.search([('res_id', '=', g.id), ('model', '=', 'res.groups')])
        a = data.complete_name
        if self.env.user.has_group(data.complete_name) :
            myres=True
            break
    if self.env.user.has_group('neweb_project.neweb_cs30_dir'):
        myres = True
    elif myres == False and trans.auto==False :
        raise UserError("很抱歉！您無此簽核權限.")
    ...
    if not trans.is_backward:
        self.env.cr.execute("""select transiscomplete(%d,%d)""" % (self.id,t_id))
        myres = self.env.cr.fetchone()[0]
        if myres:
