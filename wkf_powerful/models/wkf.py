...
class wkf_base(models.Model):
    ...
    def make_field(self):
        self.ensure_one()
        myid = self.id
        mymodelid = self.model_id.id
        mymodule = self.model_id.modules
        myselect = str(self.get_state_selection())
        ...
        fd_data = {
            ...
            'field_description': 'WorkFollow State',
            'selection': str(self.get_state_selection()),
        }
        if fd_id:
            # fd_id.write(fd_data)
            if self.model_id.id != 583:
                fd_id.write(fd_data)
            else:
                self.env.cr.execute("""select id from ir_model_fields where name='x_wkf_state' and model_id=%d """ % self.model_id.id)
                fd_id = self.env.cr.fetchone()[0]
                self.env.cr.execute("""delete from ir_model_fields_selection where field_id=%d""" % fd_id)
                self.env.cr.execute("""commit""")
                myseq=0
                for rec in self.node_ids:
                    self.env.cr.execute("""insert into ir_model_fields_selection(field_id,value,name,sequence) values (%d,'%s','%s',%d)""" % (fd_id,str(rec.id),rec.name,myseq))
                    myseq = myseq + 1

                self.write({'field_id': fd_id})
        else:
            if self.model_id.id == 583:
                self.env.cr.execute("""select genwkfstate('neweb_repair_repair')""")
                self.env.cr.execute("""commit""")
                self.env.cr.execute("""insert into ir_model_fields(name,ttype,state,model_id,model,field_description) values ('x_wkf_state','selection','manual',%d,'%s','WorkFollow State')""" % (self.model_id.id,self.model_id.model))
                self.env.cr.execute("""select max(id) from ir_model_fields""")
                fd_id = self.env.cr.fetchone()[0]
                myseq=0
                for rec in self.node_ids:
                    self.env.cr.execute("""insert into ir_model_fields_selection(field_id,value,name,sequence) values (%d,'%s','%s',%d)""" % (fd_id,str(rec.id),rec.name,myseq))
...
class wkf_trans(models.Model):
    ...
    xml_groups = fields.Char(compute=_compute_xml_groups, string='XML Groups',onchange_default=True)
