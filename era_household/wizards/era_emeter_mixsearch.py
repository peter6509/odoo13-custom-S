...
class eraemetermixsearch(models.TransientModel):
    _name = "era_household.emeter_mixsearch"

    house_id = fields.Many2one('era.household_house_line',string="房號",required=True)
    ...
    def run_emeter_mixsearch(self):
        ...
        myviewid = self.env.ref('era_household.era_mixsearch_line_tree')

        return {
            'view_name': 'era_mixsearch_line_tree',
            'name': (u'租房區間用電查詢統計'),
            'type': 'ir.actions.act_window',
            'res_model': 'era_household.emeter_mixsearch_line',
            'view_id': myviewid.id,
            'view_type': 'form',
            'view_mode': 'tree',
            'target': 'list'}
