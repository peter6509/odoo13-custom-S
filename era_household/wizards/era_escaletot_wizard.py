...
class EraEscaletotWizard(models.TransientModel):
    _name = "era.escaletot_wizard"
    ...
    def run_scaletot(self):
        ...
        scale_rec = self.env['era.escaletot_line'].search([])
        self.env.cr.execute("""select sum(duration_scale) from era_escaletot_line""")
        mytotscale = self.env.cr.fetchone()[0]
        output = io.BytesIO()
        myxlsfilename = "ERA案場區間用電數報表(%s ~ %s).xlsx" % (self.start_date,self.end_date)
        mysubject = 'ERA案場區間用電數報表(%s ~ %s).xlsx' % (self.start_date,self.end_date)
        ...
        nnum = 1
        ws = {}
        ws = wb.add_worksheet("ERA案場區間用電數報表(%s ~ %s)" % (self.start_date,self.end_date))
