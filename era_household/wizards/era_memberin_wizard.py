...
class EraMemberInWizard(models.TransientModel):
    _name = "era_household.member_in_wizard"
    ...
    member_id = fields.Many2one('era.household_member', string="新租戶")
    ...
    def run_member_in(self):
        self.env.cr.execute("""update era_household_house_line set member_id=%d,user_id=%d,in_used=True where id=%d""" % (self.member_id.id,self.user_id.id,self.house_id.id))
        self.env.cr.execute("""commit""")
        self.env.cr.execute("""update era_household_member set user_id=%d,house_id=%d where id=%d""" % (self.user_id.id,self.house_id.id,self.member_id.id))
        self.env.cr.execute("""commit""")
