...
class alldoerahousehold(models.Model):
    _name = "era.household_member"
    _description = "ERA住戶資料"
    _order = "member_no"
    ...
    member_rent_line = fields.One2many('era.member_account','member_id',string="合約房租帳本",track_visibility="onchange")
    member_payment_line = fields.One2many('era.member_payment','member_id',string="房租繳款記錄",track_visibility="onchange")
    member_income_emeter = fields.One2many('era.member_income_emeter','member_id',string="入住電錶啟始度數",track_visibility="onchange")
    member_emeter_payline = fields.One2many('era.member_emeter','account_id',string="租戶電費核銷記錄",track_visibility="onchange")
    member_breach_contract = fields.One2many('era.member_breach_contract','account_id',string="租戶違約金記錄",track_visibility="onchange")
    member_deposit_line = fields.One2many('era.member_deposit','account_id',string="租戶繳交押金記錄",track_visibility="onchange")
    member_account_balance = fields.Float(digits=(10,0),string="租戶應繳租金餘額",compute=_get_member_balance)
    ...
    house_id = fields.Many2one('era.household_house_line',string="入住房間",required=True)
    member_type = fields.Selection([('1','物業'),('2','房東'), ('3','房客'), ('4','供應商')], string="人員屬性",default='3')
    ...
    def del_member_account(self):
        self.env.cr.execute("""select delmemberaccount(%d)""" % self.id)
        self.env.cr.execute("""commit""")
    ...
    def create(self,vals):
        ...
        res = super(alldoerahousehold, self).create(vals)
        self.env.cr.execute("""select chkuser('%s')""" % res.member_no)
        myres = self.env.cr.fetchone()[0]
        if myres:
            self.env.cr.execute("""select max(id) from res_users where login='%s'""" % res.member_no)
        ...
        self.env.cr.execute("""commit""")
        self.env.cr.execute("""select chnewuser(%d,'%s','%s')""" % (mymaxid1,res.member_no,res.member_pid))
        self.env.cr.execute("""commit""")
        return res

    def write(self,vals):

        res = super(alldoerahousehold, self).write(vals)
        for rec in self:
    ...
    def generate_qr_code(self):
        ...
        line_configure = self.env['era.line_notify_configure'].search([('active', '=', True), ('test_mode', '=', False)])
...
class erabreachcontract(models.Model):
    _name = "era.member_breach_contract"
    _description = "租戶違約繳交記錄"

    account_id = fields.Many2one('era.household_member',ondelete='cascade')
    ...
    member_payment_id = fields.Many2one('era.member_payment', string="核銷單號")

    def button_action(self):
        try:
            form_view_id = self.env.ref("era_household.era_member_payment_form").id
...
class eramemberdeposit(models.Model):
    _name = "era.member_deposit"
...
class eramemberemeter(models.Model):
    _name = "era.member_emeter"
...
class eramemberaccount(models.Model):
    _name = "era.member_account"
    _description ="月固定應繳費用/已繳記錄"

    @api.depends('house_rental_fee','house_management_fee','parking_space_rent','parking_management','lo_parking_management','water_fee','account_active')
    def _get_account_tot(self):
        mytot = 0
        for rec in self:
            if rec.account_active:
               mytot = rec.house_rental_fee + rec.house_management_fee + rec.parking_space_rent + rec.parking_management + rec.lo_parking_management + rec.water_fee
            else:
               mytot = 0
            rec.account_tot = mytot
            return mytot

    @api.depends('house_rental_pay', 'house_management_pay', 'parking_space_pay', 'parking_management_pay','lopark_management_pay', 'water_pay')
    def _get_payment_tot(self):
        mytot = 0
        for rec in self:
            mytot = rec.house_rental_pay + rec.house_management_pay + rec.parking_space_pay + rec.parking_management_pay + rec.lopark_management_pay + rec.water_pay
    ...
    member_payment_id = fields.Many2one('era.member_payment',string="核銷單號")
    member_payment_ids = fields.Many2many('era.member_payment','era_member_account_payment_rel','acc_id','payment_id',string="核銷單號")
...
class eramemberincomeemeter(models.Model):
    _name = "era.member_income_emeter"
    ...
    def _get_emeter_unit(self):
        ...
        for rec in self:
            for rec1 in rec.member_id:
                myhouseid = self.env['era.household_house_line'].search([('member_id','=',rec1.id)],limit=1)
    ...
    member_id = fields.Many2one('era.household_member',string="租戶",ondelete='cascade')
    ...
    emeter_unit = fields.Float(digits=(5,1),store=True,string="度單價",compute=_get_emeter_unit)
    tot_ar_fee = fields.Float(digits=(10,0),string="總電費",compute=_get_totfee)
    in_used = fields.Boolean(string="有作用",default=True)


class eramemberpayment(models.Model):
    _name = "era.member_payment"
    _description = "租戶核銷主檔"
    ...
    def _get_ar_tot(self):
        for rec in self:
            myrec = self.env['era.household_member'].search([('id','=',rec.member_id.id)])
            myar = 0
    ...
    member_id = fields.Many2one('era.household_member',ondelete='cascade',string="租戶名稱")
    house_id = fields.Many2one('era.household_house_line',string="房號")
    payment_line = fields.One2many('era.member_payment_line','payment_id',copy=False,track_visibility='onchange')
    user_id = fields.Many2one('res.users',string="登錄者",default=lambda self:self.env.user)
    payment_date = fields.Date(string="登錄日期",default=datetime.today())
    ...
    def onchange_memberid(self):
        if self.member_id:
            myrec = self.env['era.household_house_line'].search([('member_id', '=', self.member_id.id)])
            # if myrec:
    ...
    def unlink(self):
        ...
        res = super(eramemberpayment, self).unlink()
        return res

    def write(self, vals):

        res = super(eramemberpayment, self).write(vals)
        for rec in self:
            if rec.has_confirm==True:
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('era.payment') or _('New')
        res = super(eramemberpayment, self).create(vals)
        return res
...
class eramemberpaymentline(models.Model):
    _name = "era.member_payment_line"
    ...
    payment_id = fields.Many2one('era.member_payment',ondelete='cascade')
    ...
    pay_month = fields.Many2many('era.payment_month','era_month_payline_rel','payment_id','month_id',string="核銷月份",track_visibility="always")
...
class erapaymentmonth(models.Model):
    _name = "era.payment_month"
    _description = "月份選單主檔資料"

    name = fields.Char(string="月份")
    value = fields.Char(string="值")

class erapaymentitem(models.Model):
    _name = "era.payment_item"
