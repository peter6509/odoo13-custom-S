...
class newebstockpickinginherit(models.Model):
    ...
    def stock_copy(self):
        if self.stockin_picktype=='outgoing':   # 出貨
            ...
            if mycount > 0 :
                myres = myrec.copy()
                # self.env.cr.execute("""select gen_copy_stockout1('%s',%d)""" % (self.stockout_proj_no,myres.id))
                # self.env.cr.execute("""commit""")
        ...
        elif self.stockin_picktype=='incoming': # 進貨
            b=1

    # def copy(self, default=None):
    #     default['is_proj_main']=False
    #     res = super(newebstockpickinginherit, self).copy()
    #     return res
