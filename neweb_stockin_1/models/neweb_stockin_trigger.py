...
class newebstockintrigger(models.Model):
    ...
    def init(self):
        ...
        self._cr.execute("""drop trigger if exists delete_on_stockin_list on neweb_stockin_list ;""")
        ...
        self._cr.execute("""drop function if exists before_delete_stock_picking() cascade""")
        self._cr.execute("""create  or replace function before_delete_stock_picking() returns trigger as $BODY$
          DECLARE
             ncount int ;
          BEGIN
             if NEW.picking_type_id in (select id from stock_picking_type where sequence_code='OUT') then
                 select count(*) into ncount from neweb_stockoutno_line where name=NEW.id ;
                 if ncount > 0 then
                    delete from neweb_stockoutno_line where name=NEW.id ;
                 end if ;
             end if ;    
             return NEW ;
          END;$BODY$
        ...
        self._cr.execute("""drop trigger if exists before_delete_stock_picking on stock_picking ;""")
        self._cr.execute("""create trigger before_delete_stock_picking before delete on stock_picking
                                        for each row execute procedure before_delete_stock_picking();""")
