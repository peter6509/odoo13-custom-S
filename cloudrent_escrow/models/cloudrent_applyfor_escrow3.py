...
class CloudRentApplyforEscrow3(models.Model):
    _name = "cloudrent.applyfor_escrow3"
    _description = "民眾(房客)承租住宅申請書(代管)"
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            self.env.cr.execute("""select get_cloudrent_seqno(%d,'%s')""" % (vals['escrow_no'], 'A4'))
            vals['name'] = self.env.cr.fetchone()[0]
        res = super(CloudRentApplyforEscrow3, self).create(vals)
        return res
