...
class CloudRentEntrustManagementContract(models.Model):
    _name = "cloudrent.entrust_management_contract"
    _decription = "社會住宅代租代管委託管理契約書(entrust2)"
    ...
    entrust_start_date = fields.Date(string="委託管理起始日")
    entrust_end_date = fields.Date(string="委託管理截止日")
    build_for_rent = fields.Integer(string="待租租金(元/月)")
    collection_rent = fields.Boolean(string="代收租金")
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            self.env.cr.execute("""select get_cloudrent_seqno(%d,'%s')""" % (vals['escrow_no'], 'C2'))
            vals['name'] = self.env.cr.fetchone()[0]
        res = super(CloudRentEntrustManagementContract, self).create(vals)
        return res
