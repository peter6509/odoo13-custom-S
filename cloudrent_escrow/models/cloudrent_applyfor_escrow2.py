...
class CloudRentApplyforEscrow2(models.Model):
    _name = "cloudrent.applyfor_escrow2"
    _description = "出租人出租住宅申請書(包租)"
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            self.env.cr.execute("""select get_cloudrent_seqno(%d,'%s')""" % (vals['escrow_no'], 'A3'))
            vals['name'] = self.env.cr.fetchone()[0]
        res = super(CloudRentApplyforEscrow2, self).create(vals)
        return res