...
class iplaaccountmoveinherit(models.Model):
    _inherit = "account.move"

    taiwan_receipt = fields.Char(string="發票號碼")
    report_no = fields.Char(string="對帳單號")
    receive_check = fields.Boolean(string="支票收付？")
    check_duedate = fields.Date(string="支票到期日")

class iplaaccountmoveseq(models.Model):
    _name = "alldo_ipla_iot.account_seq"
...
class iplaaccountmoveselectitem(models.Model):
    _name = "alldo_ipla_iot.accountmove_selectitem"
    _description = "對帳選單主檔"
    ...
    account_move_line = fields.One2many('alldo_ipla_iot.accountmove_selectitem_line','move_id',string="對帳明細")

    def _write(self, vals):

        res = super(iplaaccountmoveselectitem, self)._write(vals)
        for rec in self:
...
class iplaaccountmoveselectitemline(models.Model):
    _name = "alldo_ipla_iot.accountmove_selectitem_line"
    _description = "對帳選單明細檔"

    move_id = fields.Many2one('alldo_ipla_iot.accountmove_selectitem',ondelete='cascade')
