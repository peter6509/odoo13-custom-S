...
class iplaiotbells(models.Model):
    _name = "alldo_ipla_iot.bells_set"

    name = fields.Char(string="名稱",required=True)
    bells_ip = fields.Char(string="IP ADDRESS",required=True)
    bells_line = fields.One2many('alldo_ipla_iot.bells_line','bells_id',string="bells line")

    @api.model
    def create(self, vals):
        mycount = self.env['alldo_ipla_iot.bells_set'].search_count([])
        if mycount > 0 :
            raise UserError("只能設定一筆鐘聲記錄")
        res = super(iplaiotbells, self).create(vals)
        return res
    ...
    def run_mp3_checktime(self):
        myrec = self.env['alldo_ipla_iot.bells_set'].search([])
...
class iplaiotbellsline(models.Model):
    _name = "alldo_ipla_iot.bells_line"

    bells_id = fields.Many2one('alldo_ipla_iot.bells_set',ondelete='cascade')
