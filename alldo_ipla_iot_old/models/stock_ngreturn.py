...
class stockngreturndata(models.Model):
    _name = "alldo_ipla_iot.ngreturn_report"
    ...
    report_line = fields.One2many('alldo_ipla_iot.ngreturn_report_line','rep_id',copy=False)
...
class stockngreturnline(models.Model):
    _name = "alldo_ipla_iot.ngreturn_report_line"

    rep_id = fields.Many2one('alldo_ipla_iot.ngreturn_report',ondelete='cascade')
...
class iplareturnselect(models.Model):
    _name = "alldo_ipla_iot.ngreturn_select"
    _description = "NG退回數據選單主檔"
    _rec_name = "report_date"

    report_date = fields.Date(string="印表日期",default=fields.Date.today())
    partner_id = fields.Many2one('res.partner',string="客戶")
    select_line = fields.One2many('alldo_ipla_iot.ngreturn_selectitem','select_id')

    def run_gen_ngreturn(self):
        self.env.cr.execute("""delete from alldo_ipla_iot_ngreturn_report""")
        self.env.cr.execute("""commit""")
        myrec = self.env['alldo_gh_iot.ngreturn_report']
        myname = self.env['ir.sequence'].next_by_code('alldo_ipla_iot.return_ng')
        ...
        myrec = self.env['alldo_ipla_iot.ngreturn_report'].search([])
        for rec in myrec:
            myid = rec.id

        myviewid = self.env.ref('alldo_ipla_iot.ngreturn_report_action')
        return {'view_name': 'ngreturn_report_action',
                'name': (u'NG return Data'),
                'views': [[False, 'form']],
                'res_model': 'alldo_ipla_iot.ngreturn_report',
...
class iplangreturnselectitem(models.Model):
    _name = "alldo_ipla_iot.ngreturn_selectitem"
    _description = "NG退回數據選單明細"
    _order = "product_no,qc_date desc"

    select_id = fields.Many2one('alldo_ipla_iot.ngreturn_select',ondelete='cascade')
