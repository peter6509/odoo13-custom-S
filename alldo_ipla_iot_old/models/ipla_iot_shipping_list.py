...
class iplaiotprocessingview(models.Model):
    _name = "alldo_ipla_iot.processing_view"
    _description = "工單生產良品查詢主檔"
    _rec_name = "mo_no"

    mo_no = fields.Many2one('alldo_ipla_iot.workorder',string="工單號碼")
    ...
    processing_line = fields.One2many('alldo_ipla_iot.processing_view_line','processing_id')


class iplaiotprocessingviewline(models.Model):
    _name = "alldo_ipla_iot.processing_view_line"
    _description = "工單生產良品查詢明細檔"

    processing_id = fields.Many2one('alldo_ipla_iot.processing_view',ondelete='cascade')
