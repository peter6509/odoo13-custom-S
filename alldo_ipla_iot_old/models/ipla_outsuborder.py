...
class alldoiplaiotoutsuborder(models.Model):
    _name = "alldo_ipla_iot.outsuborder"
    _description = "委外加工單"
    _order = "name desc"
    ...
    qc_line = fields.One2many('alldo_ipla_iot.outsuborder_qc','order_id',string="QC明細",copy=False)
    prodout_line = fields.One2many('alldo_ipla_iot.outsuborder_prodout','order_id',string="委外給料記錄",copy=False)
    prodin_line = fields.One2many('alldo_ipla_iot.outsuborder_prodin','order_id',string="委外入庫記錄",copy=False)
    ...
    def run_archive(self):
        for rec in self:
            if rec.state=='3':
                if rec.active==True:
                    self.env.cr.execute("""update alldo_ipla_iot_outsuborder set active=False where mo_group_id=%d""" % rec.mo_group_id)
                else:
                    self.env.cr.execute("""update alldo_ipla_iot_outsuborder set active=True where mo_group_id=%d""" % rec.mo_group_id)
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('alldo_ipla_iot.outsuborder') or _('New')
        res = super(alldoiplaiotoutsuborder, self).create(vals)
    ...
    def _write(self, vals):
        res = super(alldoiplaiotoutsuborder, self)._write(vals)
        for rec in self:
...
class alldoiplaiotoutsuborderqc(models.Model):
    _name = "alldo_ipla_iot.outsuborder_qc"
    _description = "委外加工加工質檢"
    _order = "order_id,qc_date"

    order_id = fields.Many2one('alldo_ipla_iot.outsuborder', ondelete='cascade')
...
class alldoiplaiotoutsuborderprodin(models.Model):
    _name = "alldo_ipla_iot.outsuborder_prodin"
...
class alldoiplaiotoutsuborderprodout(models.Model):
    _name = "alldo_ipla_iot.outsuborder_prodout"
