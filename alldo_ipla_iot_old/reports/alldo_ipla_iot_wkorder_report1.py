...
class ReportSaleOrder1(models.AbstractModel):
    _name = 'report.alldo_ipla_iot.alldo_ipla_iot_wkorder_report1'

    @api.model
    def _get_report_values(self, docids, data=None):
        # print(docids)
        self.env.cr.execute("""delete from alldo_ipla_iot_wkorder_selectitem1""")
        self.env.cr.execute("""commit""")
        self.env.cr.execute("""insert into alldo_ipla_iot_wkorder_selectitem1(report_owner) values (%d)""" % self.env.uid)
        self.env.cr.execute("""commit""")
        wkrec=[]
        for docid in docids:
        ...
        self.env.cr.execute("""commit""")
        docs = self.env['alldo_ipla_iot.wkorder_printsheet1'].search([])
        #print(docs)
        res_doc = []
        for line in docs:
        ...
        docargs = {
            'doc_ids': docids,
            'doc_model': 'alldo_ipla_iot.wkorder_printsheet1',
            'docs': res_doc,
        }
        return docargs
