...
class iplaiotstockmovemixsearch(models.TransientModel):
    _name ="alldo_ipla_iot.stockmove_mixsearch"
    ...
    def run_stockmove_mixsearch(self):
        ...
        myviewid = self.env.ref('alldo_ipla_iot.iplaiot_stockmove_tree')
        return {
            'view_name': 'iplaiot_stockmove_tree',
            'name': (u'產品出貨複合數據'),
            'type': 'ir.actions.act_window',
            'res_model': 'alldo_ipla_iot.stock_move_list',
            'view_id': myviewid.id,
            'view_type': 'form',
            'view_mode': 'tree',
            'target': 'list'}
