...
class alldoiplaiotwkperformancewizard(models.TransientModel):
    _name = "alldo_ipla_iot.wkorder_performance_wizard"
    ...
    def run_wk_performance(self):
        ...
        mywkrec = self.env['alldo_ipla_iot.workorder_performance_list1'].search([])
        mycount = self.env['alldo_ipla_iot.workorder_performance_list1'].search_count([])
        output = io.BytesIO()
        if not self.iot_owner:
