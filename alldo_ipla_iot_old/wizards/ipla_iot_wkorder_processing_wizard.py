...
class wkorderprocessingwizard(models.TransientModel):
    _name = "alldo_ipla_iot.wk_processing_wizard"

    product_no = fields.Many2one('product.product',string="產品",required=True)
    mo_no = fields.Many2one('alldo_ipla_iot.workorder',string="工單號碼",required=True)

    @api.onchange('product_no')
    def onchangeprodno(self):
        myrec = self.env['alldo_ipla_iot.workorder'].search([('product_no','=',self.product_no.id),('state','!=','4')])
    ...
    def run_genwkprocessing(self):
        ...
        myrec = self.env['alldo_ipla_iot.processing_view'].search([])
        myid = myrec[0].id
        myviewid = self.env.ref('alldo_ipla_iot.iplaiot_processing_view_form')
        return {'view_name': 'wkorder_processing',
                'name': (u'wkordr processing Data'),
                'views': [[False, 'form']],
                'res_model': 'alldo_ipla_iot.processing_view',
