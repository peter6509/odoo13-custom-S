...
class empattendanceexportwizard(models.TransientModel):
    ...
    def run_attendance_export(self):
        ...
        ws={}
        for rec in emprec:
            myattendrec = self.env['alldo_ipla_iot.emp_attendance_list'].search([('emp_no','=',rec.name)],order="emp_no,attend_date")
            # mycount = self.env['alldo_ipla_iot.emp_attendance_list'].search_count([('emp_no','=',rec.name)])
            ########   worksheet create
            ...
            for line in myattendrec:
                ...
                ws[nnum].write(row, 3, line.attendance_start if line.attendance_start else ' ', okl_content_format)
                ws[nnum].write(row, 4, line.attendance_end if line.attendance_end else ' ', okl_content_format)
                ws[nnum].write(row, 5, line.att_duration if line.att_duration else '0', okc_content_format)
                ws[nnum].write(row, 6, line.otattendance_start if line.otattendance_start else ' ', okl_content_format)
                ws[nnum].write(row, 7, line.otattendance_end if line.otattendance_end else ' ', okl_content_format)
                ...
                if line.att1_duration > 0 :
                    ...
                    ws[nnum].write(row, 1, line.emp_no.emp_code if line.emp_no else ' ', okl_content_format)
                    ws[nnum].write(row, 2, line.emp_no.name if line.emp_no else ' ', okl_content_format)
                    ws[nnum].write(row, 3, line.attendance_start1 if line.attendance_start1 else ' ', okl_content_format)
                    ws[nnum].write(row, 4, line.attendance_end1 if line.attendance_end1 else ' ', okl_content_format)
                ...
                if line.att2_duration > 0 :
                    ...
                    ws[nnum].write(row, 1, line.emp_no.emp_code if line.emp_no else ' ', okl_content_format)
                    ws[nnum].write(row, 2, line.emp_no.name if line.emp_no else ' ', okl_content_format)
                    ws[nnum].write(row, 3, line.attendance_start2 if line.attendance_start2 else ' ', okl_content_format)
                    ws[nnum].write(row, 4, line.attendance_end2 if line.attendance_end2 else ' ', okl_content_format)
