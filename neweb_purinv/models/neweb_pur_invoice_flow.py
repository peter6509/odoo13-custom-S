...
class NewebPurInvoiceFlow(models.Model):
    _inherit = "neweb_purinv.invoice"

    @api.depends('create_uid')
    def _get_pos_type(self):
        for rec in self:
            myrec = self.env['res.users'].search([('id','=',rec.create_uid.id)])
    ...
    pos_type = fields.Char(string='簽核判斷字元',store=True,compute=_get_pos_type)
    ...
    def create(self, vals):
        res = super(NewebPurInvoiceFlow, self).create(vals)
        self.env.cr.execute("""select genpurinvflowman(%d)""" % res.id)
    ...
    def write(self, vals):
        if 'x_wkf_state' in vals:
           ...
           myflow = False
        res = super(NewebPurInvoiceFlow, self).write(vals)
        for rec in self:
            self.env.cr.execute("""select genpurinvflowman(%d)""" % rec.id)
            self.env.cr.execute("""commit""")
    ...
    # def RFQ_start(self):
    #     self.env.cr.execute("""update neweb_require_purchase set write_lock=True where id=%d""" % self.id)
    #     self.env.cr.execute("""commit""")
    #
    # def RFQ_email_1_line(self):
    #     self.env.cr.execute("""update neweb_require_purchase set write_lock=True where id=%d""" % self.id)
    #     self.env.cr.execute("""commit""")
    ...
    #                 'order_url': base_url
    #             })
    #             template = self.env.ref('neweb_purchase.RFQ_flow_message')
    #             template.with_context(local_context).send_mail(self.id, force_send=True)
    #
    # def RFQ_email_2_line(self):
    #     self.env.cr.execute("""update neweb_require_purchase set write_lock=True where id=%d""" % self.id)
    #     self.env.cr.execute("""commit""")
    #     base_url = request.env['ir.config_parameter'].sudo().get_param('web.bf.url')
    #     base_url += '/web#id=%d&view_type=form&model=%s' % (self.id, self._name)
    ...
    #             template.with_context(local_context).send_mail(self.id, force_send=True)
    #
    # def RFQ_email_1_reject(self):
    #     self.env.cr.execute("""update neweb_require_purchase set write_lock=False where id=%d""" % self.id)
    #     self.env.cr.execute("""commit""")
    #     base_url = request.env['ir.config_parameter'].sudo().get_param('web.bf.url')
    #     base_url += '/web#id=%d&view_type=form&model=%s' % (self.id, self._name)
    #     for rec in self:
    ...
    #                 'order_url': base_url
    #             })
    #             template = self.env.ref('neweb_purchase.RFQ_flow_message_reject')
    #             template.with_context(local_context).send_mail(self.id, force_send=True)
    #
    # def RFQ_email_2_reject(self):
    #     self.env.cr.execute("""update neweb_require_purchase set write_lock=False where id=%d""" % self.id)
    #     self.env.cr.execute("""commit""")
    #     base_url = request.env['ir.config_parameter'].sudo().get_param('web.bf.url')
    #     base_url += '/web#id=%d&view_type=form&model=%s' % (self.id, self._name)
    ...
    #                 'order_url': base_url
    #             })
    #             template = self.env.ref('neweb_purchase.RFQ_flow_message_complete')
    #             template.with_context(local_context).send_mail(self.id, force_send=True)
    #
