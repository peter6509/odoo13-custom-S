...
class cloudrentSendMessageWizard(models.TransientModel):
    _name = "cloudrent.manual_send_line_message"
    _description = "手動發布通知訊息精靈"

    project_no = fields.Many2one('cloudrent.household_house', string="案場",required=True)
    member_id = fields.Many2many('cloudrent.household_member','cloudrent_manual_line_member_rel','message_id','member_id', string="租戶")
    ...
    def onchangeprojectno(self):
        myrec = self.env['cloudrent.household_member'].search([('house_id.house_id','=',self.project_no.id)])
        myids = []
        for rec in myrec:
            myids.append(rec.id)
