...
class cloudrentSendLineMessage(models.Model):
    _name = "cloudrent.send_line_message"
    _description = "cloudrent發送公告通知執行表"
    ...
    project_no = fields.Many2one('cloudrent.household_house',string="案場")
    ...
    def check_balance(self,memberid):
        user = self.env['cloudrent.household_member'].search([('id','=',memberid.id)])
        amount = [
    ...
    def push_ar_message(self,memberid):
        user = self.env['cloudrent.household_member'].sudo().search([('id', '=', memberid.id)])
        amount = [
            user.period_start if user.period_start else ' ',
            user.period_end if user.period_end else ' ',
            user.now_ym if user.now_ym else ' ',
    ...
    def run_send_line_message(self):  # 每10分鐘 cron job 將 到發送日期 LINE MESSAGE 執行發送
        #line_bot_api = LineBotApi('DB9ZYXsCQR1tzirulv6RnIALVKj0yC/ypDyy3/ycnp7c8FezbYUAQCdrLpyD03k1CEhmWhw+ZHFv5AaUkXLtZvHGfUaLgzk06fyIRB/66tfDaITyhEYf0C9C0tcUpHPs1OBTyVu9sWjXQdKANUKvIAdB04t89/1O/w1cDnyilFU=')
        line_bot_api = LineBotApi(params('access_token'))
        self.env.cr.execute("""select ckcando()""")
        self.env.cr.execute("""commit""")
        myrec = self.env['cloudrent.send_line_message'].sudo().search([('send_status','=',False),('can_do','=',True)])
        for rec in myrec:
