...
class cloudrentHouseholdMember(models.Model):
    ...
    line_request_status = fields.Selection(
        selection=[('', '無'), ('notify', '繳費通知'),('service','修繕通知')]
    )
    ...
    history_ids = fields.One2many('cloudrent.household_member.history', 'household_member_id')
    linelog_ids = fields.One2many('cloudrent.linelog','member_id')
