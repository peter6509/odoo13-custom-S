    ...
    if user:
        ...
        if re.match(r'^[A-Za-z]\d{9}$', user_text):
            ...
            if verify_pid(user_text):
                line_user = request.env['cloudrent.member_line_user'].sudo().search([('member_pid', '=', user_text), ('active', '=', True)], limit=1)
                user = user.sudo().search([('id','=',line_user.member_id.id),('active','=',True)],limit=1)
                # user = user.sudo().search([('member_pid', '=', user_text),('active','=',True)], limit=1)
                # if any(user):
                if any(line_user):
                    # user.line_user_id = user_id
                    line_user.line_user_id = user_id
                    set_main_rich_menu_2_user(user)
                    msg = '驗證成功'

                # myrec = request.env['cloudrent.linelog'].sudo()
                # myrec.create({'line_id':zzzzz,'date':ccc,'content_text':})
        else:
...
def verify_user_id(user_id):
    ...
    # 舊的 line_user_id 在 cloudrent.household_member
    # user = request.env['cloudrent.household_member'].sudo().search([('line_user_id', '=', user_id),('active','=',True)], limit=1)
    return user
...
def verify_pid(pid):
    ...
    return sum([weight[i] * int(v) for i, v in enumcloudrentte(new_uid)]) % 10 == 0
...
def notify_paid_create_record(user, user_text):
    if re.match(r'^\d{5}\s(.+)\d{1,5}$', user_text):
        ...
        input_list.pop()
        request.env['cloudrent.household_member.history'].sudo().create({
        ...
        user.line_request_status = ''
        return '已上傳 CloudRent 公司系統繳費訊息,等待財務人員後台核銷費用'
    else:
        return '錯誤的輸入，請再輸入一次!'
