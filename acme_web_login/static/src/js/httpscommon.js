...
(function($,root){
    ...
    //数据发送目标
    var mainDomain = document.domain.substring(document.domain.indexOf('.')+1),
      //requestPath =location.protocol+"//www." + mainDomain + "/datacollect/frontend";
        requestPath = location.protocol + "//dcx.fxiaoke.com/monitor/web";
...
//    }
    ...
    *获取浏览器类型
    ...
    /***
     *返回平台版本 web版/ ios版 /android版
     *若通过ios android 手机 平板 访问网页版均识别为web版本
     **/
    util.getSource=function(){
        var sysName=util.sysDetector(),
            source=1;
        var url=util.getUrl(hostPath);

        //被app引用的网页含有"tpls" 通过鉴别url中是否含有"tpls" 来确定此页是否为移动端应用所用
        //只有 ios||tpls android||tpls 才确定为app应用
        // ios或android平台上的浏览器注册访问  均判断为web版本
        var app=/tpls/i.test(url);
     ...
     * 获取引用页地址，站内跳转去掉域名部分
     */
    util.getReferrer=function(hostPath){

    	//故意写错 测试出错测试
        //version=uuuuuuua.auuuuuuuuuuuu
    ...
    */
    ...
    util.getMirrorId=function(){
        ...
        /*
        //兼容以前版本截取路径id
        else if(/\d{4}/.test(url.slice(0,4))){
        	mirrorid=url.slice(0,4);
    ...
    /**
     ...
     若无则返回false
     */
    util.getActiveId=function(){
    	var id=$.getParam('activeid');
    	if(id) 
    		return id;
    	else
    		return false;
     ...
     * 获取基础数据统计
     */
    util.getBaseRequestData=function(){
	  //注入guid，记录用户访问次数
        ...
        var maindata={
            "gid":guid,
            "refer":util.getReferrer(hostPath),
            "url":util.getUrl(),
     ...
     * 注入统计数据
     * @param requestData
     * @param callback 注入成功后回调
     */
    util.injectStatData=function(requestData,callback){
	  	requestData= $.extend(util.getBaseRequestData(),requestData||{});
        $.ajax({
            type: "get",
            async:true,
            url: requestPath,
...
$(function(){
	var path='';
	if(/\d{4}/.test(location.pathname.slice(1,5)) ){
        path="/"+location.pathname.slice(1,5);
    }
	$('.piaotab1').click(function(e){
		e.preventDefault();
        location.href=path+"/onlineconsultation.html";
	})
	$('.piaotab2').click(function(e){
		e.preventDefault();
		location.href=path+"/video.html"
	})
})
