    ...
    if user:
        ...
        if re.match(r'^[A-Za-z]\d{9}$', user_text):
            ...
            if verify_pid(user_text):
                line_user = request.env['era.member_line_user'].sudo().search([('member_pid', '=', user_text),('active', '=', True)], limit=1)
...
def verify_user_id(user_id):
    ...
    :return:
    """
    line_user = request.env['era.member_line_user'].sudo().search([('line_user_id','=',user_id),('active','=',True)],limit=1)
    user = request.env['era.household_member'].sudo().search([('id','=',line_user.member_id.id),('active','=',True)],limit=1)
    # 舊的 line_user_id 在 era.household_member
    # user = request.env['era.household_member'].sudo().search([('line_user_id', '=', user_id),('active','=',True)], limit=1)
    return user
