...
class acmeiotempinfo(models.Model):
   _name = "alldo_acme_iot.empinfo"
   _rec_name= "empinfo_date"

   empinfo_date = fields.Date(string="製作日期")
   barcode_line = fields.One2many('alldo_acme_iot.empbarcode','barcode_id',copy=False)


class acmeiotempbarcode(models.Model):
   _name = "alldo_acme_iot.empbarcode"
   _description = "人員卡片產生檔"


   barcode_id = fields.Many2one('alldo_acme_iot.empinfo',ondelete='cascade')
