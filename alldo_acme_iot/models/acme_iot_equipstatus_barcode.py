...
class acmeiotequipstatusinfo(models.Model):
   _name = "alldo_acme_iot.equipstatusinfo"
   _rec_name= "print_num"

   print_num = fields.Integer(string="製作份數")
   barcode_line = fields.One2many('alldo_acme_iot.equipstatusbarcode','barcode_id',copy=False)


class acmeiotequipstatusbarcode(models.Model):
   _name = "alldo_acme_iot.equipstatusbarcode"
   _description = "設備異常條碼生檔"

   barcode_id = fields.Many2one('alldo_acme_iot.equipstatusinfo',ondelete='cascade')
