...
class quantreport(models.Model):
    _name = "alldo_acme_iot.quant_report"
    _description = "批次號主檔"

    report_owner = fields.Many2one('res.users',string="製表人",default=lambda self:self.env.uid)
    quant_line = fields.One2many('alldo_acme_iot.quant_report_line','quant_id',string="條碼列印檔",copy=False)


class quantreportline(models.Model):
    _name = "alldo_acme_iot.quant_report_line"
    _description = "批次號條碼列印檔"

    quant_id = fields.Many2one('alldo_acme_iot.quant_report',ondelete='cascade')
