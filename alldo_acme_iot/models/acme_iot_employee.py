...
class alldoacmeiotempinerit(models.Model):
    _inherit = "hr.employee"

    emp_code = fields.Char(string="員工編號")
    attendance_line = fields.One2many('alldo_acme_iot.emp_attendance','attendance_id',string="出勤明細")
    duty_type = fields.Selection([('1',u'上班'),('2',u'下班')],string=u"出勤狀態",default='2')
    ...
    def create(self, vals):
        ...
        res = super(alldoacmeiotempinerit, self).create(vals)
        return res

    def _write(self, vals):
        res = super(alldoacmeiotempinerit, self)._write(vals)
        for rec in self:
...
class alldoacmeiotempattendance(models.Model):
    _name = "alldo_acme_iot.emp_attendance"
    ...
    attendance_type = fields.Selection([('1','上班'),('2','下班'),('3','加班上班'),('4','加班下班')],string="刷卡類別")
    ...
    def _get_attendancename(self):
        for rec in self:
            if rec.attendance_type=='1' :
                rec.attendance_name='上班'
            elif rec.attendance_type=='2':
                rec.attendance_name='下班'
