...
class acmemrpproductioninherit(models.Model):
    _inherit = "mrp.production"
    _order = "create_date desc,name asc"

    acme_wk_line = fields.One2many('alldo_acme_iot.workorder','mrp_prod_id',string="製造命令工單")
    acme_sub_line = fields.One2many('alldo_acme_iot.outsuborder','mrp_prod_id',string="製造命令工單-委外")
    acme_material_line = fields.One2many('alldo_acme_iot.materialline','mrp_prod_id',string="製造命令耗料檢核表")
    ...
    def action_assign(self):
        super(acmemrpproductioninherit, self).action_assign()
        for rec in self:
            self.env.cr.execute("""select gen_mrp_materialline(%d)""" % rec.id)
            self.env.cr.execute("""commit""")
            self.env.cr.execute("""select resetproductionnum(%d)""" % rec.id)
            self.env.cr.execute("""commit""")


    def action_confirm(self):
        super(acmemrpproductioninherit, self).action_confirm()
        for rec in self:
    ...
    def open_produce_product(self):
        ...
        for rec in self:
            ...
            myworkorderrec = self.env['alldo_acme_iot.workorder']
            ...
            for rec1 in myrec.eng_line:
                ...
                if myres == '1':
                    ...
                    if rec1.is_outsourcing:
                        ...
                        if not rec1.eng_type:
                            myengtype1 = ' '
                        else:
                            myengtype1 = rec1.eng_type
                        if not rec1.eng_desc:
                            myengdesc = ' '
                        else:
                            myengdesc = rec1.eng_desc
                        myengtype =myengtype1 + '-' + myengdesc
    ...
    def button_mark_done(self):
        self.env.cr.commit()
        self.env.cr.execute("""select changequant1(%d)""" % self.id)
        self.env.cr.execute("""commit""")
        self.ensure_one()
        self._check_company()
