...
class stockoutsourcingdata(models.Model):
    _name = "alldo_acme_iot.outsourcing_report"
    ...
    report_line = fields.One2many('alldo_acme_iot.outsourcing_report_line','rep_id',copy=False)
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('alldo_acme_iot.outsourcing_out') or _('New')
        res = super(stockoutsourcingdata, self).create(vals)
        return res


class stockoutsourcingline(models.Model):
    _name = "alldo_acme_iot.outsourcing_report_line"

    rep_id = fields.Many2one('alldo_acme_iot.outsourcing_report',ondelete='cascade')
