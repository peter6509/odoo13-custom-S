...
class acmeiotcompanystockloc(models.Model):
    _name = "alldo_acme_iot.company_stockloc"
    ...
    balance_loc = fields.Many2one('stock.location',string="原料批號餘額倉")

    @api.model
    def create(self, vals):
        mycount = self.env['alldo_acme_iot.company_stockloc'].search_count([])
        if mycount > 0 :
            raise UserError("公司倉庫配置只能存在單筆")
        res = super(acmeiotcompanystockloc, self).create(vals)
        return res
