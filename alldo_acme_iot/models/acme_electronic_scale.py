...
class acmeelectronicscale(models.Model):
    _name = "alldo_acme_iot.electronic_scale"
    ...
    def _get_lot(self):
        for rec in self:
            self.env.cr.execute("""select genlotid(%d)""" % rec.product_no.id)
    ...
    def run_scale_move(self):  ## odoo schedule action  每5分鐘一次
        ## 熔爐混合料號
        myfurnaceprodid = self.env['alldo_acme_iot.company_stockloc'].search([])[0].furnace_prod_id.id
        ## 熔爐位置
        myfurnacelocid = self.env['alldo_acme_iot.company_stockloc'].search([])[0].furnace_loc.id
        ## 原物料位置
        mymateriallocid = self.env['alldo_acme_iot.company_stockloc'].search([])[0].material_loc.id
        mymolocid = self.env['alldo_acme_iot.company_stockloc'].search([])[0].mo_loc.id
        mypickingrec = self.env['stock.picking'].search([])
        myrec = self.env['alldo_acme_iot.electronic_scale'].search([('picking_no','=',False)])
        for rec in myrec:
            if rec.scale_type=='1':  # 熔爐投料
                if rec.product_no.product_tmpl_id.tracking=='lot' and not rec.lot_no :
                    ...
                    self.env.cr.execute("""update alldo_acme_iot_electronic_scale set is_posting='3' where id=%d""" % rec.id)
                    ...
                    if not rec.lot_no:
                        try:
                            ...
                            myres.action_done()
                            self.env.cr.commit()
                            self.env.cr.execute("""update alldo_acme_iot_electronic_scale set is_posting='2',picking_no=%d where id=%d""" % (myres.id, rec.id))
                            self.env.cr.execute("""commit""")
