...
class acmeiotmoldreportwizard(models.TransientModel):
    _name = "alldo_acme_iot.mold_report_wizard"

    report_type = fields.Selection([('1','自選模具條碼'),('2','全選模具條碼')],string="列印類別",default='1')
    mold_ids = fields.Many2many('alldo_acme_iot.acme_mold',string="模具")

    def run_mold_report(self):
        self.env.cr.execute("""delete from alldo_acme_iot_mold_report""")
        self.env.cr.execute("""commit""")
        mold_rec = self.env['alldo_acme_iot.mold_report']
        if self.report_type=='1':
           nitem = 1
           ncount = 1
           for rec in self.mold_ids:
               moldcode = self.env['alldo_acme_iot.acme_mold'].search([('id','=',rec.id)]).mold_no
               if nitem == 1:
                  if ncount == 1:
            ...
            mold1_rec = self.env['alldo_acme_iot.acme_mold'].search([('active','=',True)])
            mold_rec = self.env['alldo_acme_iot.mold_report']
            nitem = 1
            ncount = 1
        ...
        myviewid = self.env.ref('alldo_acme_iot.alldo_acme_mold_report_action')
        myrec = self.env['alldo_acme_iot.mold_report'].search([])
        myid = myrec[0].id
        return {'view_name': 'views_mold_report_action',
                'name': (u'mold report item Data'),
                'views': [[False,'form']],
                'res_model': 'alldo_acme_iot.mold_report',
