...
class acmeoutsourcinginwizard(models.TransientModel):
    _name = "alldo_acme_iot.outsourcing_in"
    _description = "委外加工完工回廠記錄"


    suborder_id = fields.Many2one('alldo_acme_iot.outsuborder',string="委外加工單",required=True)
    ...
    in_date = fields.Date(string="回廠日期",default=datetime.today())
    ...
    loc_type = fields.Selection([('1','ACME成品倉'),('2','ACME毛胚倉')],default='1',string="回廠倉別",required=True)

    @api.onchange('product_id')
    def onchangeproductid(self):
        myrec = self.env['alldo_acme_iot.outsuborder'].search([('product_no','=',self.product_id.id),('state','not in',['4','3'])])
        ids=[]
        for rec in myrec:
            ids.append(rec.id)
    ...
    def run_outsourcing_in(self):
        ...
        if self.suborder_id:
            if self.in_good_num > 0 or self.in_ng_num > 0 :
                self.env.cr.execute("""select genoutsourcingin(%d,%d,%d,%d,%d,'%s')""" % (self.suborder_id.id,0,self.in_owner.id,self.in_good_num,self.in_ng_num,self.in_date))
        ...
        if self.is_complete == True :
           self.env.cr.execute("""update alldo_acme_iot_outsuborder set state='4' where id=%d""" % self.suborder_id.id)
           self.env.cr.execute("""commit""")
