...
class acmeiotpostockin(models.TransientModel):
    _name = "alldo_acme_iot.po_stockin_wizard"
    _description = "製造生產單完工入庫"

    po_id = fields.Many2one('alldo_acme_iot.po_wkorder',string="客戶訂單")
    mo_no = fields.Many2one('alldo_acme_iot.workorder', string="入庫工單")
    ...
    def onchangeproductno(self):
        myrec = self.env['alldo_acme_iot.workorder'].search([('product_no','=',self.product_no.id),('state','not in',['4','5'])])
        ids=[]
        for rec in myrec:
            ids.append(rec.id)
    ...
    def run_po_stockin(self):
        ...
        if self.prod_num > 0 or self.ng_num > 0 :
            ...
            myres.action_done()
            self.env.cr.commit()
            mywkrec = self.env['alldo_acme_iot.workorder'].search([('id','=',self.mo_no.id)])
