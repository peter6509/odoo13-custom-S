...
class outtooutwizard(models.TransientModel):
    _name = "aldo_acme_iot.out_to_out_wizard"
    _description = "委外->委外加工物料移轉"

    outsourcing_no = fields.Many2one('alldo_acme_iot.outsuborder',string="Form 委外工單")
    sup_no = fields.Many2one('res.partner',string="From 委外商")
    sup_date_delivery = fields.Date(string="上工序交貨日",required=True,default=datetime.today())
    outsourcing_no1 = fields.Many2one('alldo_acme_iot.outsuborder',string="To 委外工單")
    sup_no1 = fields.Many2one('res.partner', string="To 委外商")
    sup1_date_due = fields.Date(string="下工序應交日",required=True)
    ...
    def onchangeproductno(self):
        myrec = self.env['alldo_acme_iot.outsuborder'].search([('product_no','=',self.product_no.id),('state','not in',['3','4'])])
        ids=[]
        for rec in myrec:
            ids.append(rec.id)
    ...
    def onchangeoutsourcing(self):
        myoutrec = self.env['alldo_acme_iot.outsuborder'].search([('id','=',self.outsourcing_no.id)])
        self.sup_no = myoutrec.cus_name.id
        # self.prod_num = myoutrec.blank_num - myoutrec.order_num
        # myoutrec1 = self.env['alldo_acme_iot.outsuborder'].search([('mrp_prod_id','=',myoutrec.mrp_prod_id.id)])
        myoutrec1 = self.env['alldo_acme_iot.outsuborder'].search([('product_no','=',self.product_no.id),('state','not in',['3','4'])])
        ids=[]
    ...
    def onchangeoutsourcing1(self):
        myoutrec1 = self.env['alldo_acme_iot.outsuborder'].search([('id', '=', self.outsourcing_no1.id)])
    ...
    def run_outtooutmove(self):
        ...
        if self.outsourcing_no and self.outsourcing_no1 :
            if self.prod_num > 0 or self. ng_num > 0 :
                self.env.cr.execute("""select genoutsourcingin(%d,%d,%d,%d,%d,'%s')""" % (self.outsourcing_no.id,self.outsourcing_no1.id,self.move_owner.id,self.prod_num,self.ng_num,self.sup_date_delivery))
                ...
                self.env.cr.execute("commit")
                self.env.cr.execute("""select genoutsourcingout(%d,%d,%d,%d,'%s','%s')""" % (self.outsourcing_no.id,self.outsourcing_no1.id, self.move_owner.id, self.prod_num,self.sup_date_delivery,self.sup1_date_due))
                self.env.cr.execute("commit")
                ...
                self.env.cr.execute("commit")
                self.env.cr.execute("""select gensubngratio(%d)""" % self.outsourcing_no.id)
                self.env.cr.execute("""commit""")
        else:
            if self.prod_num > 0 or self.ng_num > 0:
                self.env.cr.execute("""select geninpartner(%d,%d,%d,%d,%d,%d,'%s')""" % (self.sup_no.id, self.sup_no1.id, self.product_no.id, self.move_owner.id, self.prod_num, self.ng_num, ' '))
        ...
        ## 報廢倉位置(NG)
        myscraplocid = self.env['alldo_acme_iot.company_stockloc'].search([])[0].scrap_loc.id
        ## GOOD 加工完工回廠
        if self.prod_num > 0 :
