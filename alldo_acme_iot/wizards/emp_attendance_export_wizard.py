...
class empattendanceexportwizard(models.TransientModel):
    _name = "alldo_acme_iot.emp_attendance_export_wizard"
    ...
    def run_attendance_export(self):
        if not self.emp_no:
            ...
            self.env.cr.execute("""commit""")
        myattendrec =  self.env['alldo_acme_iot.emp_attendance_list'].search([])
        mycount = self.env['alldo_acme_iot.emp_attendance_list'].search_count([])
        output = io.BytesIO()
