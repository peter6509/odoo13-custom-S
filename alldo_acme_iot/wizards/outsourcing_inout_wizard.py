...
class outsourcinginoutwizard(models.TransientModel):
    _name = "alldo_acme_iot.outsourcing_inout_wizard"
    _description = "委外加工進出料記錄匯出EXCEL"
    ...
    def run_outsourcing_inout(self):
        self.env.cr.execute("""select genoutsourcinginout(%d,%d,'%s','%s')""" % (self.partner_id.id,self.product_id.id,self.start_date,self.end_date))
        self.env.cr.execute("""commit""")
        myinoutrec = self.env['alldo_acme_iot.inout_prod_list'].search([])
        mycount = self.env['alldo_acme_iot.inout_prod_list'].search_count([])
        output = io.BytesIO()


        myxlsfilename = "委外廠商進出料記錄_%s_%s.xlsx" % (self.product_id.name,datetime.now().strftime("%Y%m%d"))
        mysubject = '委外廠商進出料記錄_%s_%s.xlsx' % (self.product_id.name,datetime.now().strftime("%Y%m%d"))

        wb = xlsxwriter.Workbook(output, {'in_memory': True})

        wb.set_properties({
            'title': '正璽金屬工業委外料件進出記錄',
            'subject': mysubject,
        ...
        ws = wb.add_worksheet("正璽金屬工業委外料件進出記錄")
        ########################################
        ...
        titles = ['進出日期', '產品料號', '出廠數量', '回廠良品數', '回廠NG數', '餘額', '出廠承辦人', '回廠承辦人']
        title_width = [20, 60, 20, 20, 20, 20, 20, 20]
        ...
        for line in myinoutrec:
            ...
            ws.write(row, 1, line.product_id.name if line.product_id else ' ', okl_content_format)
            ws.write(row, 2, line.out_num if line.out_num else ' ', okc_content_format)
            ws.write(row, 3, line.in_good_num if line.in_good_num else ' ', okc_content_format)
            ws.write(row, 4, line.in_ng_num if line.in_ng_num else ' ', okc_content_format)
            ws.write(row, 5, line.balance_num if line.balance_num else '0', okc_content_format)
            ws.write(row, 6, line.out_owner.name if line.out_owner else ' ', okc_content_format)
            ws.write(row, 7, line.in_owner.name if line.in_owner else ' ', okc_content_format)
            row += 1
            nitem += 1
        ...
        if not self.run_desc:
            myrundesc = ' '
        else:
            myrundesc = self.run_desc
        if mycount > 0:
