...
class iotnewwkorderreportwizard(models.TransientModel):
    _name = "alldo_acme_iot.wkordernewreport_wizard"
    # 目前取消無用

    print_type = fields.Selection([('1','新工單列印'),('2','工單重印')],string="印表類別",required=True)
    workorder_ids = fields.Many2many('alldo_acme_iot.workorder','alldo_workorder_newreport_rel','wkorder_id','wizard_id',string="列印工單",domain="[('state','in',['2','3','4'])]")
    ...
    def run_workorder_report(self):
        ...
        myid=myrec[0].id
        myviewid = self.env.ref('alldo_acme_iot.action_wkorder_select_item_view')
        return {'view_name': 'wkorder_select_item',
                'name': (u'wkordr item Data'),
                'views': [[False, 'form']],
                'res_model': 'alldo_acme_iot.wkorder_selectitem',
