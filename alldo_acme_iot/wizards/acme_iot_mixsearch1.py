...
class alldoacmeiotmixsearch(models.TransientModel):
    ...
    def run_mixsearch(self):
        ...
        self.env.cr.execute("""select genwkperformance('%s','%s',%d,%d,%d)""" % (self.wk_start_date, self.wk_end_date, myprodid, myownerid,myequipid))
        ...
        return {
            ...
            'res_model': 'alldo_acme_iot.workorder_performance_list',
            'view_id': myviewid.id,
            'view_type': 'form',
            'view_mode': 'tree',
            'target': 'list'}
