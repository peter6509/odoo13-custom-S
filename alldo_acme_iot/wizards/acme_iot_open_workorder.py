...
class acmeiotopenworkorder(models.TransientModel):
    _name = "alldo_acme_iot.open_workorder_wizard"
    _description = "工單開工精靈"
    # ACME 目前使用製造命令直接開工單
    ...
    def run_open_workorder(self):
        ...
        if myfirstid != 0:
            myid1 = self.env['alldo_acme_iot.workorder'].search([('id', '=', myfirstid.id)])
            return {'view_name': 'alldoacmeiotworkorder',
                    'name': (u'工單作業'),
                    'views': [[False, 'form'], [False, 'tree']],
                    'res_model': 'alldo_acme_iot.workorder',
