...
class acmeiotngreturnexcelwizard(models.TransientModel):
    _name = "alldo_acme_iot.ngreturn_excel_wizard"
    ...
    def run_ngreturn_excel(self):
        ...
        myngreturnrec = self.env['alldo_acme_iot.ngreturn_list'].search([])
        mycount = self.env['alldo_acme_iot.ngreturn_list'].search_count([])
        output = io.BytesIO()
        ...
        ws = wb.add_worksheet("ACME委外加工記錄")
        ########################################
