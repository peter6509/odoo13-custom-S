...
class acmemixsearchscalemove(models.TransientModel):
    _name = "alldo_acme_iot.mixsearch_scalemove"
    ...
    def run_mixsearch_scalemove(self):
        ...
        myviewid = self.env.ref('alldo_acme_iot.acme_mixsearch_scalemovelist_tree')
        return {
            'view_name': 'acme_mixsearch_scalemove',
            'name': (u'磅秤投料入庫複合查詢'),
            'type': 'ir.actions.act_window',
            'res_model': 'alldo_acme_iot.mixsearch_scalemovelist',
            'view_id': myviewid.id,
            'view_type': 'form',
            'view_mode': 'tree',
            'target': 'current'}
