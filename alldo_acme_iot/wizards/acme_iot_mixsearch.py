...
class alldoacmeiotmixsearch(models.TransientModel):
    _name = "alldo_acme_iot.mix_search_wizard"
    ...
    def run_mixsearch(self):
        if self.search_type=='1':
            ...
            myviewid = self.env.ref('alldo_acme_iot.acmeiot_workorder_performance_tree')

            return {
                'view_name': 'workorder_performance_list',
                'name': (u'生產數據複合查詢統計'),
                'type': 'ir.actions.act_window',
                'res_model': 'alldo_acme_iot.workorder_performance_list1',
                'view_id': myviewid.id,
                'view_type': 'form',
                'view_mode': 'tree',
                'target': 'list'}
            ...
            self.env.cr.execute("""commit""")
            myviewid = self.env.ref('alldo_acme_iot.acme_iot_equipperformance_tree')

            return {
                'view_name': 'equipment_performance_list',
                'name': (u'生產數據機台稼動時間記錄'),
                'type': 'ir.actions.act_window',
                'res_model': 'alldo_acme_iot.equipment_performance_list',
                'view_id': myviewid.id,
                'view_type': 'form',
                'view_mode': 'tree',
                'target': 'list'}
            ...
            self.env.cr.execute("""commit""")
            myviewid = self.env.ref('alldo_acme_iot.acme_iot_manperformance_tree')
            return {
                'view_name': 'man_performance_list',
                'name': (u'人員達成率生產數據'),
                'type': 'ir.actions.act_window',
                'res_model': 'alldo_acme_iot.man_performance_list',
                'view_id': myviewid.id,
                'view_type': 'form',
                'view_mode': 'tree',
                'target': 'list'}
            ...
            self.env.cr.execute("""commit""")
            myviewid = self.env.ref('alldo_acme_iot.acmeiot_goodng_list_tree')
            return {
                'view_name': 'man_performance_list',
                'name': (u'生產良率/不良率數據統計'),
                'type': 'ir.actions.act_window',
                'res_model': 'alldo_acme_iot.prod_goodng_list',
                'view_id': myviewid.id,
                'view_type': 'form',
                'view_mode': 'tree',
                'target': 'list'}
