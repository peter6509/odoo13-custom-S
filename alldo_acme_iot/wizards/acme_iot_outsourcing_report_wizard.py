...
class acmeiotoutsourcingreportwizard(models.TransientModel):
    _name = "alldo_acme_iot.outsourcing_report_wizard"
    ...
    def run_outsourcing_report(self):
        ...
        myrec = self.env['alldo_acme_iot.outsourcing_report'].search([])
        for rec in myrec:
            myid = rec.id

        myviewid = self.env.ref('alldo_acme_iot.outsourcing_report_action')
        return {'view_name': 'outsourcing_report_action',
                'name': (u'Outsourcing Data'),
                'views': [[False, 'form']],
                'res_model': 'alldo_acme_iot.outsourcing_report',
