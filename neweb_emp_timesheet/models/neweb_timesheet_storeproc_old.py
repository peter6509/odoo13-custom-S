...
class newebtimesheetstoreproc(models.Model):
    ...
    def init(self):
        ...
        self._cr.execute("""create or replace function update_inspection_timesheet(contractid int) returns void as $BODY$
               DECLARE 
                 ins_cur refcursor ;
                 ins_rec record ;
                 ncount int ;
        ...
        self._cr.execute("""create or replace function delinspectiontimesheet(originid int) returns void as $BODY$
               DECLARE 
                 ncount int ;
                 lockdate date ;
               BEGIN
        ...
        self._cr.execute("""create or replace function update_repair_timesheet(repairid int) returns void as $BODY$
              DECLARE 
                 work_cur refcursor ;
                 work_rec record ;
                 ncount int ;
        ...
        self._cr.execute("""create or replace function del_repair_timesheet(repid int) returns void as $BODY$
              DECLARE 
                work_cur refcursor ;
                work_rec record ;
                lockdate date ;
        ...
        self._cr.execute("""create or replace function del_repair_work_log(originid int) returns void as $BODY$
              DECLARE 
                 ncount int ;
              BEGIN 
                  delete from neweb_emp_timesheet_timesheet_calendar_line where origin_id = originid and 
        ...
        self._cr.execute("""create or replace function del_assingline_timesheet(assignlineid int) returns void as $BODY$
                      DECLARE 
                         ncount int ;
                      BEGIN 
        ...
        self._cr.execute("""create or replace function gen_assign_timesheet(assignid int) returns void as $BODY$
                       DECLARE 
                         assl_cur refcursor ;
                         assl_rec record ;
                         ncount int ;
        ...
        self._cr.execute("""create or replace function del_assign_timesheet(assignid int) returns void as $BODY$
                 DECLARE 
                   assl_cur refcursor ;
                   assl_rec record ;
                 BEGIN
                   ...
                   loop
                      fetch assl_cur into assl_rec ;
                      exit when not found ;
                      execute del_assingline_timesheet(assl_rec.id) ;
                   end loop ;
                   close assl_cur ;
                 END;$BODY$
                 LANGUAGE plpgsql;""")
        ...
        self._cr.execute("""create or replace function gen_contract_list(contractid int,inspectionmode int) returns void as $BODY$
               ...
               BEGIN
                ...
                END LOOP;
                execute update_plan_ins_calendar(contractid) ;
               end; $BODY$
               LANGUAGE plpgsql;""")
        ...
        self._cr.execute("""create or replace function updaterepaircalendar(repid int) returns void as $BODY$
                 DECLARE 
                   ncount int ;
                   ncount1 int ;
                   rep_cur refcursor ;
        ...
        self._cr.execute("""create or replace function delrepaircalendar(repid int) returns void as $BODY$
                 DECLARE 
                   ncount int ;
                   lockdate date ;
                 BEGIN 
        ...
        self._cr.execute("""create or replace function runallrepcalendar() returns void as $BODY$
                 ...
                 BEGIN 
                   ...
                   loop
                      fetch rep_cur into rep_rec ;
                      exit when not found ;
                      execute updaterepaircalendar(rep_rec.id) ;
                   end loop ;
                   close rep_cur ;
                 END;$BODY$
                  LANGUAGE plpgsql;""")
        ...
        self._cr.execute("""create or replace function update_plan_ins_calendar(contractid int) returns void as $BODY$
                DECLARE 
                  con_cur refcursor ;
                  con_rec record ;
                  lockdate date ;
        ...
        self._cr.execute("""create or replace function del_plan_ins_calendar(inslistid int) returns void as $BODY$
                   DECLARE 
                      lockdate date ;
                      ncount int ;
                   BEGIN 
        ...
        self._cr.execute("""create or replace function runallinscalendar() returns void as $BODY$
                  ...
                  BEGIN 
                     ...
                     loop
                       ...
                       exit when not found ;
                       execute update_plan_ins_calendar(contract_rec.id) ;
                     end loop ;
                     close contract_cur ;
                  END;$BODY$
                  LANGUAGE plpgsql;""")
        ...
        self._cr.execute("""create or replace function import_timesheet_set(empid int,workdate varchar,starttime varchar,endtime varchar,worktype varchar,originno varchar,tmemo varchar) returns void as $BODY$
                 DECLARE 
                   ncount int ;
                   myyearmonth varchar ;
                   timesheetid int ;
                 ...
                 BEGIN 
                   ...
                   select to_timestamp(concat(workdate,' ',starttime),'YYYY-MM-DD HH24:MI:SS') into mystartdatetime ;
                   select to_timestamp(concat(workdate,' ',endtime),'YYYY-MM-DD HH24:MI:SS') into myenddatetime ; 
        ...
        self._cr.execute(
            """drop function if exists import_timesheet_set1(empid int,workdate varchar,starttime varchar,endtime varchar,worktype varchar,originno varchar,tmemo varchar,tdays int) cascade""")
        self._cr.execute("""create or replace function import_timesheet_set1(empid int,workdate varchar,starttime varchar,endtime varchar,worktype varchar,originno varchar,tmemo varchar,tdays int) returns void as $BODY$
                         DECLARE 
                           ncount int ;
                           myyearmonth varchar ;
                           timesheetid int ;
        ...
        self._cr.execute("""create or replace function update_inscalendar_datetime(calendarid int) returns void
                   as $BODY$ 
                      DECLARE 
                        mystartdate timestamp ;
                        myenddate timestamp ;
                      ...
                      BEGIN
                        select count(*) into ncount from neweb_emp_timesheet_inspection_calendar
                          ...
                          if ncount > 0 then  
                            ...
                            execute update_inspection_timesheet(mycontractid) ;
                          end if ;       
                      END;$BODY$
                      LANGUAGE plpgsql ;""")
        ...
        self._cr.execute("""create or replace function update_todocalendar(todoid int) returns void as $BODY$
               DECLARE
                 myinsid int ;
                 mytodoorigin char ;
